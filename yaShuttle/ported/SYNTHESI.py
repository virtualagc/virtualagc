...
def SYNTHESIZE(PRODUCTION_NUMBER):
        ...
        if tmt == 0:
            ...
            if (g.PSEUDO_FORM[I]==XSYT)or(g.PSEUDO_FORM[I]==XXPT):
                if g.VAR[g.SP]=='T': 
                    ...
                    if IMPLICIT_T:
                        g.SYT_FLAGS(g.LOC_P[I], SYT_FLAGS(g.LOC_P[I])|IMPL_T_FLAG);
    ...
    # reference 350 has been relocated.
        ...
        g.PTR[g.MP]=g.BLOCK_MODE[NEST+1]=UPDATE_MODE; # WHAT BLOCK WAS
    ...
    # reference 470 has been relocated.
        ...
        if SYT_CLASS(g.BLOCK_SYTREF[NEST])==FUNC_CLASS: 
            ERROR(d.CLASS_PF,1);
        ...
        if g.BLOCK_MODE[NEST]==UPDATE_MODE: 
            ERROR(d.CLASS_UP,2);
        elif SYT_CLASS(g.BLOCK_SYTREF[NEST])!=FUNC_CLASS:
            ERROR(d.CLASS_PF,2);
        else:
            g.PTR=0;
            g.LOC_P=g.BLOCK_SYTREF[NEST];
            g.PSEUDO_LENGTH[0]= VAR_LENGTH(LOC_P);
            g.TEMP=SYT_TYPE(g.BLOCK_SYTREF[NEST]);
        ...
        #DO CASE g.DO_INX[DO_LEVEL]&0x7F;
        didl = g.DO_INX[DO_LEVEL]&0x7F
        if didl == 0:
            # SIMPLE DO
            g.TEMP=XESMP;
        elif didl == 1:
        ...
        while SYT_LINK2(I)>0:
            ...
            if SYT_LINK1(J)<0: 
                if DO_LEVEL==(-SYT_LINK1(J)): 
                    ...
                    g.SYT_LINK2(I, SYT_LINK2(J));
            g.I=J;
        if DO_LOC==0: 
            g.I=g.DO_CHAIN[DO_LEVEL];
            while I>0:
                g.CLOSE_BCD=SYT_NAME(I);
                DISCONNECT(I);
                g.I=SYT_LINK1(I);
            g.DO_LEVEL=DO_LEVEL-1;
        else:
            g.DO_LOC=DO_LOC-1;
        goto_FIX_NOLAB = True
    ...
    # reference 690 has been relocated.
        ...
        if g.FIXL[g.MP] == 0: 
            ...
            PCARGp=PCARGp(g.FIXL[g.MP]);
            g.PCARGOFF=PCARGOFF(g.FIXL[g.MP]);
            ...
            ALT_PCARGp = ALT_PCARGp(g.FIXL[g.MP]);
        XSET (PC_STMT_TYPE_BASE + g.FIXL[g.MP]);
        HALMAT_POP(XPMHD,0,0,g.FIXL[g.MP]);
        ...
        if PCARGOFF>0: 
            if PCARGp==0: 
                ...
                ERROR(d.CLASS_XM,3);
            # LITERALS ILLEGAL
            elif (SHL(1,g.PSEUDO_TYPE_P[g.PTR[g.MP]])&PCARGTYPE(PCARGOFF))==0:
    ...
    # reference 1600 relocated.
        ...
        g.SYT_FLAGS(ID_LOC, SYT_FLAGS(ID_LOC)|g.TEMP);
        g.FIXL[g.MP] = ID_LOC
        g.DO_CHAIN=ID_LOC;
        ...
        if (g.DO_INX[DO_LEVEL] == 0) and g.IFDO_FLAG[DO_LEVEL]:
            g.MOVE_ELSE = g.FALSE;
        ...
        if (g.DO_INX[DO_LEVEL] == 0) and g.IFDO_FLAG[DO_LEVEL]:
            g.MOVE_ELSE = g.FALSE;
        g.SAVE_DO_LEVEL = DO_LEVEL;
        SET_LABEL_TYPE(g.FIXL[g.MP],STMT_LABEL);
    ...
    if goto_CHECK_DECLS or PRODUCTION_NUMBER == 295: # reference 2950
        ...
        goto_CHECK_DECLS = False
        g.I = g.BLOCK_MODE[NEST];
        if (I == FUNC_MODE) or (I == PROC_MODE): 
            g.J=g.BLOCK_SYTREF[NEST];  # PROC FUNC NAME
            if SYT_PTR(J) != 0: 
                ...
                while (SYT_FLAGS(J) & PARM_FLAGS) != 0:
                    if (SYT_FLAGS(J) & IMP_DECL) != 0:
                        ...
                        g.SYT_FLAGS(J, SYT_FLAGS(J) | DEFAULT_ATTR);
                ...
                if (EXTERNAL_MODE > 0) and (EXTERNAL_MODE < CMPL_MODE):
                    while J <= NDECSY:
                        if SYT_CLASS(J)<REPL_ARG_CLASS:
                            ...
                            g.SYT_FLAGS(J, SYT_FLAGS(J) | DUMMY_FLAG);
                    g.J = J + 1;
                END;
        if EXTERNALIZE: 
            g.EXTERNALIZE=4;
    ...
    # reference 2970 relocated
        ...
        if TPL_REMOTE:
            if EXTERNAL_MODE>0 and NEST==1 and g.BLOCK_MODE[NEST]==CMPL_MODE:
                g.SYT_FLAGS(g.FIXL[g.MP], SYT_FLAGS(g.FIXL[g.MP]) | REMOTE_FLAG);
            else:
                ERROR(d.CLASS_PS,13);
        ...
        if g.BLOCK_MODE[NEST]>PROG_MODE:
            ...
            ERROR(d.CLASS_PS, 4);
        elif (SYT_FLAGS(g.FIXL[g.MP])&ACCESS_FLAG)!=0: 
            ERROR(d.CLASS_PS,10);
        else:
            g.SYT_FLAGS(g.FIXL[g.MP], SYT_FLAGS(g.FIXL[g.MP]) | ACCESS_FLAG);
        ...
        if g.BLOCK_MODE[NEST]!=CMPL_MODE: 
            ERROR(d.CLASS_PS,12);
        elif (SYT_FLAGS(g.FIXL[g.MP]) and RIGID_FLAG)!=0: 
            ERROR(d.CLASS_PS,11);
        else:
            g.SYT_FLAGS(g.FIXL[g.MP], SYT_FLAGS(g.FIXL[g.MP])|RIGID_FLAG);
        ...
        g.SYT_FLAGS(g.FIXL[g.MP], SYT_FLAGS(g.FIXL[g.MP])|EXCLUSIVE_FLAG);
    elif PRODUCTION_NUMBER == 307: # reference 3070
        #  <BLOCK STMT TOP>  ::=  <BLOCK STMT HEAD>  REENTRANT
        if g.BLOCK_MODE[NEST]>FUNC_MODE: 
            ERROR(d.CLASS_PS,2,'REENTRANT');
        g.SYT_FLAGS(g.FIXL[g.MP], SYT_FLAGS(g.FIXL[g.MP])|REENTRANT_FLAG);
        ...
        g.TEMP=g.FIXL[g.MP];
        if SYT_TYPE(g.TEMP) != IND_CALL_LAB: 
            if SYT_LINK1(g.TEMP)>0: 
                if DO_LEVEL>0:
                    if DO_STMTp(DO_LEVEL) > SYT_LINK1(g.TEMP):
                        if SYT_TYPE(g.TEMP) == STMT_LABEL: 
                            ERROR(d.CLASS_GL, 2);
                        else:
                            ERROR(d.CLASS_PL, 10);
            if SYT_LINK1(g.TEMP) >= 0: 
                g.SYT_LINK1(g.TEMP, -DO_LEVEL);
                g.SYT_LINK2(g.TEMP, SYT_LINK2(0));
                g.SYT_LINK2(0, g.TEMP);
        g.LABEL_COUNT=LABEL_COUNT+1;
        if SIMULATING: 
            STAB_LAB(g.FIXL[g.MP]);
        g.GRAMMAR_FLAGS[g.STACK_PTR[g.MP]]=g.GRAMMAR_FLAGS[g.STACK_PTR[g.MP]]|LABEL_FLAG;
        ...
        if (SHR(XREF(g.SYT_XREF(g.FIXL[g.MP])),13) & 7) == 0:
            if (XREF(g.SYT_XREF(g.FIXL[g.MP])) & XREF_MASK) != g.STMT_NUM():
                g.XREF(g.SYT_XREF(g.FIXL[g.MP]), \
                    (XREF(g.SYT_XREF(g.FIXL[g.MP])) & 0xFFFFE000) | g.STMT_NUM());
        if SYT_TYPE(g.FIXL[g.MP])==STMT_LABEL:
        ...
        if NEST>0: 
            ...
            g.SYT_FLAGS(g.FIXL[g.MP], SYT_FLAGS(g.FIXL[g.MP]) | EXTERNAL_FLAG);
    ...
    # reference 3110 relocated
        ...
        g.SYT_FLAGS(g.FIXL[g.MP], SYT_FLAGS(g.FIXL[g.MP])|LATCHED_FLAG);
        if NEST==1:
    ...
    # reference 3140 relocated
        ...
        g.SYT_FLAGS(I, SYT_FLAGS(I)|DEFINED_LABEL);
        g.VAR_LENGTH(I, 2);
        if SIMULATING: 
            STAB_LAB(g.FIXL[g.MP]);
    ...
    # reference 3340 relocated.
        ...
        if (SYT_FLAGS(g.BLOCK_SYTREF[NEST]) & EXTERNAL_FLAG) != 0:
            g.SYT_FLAGS(I, SYT_FLAGS(I) | DUMMY_FLAG);  # IGNORE IN TEMPLATES
        ...
        if (SYT_FLAGS(J) & AUTO_FLAG) != 0:
            if (SYT_FLAGS(g.BLOCK_SYTREF[NEST]) & REENTRANT_FLAG) != 0:
    ...
    # reference 3540 was relocated
        ...
        if SYT_TYPE(STRUC_PTR) != TEMPL_NAME:
            g.SYT_FLAGS(STRUC_PTR, SYT_FLAGS(STRUC_PTR) | EVIL_FLAG);
    ...
    # reference 4090 relocated
        ...
        g.SYT_FLAGS2(ID_LOC, SYT_FLAGS2(ID_LOC) | NONHAL_FLAG);
    ...
    # reference 4360 relocated
        ...
        if (SYT_FLAGS(g.FIXL[g.MPP1]) & ACCESS_FLAG) != 0:
    ...
    if goto_UPDATE_CHECK or PRODUCTION_NUMBER == 61: # reference 610
        ...
        while g.TEMP>1:
            ...
            if LABEL_MATCH: 
                HALMAT_POP(XBRA,1,0,0);
                HALMAT_PIP(DO_LOC(g.TEMP),XINL,0,0);
                g.TEMP=1;
            g.TEMP=g.TEMP-1;
        if g.TEMP==1: 
            ERROR(d.CLASS_GE,1);
        ...
        while g.TEMP>1:
            ...
            if g.DO_INX[g.TEMP]: 
                if LABEL_MATCH:
                    HALMAT_POP(XBRA,1,0,0);
                    HALMAT_PIP(DO_LOC(g.TEMP)+1,XINL,0,0);
                    g.TEMP=1;
            g.TEMP=g.TEMP-1;
        if g.TEMP==1: 
            ERROR(d.CLASS_GE,2);
    ...
    if goto_EMIT_CASE or PRODUCTION_NUMBER == 150: # reference 1500
        ...
        if not goto_EMIT_CASE:
            g.FIXV[g.MP]=1;
        if goto_EMIT_CASE or (g.DO_INX[DO_LEVEL]&0x7F)==2: 
    ...
    if goto_DO_DISCRETE or PRODUCTION_NUMBER == 160: # reference 1600
        ...
        if INLINE_LEVEL>0: 
            ERROR(d.CLASS_PP,6);
        if g.TEMP>0: 
            if (SYT_FLAGS(g.FIXL[g.MPP1])&LATCHED_FLAG)==0:
                ERROR(d.CLASS_RT,10);
    ...
    if goto_MOST_IDS or goto_STRUC_IDS or \
        ...
        if not goto_STRUC_IDS:
            goto_MOST_IDS = False
            l.H1=g.PTR[g.MP];
            g.PSEUDO_TYPE_P[l.H1]=SYT_TYPE(g.FIXL[g.MPP1]);
        ...
        if g.FIXV[g.MP]>0:
            ...
            l.H2 = (l.H2 or (SYT_FLAGS(g.FIXL[g.MP]) & NAME_FLAG) != 0);
        ...
        if g.TEMP>0:
            HALMAT_POP(XXXST,1,XCO_N,g.TEMP+FCN_LV-1);
    ...
    if goto_DO_BIT_CONSTANT_END or PRODUCTION_NUMBER == 270: # reference 2700
        ...
        if g.PSEUDO_TYPE_P[g.PTR[g.MP]]==MAJ_STRUC:
            if (SYT_FLAGS(VAR_LENGTH(g.FIXV[g.MP]))&MISC_NAME_FLAG)!=0:
                ERROR(d.CLASS_T,6);
        EMIT_ARRAYNESS();
        g.PTR_TOP=g.PTR[g.MP]-1;
    ...
    if goto_CLOSE_SCOPE or PRODUCTION_NUMBER == 293: # reference 2930
        ...
        HALMAT_PIP(g.BLOCK_SYTREF[NEST],XSYT,0,0);
        ...
        if (SYT_FLAGS(g.FIXL[g.MP]) & ACCESS_FLAG) != 0:
            if g.BLOCK_MODE[NEST] == CMPL_MODE:
                for I in range(g.FIXL[g.MP], 1 + NDECSY):
                    g.SYT_FLAGS(I, SYT_FLAGS(I) | READ_ACCESS_FLAG);
        ...
        if ((SYT_FLAGS(g.FIXL[g.MP]) & EXTERNAL_FLAG) != 0) \
                and ((SYT_FLAGS(g.FIXL[g.MP]) & REMOTE_FLAG) != 0):
            if g.BLOCK_MODE[NEST] == CMPL_MODE:
                for I in range(g.FIXL[g.MP], 1 + NDECSY):
        ...
        g.SYT_FLAGS(NDECSY, SYT_FLAGS(NDECSY) | ENDSCOPE_FLAG);
        ...
        if g.BLOCK_MODE[NEST]==INLINE_MODE: 
            ...
            BLOCK_SUMMARY();
            g.OUTER_REF_INDEX=(g.OUTER_REF_PTR_P[NEST]&0x7FFF)-1;
            g.INDENT_LEVEL = g.INDENT_STACK[NEST];
            g.NEST_LEVEL = g.NEST_STACK[NEST];
        g.REGULAR_PROCMARK=g.PROCMARK_STACK[NEST];
        g.PROCMARK = REGULAR_PROCMARK
        g.SCOPEp = g.SCOPEp_STACK[NEST];
        g.I=0;
        while ON_ERROR_PTR<-SYT_ARRAY(g.BLOCK_SYTREF[NEST]):
            if EXT_ARRAY(ON_ERROR_PTR)==0x3FFF: 
                g.I=I+0x1001;
            else:
                g.I=I+1;
            g.ON_ERROR_PTR=ON_ERROR_PTR+1;
        g.SYT_ARRAY(g.BLOCK_SYTREF[NEST], I|0xE000);
        g.NEST = NEST - 1;
        g.DO_INX[DO_LEVEL]=g.DO_INX[DO_LEVEL]&0x7F;
        if NEST==0:
            if EXTERNAL_MODE>0: 
                if SIMULATING==2:
                    g.STMT_TYPE=0;
                    g.SIMULATING=1;
                g.EXTERNAL_MODE=0;
                g.TPL_VERSION=g.BLOCK_SYTREF[NEST+1];
        ...
        SET_XREF(I,XREF_REF);
        g.SYT_TYPE(I, TYPE);
        g.SYT_FLAGS(I, SYT_FLAGS(I)|DEFINED_LABEL|ATTRIBUTES);
        ...
        goto_INIT_MACRO = False
        g.CONTEXT = 0;
        g.MACRO_TEXT(FIRST_FREE, 0xEF);  # INITIALIZE TO NULL
        ...
        g.SYT_FLAGS(ID_LOC, SYT_FLAGS(ID_LOC)|ATTRIBUTES);
        HALMAT_INIT_CONST();
        # See the comments in g.py for TYPE to understand the next few lines
        g.TYPE = 0
        g.BIT_LENGTH = 0
    ...
    if goto_STRUCT_GOING_UP or goto_STRUCT_GOING_DOWN or \
        ...
        if not goto_STRUCT_GOING_DOWN:
            ...
            if (SYT_FLAGS(ID_LOC)&DUPL_FLAG)!=0:
                g.SYT_FLAGS(ID_LOC, SYT_FLAGS(ID_LOC)&(not DUPL_FLAG));
                g.S=SYT_NAME(ID_LOC);
                g.TEMP_SYN=SYT_LINK1(g.FIXL[g.MP]);
                while TEMP_SYN!=ID_LOC:
                    if S==SYT_NAME(TEMP_SYN):
                        ERROR(d.CLASS_DQ,9,S);
                        g.S='';
                    g.TEMP_SYN=SYT_LINK2(TEMP_SYN);
        if goto_STRUCT_GOING_DOWN or g.FIXV[g.SP]>g.FIXV[g.MP]:
            if not goto_STRUCT_GOING_DOWN: 
                ...
                g.TYPE = MAJ_STRUC;
                if (ATTRIBUTES & ALDENSE_FLAGS) == 0:
                    g.ATTRIBUTES = ATTRIBUTES | (SYT_FLAGS(g.FIXL[g.MP]) & ALDENSE_FLAGS);
                # GIVE ALIGNED/DENSE OF PARENT IF NOT LOCALLY SPECIFIED
                if (ATTRIBUTES&RIGID_FLAG)==0:
                    g.ATTRIBUTES=ATTRIBUTES|(SYT_FLAGS(g.FIXL[g.MP])&RIGID_FLAG);
                SET_SYT_ENTRIES();
                if g.STACK_PTR[g.SP] > 0:
                    OUTPUT_WRITER(g.LAST_WRITE, g.STACK_PTR[g.SP] - 1);
                g.LAST_WRITE = g.STACK_PTR[g.SP];
            ...
            if TYPE==0: 
                ...
                if not NAME_IMPLIED:
                    if STRUC_PTR==REF_ID_LOC:
                        ERROR(d.CLASS_DT,6);
                        g.SYT_FLAGS(REF_ID_LOC, SYT_FLAGS(REF_ID_LOC)|EVIL_FLAG);
            if CLASS!=0: 
                if NAME_IMPLIED:
            ...
            if (ATTRIBUTES & ALDENSE_FLAGS) == 0:
                if (not NAME_IMPLIED) and ((ATTRIBUTES&ARRAY_FLAG)==0) and \
                        (TYPE==BIT_TYPE):
                    g.ATTRIBUTES=ATTRIBUTES|(SYT_FLAGS(g.FIXL[g.SP])&ALDENSE_FLAGS);
                else:
                    g.ATTRIBUTES=ATTRIBUTES|ALIGNED_FLAG;
            if (ATTRIBUTES&RIGID_FLAG)==0:
                g.ATTRIBUTES=ATTRIBUTES|(SYT_FLAGS(g.FIXL[g.SP])&RIGID_FLAG);
            if NAME_IMPLIED: 
                g.SYT_FLAGS(REF_ID_LOC, SYT_FLAGS(REF_ID_LOC) | MISC_NAME_FLAG);
            SET_SYT_ENTRIES();
        ...
        if TEMPORARY_IMPLIED:
            g.ATTR_INDENT=10;
            if g.DO_CHAIN[DO_LEVEL]==0:
                g.DO_CHAIN[DO_LEVEL]=ID_LOC;
                HALMAT_POP(XTDCL,1,0,0);
                HALMAT_PIP(ID_LOC,XSYT,0,0);
            else: 
                g.SYT_LINK1(DO_CHAIN, ID_LOC);
            g.DO_CHAIN=ID_LOC;
        else:
            g.ATTR_INDENT=8;
        ...
        if DATA_REMOTE: 
            if ((SYT_FLAGS(ID_LOC) & NAME_FLAG) == 0) and \
                    ...
                    ((SYT_FLAGS(g.BLOCK_SYTREF[NEST]) & EXTERNAL_FLAG)==0) and \
                    not (((SYT_FLAGS(ID_LOC) & AUTO_FLAG) != 0) and \
                    ((SYT_FLAGS(g.BLOCK_SYTREF[NEST]) & REENTRANT_FLAG) != 0)) and \
                    ((SYT_FLAGS(ID_LOC) & PARM_FLAGS) == 0):
                if (SYT_FLAGS(ID_LOC) & REMOTE_FLAG) != 0:
                    ERROR(d.CLASS_XR,3);
                if SYT_TYPE(ID_LOC) == EVENT_TYPE:
                    ERROR(d.CLASS_DA,9);
                g.SYT_FLAGS(ID_LOC, SYT_FLAGS(ID_LOC) | REMOTE_FLAG);
            elif SDL_OPTION and \
        ...
        if not goto_END_REPEAT_INIT:
            ...
            if NAME_PSEUDOS:
                ...
                if SYT_CLASS(g.FIXL[g.MP]) == TEMPLATE_CLASS:
                    if (((SYT_FLAGS(g.FIXV[g.MP]) & ASSIGN_PARM) > 0) or \
                            (((SYT_FLAGS(g.FIXV[g.MP]) & AUTO_FLAG) != 0) and \
                             ((SYT_FLAGS(g.BLOCK_SYTREF[NEST]) and \
                               REENTRANT_FLAG)!=0))):
                        ERROR(d.CLASS_DI,3);
                elif (((SYT_FLAGS(g.FIXL[g.MP]) & ASSIGN_PARM) > 0) or \
                        (((SYT_FLAGS(g.FIXL[g.MP]) & AUTO_FLAG) != 0) and \
                         ((SYT_FLAGS(g.BLOCK_SYTREF[NEST]) and \
                           REENTRANT_FLAG)!=0))):
                    ERROR(d.CLASS_DI,3);
    ...
    while goto_OUTERMOST_BLOCK or goto_DUPLICATE_BLOCK or \
        ...
        if goto_OUTERMOST_BLOCK or goto_DUPLICATE_BLOCK or \
            ...
            if NEST>0 and not goto_DUPLICATE_BLOCK: 
                ...
                goto_DUPLICATE_BLOCK = False
                if g.BLOCK_MODE[0]==0: 
                    g.MAIN_SCOPE=MAX_SCOPEp+1;  # WHAT SYT_SCOPE WILL BECOME
        ...
        if goto_PROC_FUNC_HEAD or goto_NEW_SCOPE or goto_INLINE_ENTRY or \
            ...
            g.XSET(0x16);
            g.NEST=NEST+1;
            g.DO_INX[DO_LEVEL]=g.DO_INX[DO_LEVEL]|0x80;
            g.BLOCK_MODE[NEST]=TEMP2;
            g.BLOCK_SYTREF[NEST]=g.FIXL[g.MP];
            SYT_ARRAY(g.BLOCK_SYTREF[NEST], -ON_ERROR_PTR);
            if NEST > MAXNEST:
                g.MAXNEST = NEST;
                if NEST >= NEST_LIM:
                    ERROR(d.CLASS_BN,1);
            g.SCOPEp_STACK[NEST] = SCOPEp;
            SCOPEp = MAX_SCOPEp + 1;
            MAX_SCOPEp = SCOPEp
            if (SYT_FLAGS(g.FIXL[g.MP])&EXTERNAL_FLAG) != 0: 
                NEXT_ELEMENT(CSECT_LENGTHS);
                g.CSECT_LENGTHS[SCOPEp].PRIMARY = 0x7FFF;# SET LENGTH TO MAX
                g.CSECT_LENGTHS[SCOPEp].REMOTE = 0x7FFF; # TO TURN OFF PHASE2
                # %COPY CHECKING
            SYT_SCOPE(g.FIXL[g.MP], SCOPEp); # UPDATE BLOCK NAME TO SAME SCOPE
            # AS CONTENTS
            g.PROCMARK_STACK[NEST] = PROCMARK;
            g.PROCMARK = NDECSY + 1;
            ...
            COMPRESS_OUTER_REF();
            g.NEST = NEST + 1;
            g.OUTER_REF_PTR_P[NEST]=OUTER_REF_INDEX+1;
            ...
            if g.BLOCK_MODE[NEST]==INLINE_MODE: 
                ...
                g.INDENT_STACK[NEST] = g.INDENT_LEVEL;
                g.NEST_STACK[NEST] = NEST_LEVEL;
