...
Templates are imported via the "`D INCLUDE TEMPLATE name`" compiler directive.  The compiler has several possible sources, all of them PDS files, and prioritizes searches for the member `name` among these files.  (Actually, a mangled form of `name` is sought, but that's not immediately relevant to the issues I'm confronting, so I'll ignore that.)  The prioritized PDS files searched, in descending order, are:
 ...
 1. The file attached to `INPUT4`, which I think may be called "TEMPLIB" (for "template library" rather than "temporary library"; for us, the *directory* "TEMPLIBp", or "TEMPLIBBp" with `--bfs`).  This would be a permanent, persistent template library.
...
> **Note on the physical form of these libraries:**  `INPUT4`/`OUTPUT6`'s permanent template library and the inclusion library on `INPUT8` are each a *directory* of members, one file per member, encoded in EBCDIC — exactly the form `HALSFC-PASS1` reads and writes, which is how XCOM-I implements a PDS given the `,E` suffix on a `--pdsi=`/`--pdso=` switch.  Every record in a member is 80 bytes, blank-padded with EBCDIC 0x40.  They used to be the JSON files "TEMPLIB.json" and "INCLIB.json", and the reason they are not any more is concurrency: a JSON file holds the whole library, so two compilations cannot both rewrite it, whereas two compilations can write different members of a directory.  That is what `HALSFC --concurrent` needs in order to compile a corpus several files at a time.
>
> The inclusion library on `INPUT8` is read and never written, and its members are pure EBCDIC, so it is *the same* `INCLIB` directory `HALSFC-PASS1` reads, translated from EBCDIC on the fly rather than kept as a second copy in step with the first.  The template library cannot be shared that way, because each compilation bumps a member's version code and one library written by both compilers would be bumped twice; hence "TEMPLIBp", beside PASS1's own "TEMPLIB".  With the two in the same format, `HALSFC --test` can compare them member for member, which it now does.
>
> Neither directory has to be in the compiler's working directory: `--templib-dir=D` and `--inclib-dir=D` say where they are.  That matters under `--concurrent`, where the working directory is a private one and the libraries stay where `HALSFC` was run from.
>
> The temporary datasets are unchanged, being per-compilation rather than persistent: `OUTPUT6` without `--templib` is still "&&TEMPLIB.json", and `OUTPUT8`/`INPUT9` are still "&&TEMPINC.json".  A private working directory is what keeps those from colliding.
