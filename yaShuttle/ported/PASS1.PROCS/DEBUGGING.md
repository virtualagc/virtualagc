...
Each run creates a new numbered subdirectory under archive.results/; current.results is always a symlink-like pointer to the newest one, current-1.results to the previous one, current-2.results to the one before that, and so on. This makes it easy to compare a fresh run against earlier attempts while iterating on a fix. TEMPLIBp/ (TEMPLIBBp/ for BFS), &&TEMPINC/, &&TEMPLIB.json, &&TEMPINC.json, sdfpkg.py, and the archive.results/current*.results directories/files are all compiler-generated artifacts of this run, not source files — ignore them unless you're specifically inspecting compiler state. INCLIB/ is *not* generated by a run: it is a persistent library, built by prepareINCLIB and shared with HALSFC-PASS1.

(Under `HALSFC --concurrent` there is no current.results, since that is a single name several simultaneous runs would fight over; each run's results directory is named for its source file and timestamp under archive.results/ as usual.)

The members of the template and inclusion libraries are EBCDIC, so `cat` shows nothing useful. Read one with a hexdump, or with asciiToEbcdic.py's standalone conversion mode. They used to be the JSON files INCLIB.json and TEMPLIB.json, which were readable but could not be written by two compilations at once.
