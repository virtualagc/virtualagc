#!/usr/bin/env bash
# Proves interp_run()'s wall-clock burst-execute-then-sleep pacing
# (state.h's scheduler comment, HALMAT_REALTIME_BURST_MS) actually
# throttles execution against the real clock. Every other WAIT/SCHEDULE
# fixture in this suite passes a large --time-scale specifically to make
# its sleep negligible, so none of them can catch a regression that
# silently disables throttling entirely (or one that hangs/massively
...
#
# Usage: run_realtime_fixture.sh NAME EXPECTED_OUTPUT EXPECTED_SECONDS
...
if [ "$actual" = "$expected" ] && [ "$in_bounds" = "yes" ]; then
    echo "PASS: realtime($name) elapsed=${elapsed}s (expected ~${expected_seconds}s, bounds [${min_ok}s, ${max_ok}s])"
    exit 0
else
    echo "FAIL: realtime($name)"
