...
struct halmat_state {
    ...
    int64_t wallclock_anchor; /* Unix epoch seconds of the mission clock at program start, supplied by
                                * whatever INTEGRATES the engine (the CLI, a --compile'd F, or an
                                * embedding app) via interp_set_wallclock_override() -- the engine itself
                                * only ever knows emulated time from start, so this real-world reference
                                * comes from outside it. DATE()/CLOCKTIME() (USA00309 Sec. 8.2) read this
                                * anchor advanced by the *virtual* elapsed time (virtual_time /
                                * HALMAT_TICKS_PER_SECOND), so the calendar/clock progress on the same
                                * simulated base as RUNTIME(). 0 (Unix epoch) if the integrator never sets
                                * it -- a deterministic default for direct/embedded construction. Kept as
                                * int64_t (not time_t) so state.h needn't pull in <time.h> everywhere. */
