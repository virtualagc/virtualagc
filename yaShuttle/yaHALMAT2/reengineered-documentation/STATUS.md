...
| [USA003088] | **Full text extracted** (237 pages); §10.1.1 ("The READ and READALL Statements," pp. 10-1–10-4) read in full, remainder not yet reviewed | "HAL/S Language Specification" (2005) — the formal syntax/semantics companion to [USA003087]'s Programmer's Guide, cited *by* [USA003087] itself (its Sec. 12.3 READ discussion points here for "further clarification," Sec. 10.1.1). User-supplied pointer, prompted by a follow-up question about [READ](class-0/READ.md)'s comma-separator fix (a *leading* comma, e.g. `READ(5) A,B,C;` fed `",2,3"`) — extracted the same way as [USA003087]/[USA003090] into `source-documentation/USA003088.txt`. Local copy found at `../../Desktop/sandroid.org/public_html/apollo/Shuttle/HAL_S Language Specification Nov 2005.pdf` (same mirror as other sources — see "Alternate source locations" above); also at `https://www.ibiblio.org/apollo/Shuttle/HAL_S%20Language%20Specification%20Nov%202005.pdf`. §10.1.1 gives a considerably more precise, rule-numbered account of READ's field-separator/null-field mechanism than [USA003087] §12.3's prose summary — see the "READ leading-comma null field" section below for what this settled. **Discrepancy noted, resolved in a follow-up session**: §10.1.1 rule 4 states "Fields may not cross line boundaries except when reading character strings," which appears to conflict with [USA003087] §12.3's "Data fields may be broken over the line boundary" — [Programming in HAL/S] §8.3 (see below) settled this in [USA003088]'s favor (the narrow reading: individual field *values* can't straddle a line, but a multi-item list continues across lines freely) and separately confirmed the "missing trailing item(s)" mechanism is a semicolon terminator, not short lines — see the "READ semicolon list-terminator" section below. |
| [Programming in HAL/S] | **Full text extracted** (360 pages); §8.3 ("The READ Statement," p. 153) read in full, remainder not yet reviewed | "Programming in HAL/S" (Sept. 1978) — **not primary source** (per direct user guidance: a generic introductory HAL/S textbook, not specific to this project's actual HAL/S-FC compiler), but flagged as significant since it's typically the first HAL/S material a student encounters, and this project's own `Source Code/Programming in HAL-S/*.hal` regression-test corpus is drawn directly from its worked examples. User-supplied pointer, prompted by the same READ-semicolon question that resolved the [USA003088] discrepancy above. Extracted the same way as the other sources into `source-documentation/ProgrammingInHALS.txt`. Local copy found at `../../Desktop/sandroid.org/public_html/apollo/Shuttle/Programming in HAL_S Sept 1978.pdf` (same mirror as other sources — see "Alternate source locations" above); also at `https://www.ibiblio.org/apollo/Shuttle/Programming%20in%20HAL_S%20Sept%201978.pdf`. §8.3's own worked example (`READ(5) A,B,C;` fed `"1.5, 2.6;"`) independently confirms [USA003088] §10.1.1 rule 5's semicolon-terminates-the-list behavior — see the "READ semicolon list-terminator" section below. |
...
| 0x019 | DSUB | Documented | High | 93/101 | Opcode confirmed ([##DRIVER.xpl] `XDSUB`); consolidates 9 separate HAL-1971 subscript-specifier instructions (ALC/ALCE/RALC + TASB/TIDX/TTSB + AASB/AIDX/ATSB + SASB/SIDX/STSB) into one opcode — see DSUB.md's Source Analysis. All rows of the primary-source subscript-kind table (index, asterisk, array/component to-partition, at-partition, CSZ cases) now empirically confirmed, including a full sweep confirming `VECTOR`/`MATRIX` element subscripting reuses the table's "component" α column (0-3), not a separate scheme; ASZ traced instead to ADLP's operand list |
...
**Resolved in a follow-up session — the user's own speculation
withdrawn, and the real mechanism found instead.** ["Programming in
HAL/S"] §8.3 (p. 153, newly consulted — see below) settles this
directly: HAL/S *does* have a documented "process a variable number of
input values" idiom, but it's driven by an explicit **semicolon**
terminator, not by short physical lines. §8.3's own prose confirms the
Figure-12-4 reading above was the right one to trust: "a READ statement
may take data from many lines of a file. Lines will be processed until
*either a semicolon is reached or values are found for all of the
operands* of the READ statement" — i.e. only those two things end a
READ list early; a short line by itself is not among them. That same
passage also resolves the [USA003087]-vs-[USA003088] "may fields cross
line boundaries" tension noted above: "[t]he end of each line of input
(e.g., card column 80) serves as a delimiter equivalent to a blank.
Hence, *individual values* may not be split across lines" — confirming
the narrow reading (a single field's own characters can't straddle a
line break; the list as a whole continues across lines freely) is
correct, and [USA003087]'s looser "data fields may be broken over the
line boundary" phrasing was simply imprecise rather than genuinely
contradicting [USA003088]. See the "READ semicolon list-terminator"
section immediately below for the fix this prompted.

## READ semicolon list-terminator

Direct follow-up, prompted by the "missing trailing item(s)" question
left open just above: the user pointed to ["Programming in HAL/S"] §8.3
(p. 153) — a textbook, explicitly **not** treated as primary source in
this project (it's generic to HAL/S, not specific to this project's
actual HAL/S-FC compiler), but flagged as significant since it's
typically the first HAL/S material a student encounters, and extracted
into `source-documentation/ProgrammingInHALS.txt` this session (same
`pdftotext -layout` convention as the other sources; local copy at
`../../Desktop/sandroid.org/public_html/apollo/Shuttle/Programming in
HAL_S Sept 1978.pdf`, also `https://www.ibiblio.org/apollo/Shuttle/
Programming%20in%20HAL_S%20Sept%201978.pdf`). §8.3's own worked
example: `READ(5) A, B, C;` fed `"1.5, 2.6;"` reads only `A`/`B`,
leaving `C` untouched — "[t]his fact can be useful when a program must
process a variable number of input values," illustrated there with an
`ARRAY`-sum idiom reading a semicolon-terminated list into an array,
leaving its unfilled tail at initial value. Independently confirmed by
[USA003088] §10.1.1 rule 5, already extracted (a stronger, distinct
effect from an ordinary comma null field): "a semicolon field separator
encountered during a normal sequential scan to fill a variable
element terminates the READ statement...[t]he current `<variable>`
element is left unchanged; [a]ll remaining `<variable>`s in the
statement are unchanged." yaHALMAT2 previously hit a hard parse error
on any semicolon in READ input (`fscanf` choking on `;` as invalid
data) — user-reported.

Fixed by widening `read_skip_separator()`'s return from a bool into a
three-way `halmat_read_field_t` (`DATA`/`NULL`/`TERMINATE`) — `interp.c`.
**Took two passes to get right**, caught by testing the primary
source's own worked example directly rather than a simplified case: the
first attempt's "no comma found here, this must be plain space-only
separation" early-return path skipped the semicolon check entirely, so
a comma-then-semicolon input (`"1.5, 2.6;"`, the exact §8.3 example)
still failed even though a *bare* leading semicolon alone (nothing else
in the input) already worked — the semicolon check needed to run
whether or not a comma preceded it, since [USA003088] rule 4 describes
the scan as "looking for fields...separated by commas, semicolons, *or
blanks*" (a semicolon reached via plain blanks alone is just as
terminating as one reached via a comma). See
[READ](class-0/READ.md)/[RDAL](class-0/RDAL.md) for the full mechanism
...
## MSC-01847 ADDENDUM: ZDLP/PFST/PFND — HAL-1971 instructions dropped in HAL/S

Reading the final, previously-unreviewed page of [MSC-01847] (part3 p. 41,
the document's own final page) — its text extraction came out badly
scrambled by overlapping bit-diagram artwork, resolved by rendering the
page as a 300dpi image (`pdftoppm`) and reading it directly — found an
**ADDENDUM** section, not indexed anywhere else in the document, listing
three HAL-1971 instructions "not included in the text" due to "incomplete
implementation in some areas":

- **ZDLP** (opcode 0x00F): "suppressed arrayness specifier." Per the
  addendum's own prose: "ZDLP appears in place of arrayness specifiers if
  an arrayed argument of a procedure is other than a symbol-table
  variable. It is used because of current compiler restrictions on
  procedure arguments."
- **PFST** (opcode 0x04E): "proc/func start specifier," carrying a nesting
  level (`flv`) — "currently appears prior to the start of the HALMAT
  construct for procedure or non-shaping function invocations."
- **PFND** (opcode 0x04F): "proc/func end specifier," the closing
  counterpart to PFST, same `flv` field, appearing "at the end of the
  HALMAT construct for procedure or non-shaping function invocations."

None of the three have a corresponding `X`-prefixed constant anywhere in
the HAL/S-FC compiler source (`XZDLP`/`XPFST`/`XPFND` all absent from a
full-tree grep), and their HAL-1971 opcodes (0x00F/0x04E/0x04F) are
reused for unrelated HAL/S mnemonics ([ETST](class-0/ETST.md)/`TEQU`/
`TASN` respectively) — consistent with the general opcode-renumbering
already documented elsewhere in this project. Rather than leaving this as
an open question, both were empirically tested directly:

- **PFST/PFND are confirmed superseded**, not merely absent: HAL/S
  procedure and (non-shaping) function invocations are already
  empirically documented (see [PCAL](class-0/PCAL.md)/
  [FCAL](class-0/FCAL.md)) to be bracketed by
  [XXST](class-0/XXST.md)/[XXAR](class-0/XXAR.md)/[XXND](class-0/XXND.md)
  instead — the same general-purpose bracket used for `READ`/`WRITE` and
  every other argument-list construct. HAL/S never needed a
  call-specific start/end pair once this more general bracketing
  mechanism existed.
- **ZDLP is confirmed superseded** too: compiling
  `CALL P1((S1 + S2));` (`P1` a procedure taking an `ARRAY(5) SCALAR`
  parameter, the argument a non-symbol-table array *expression* — exactly
  the triggering case the addendum describes) shows the argument's
  [XXAR](class-0/XXAR.md) operand (the `SADD` expression's `VAC` result)
  immediately followed by an ordinary [ADLP](class-0/ADLP.md)/
  [DLPE](class-0/DLPE.md) arrayness bracket (size 5), exactly like any
  other array-valued expression elsewhere in this project's corpus — no
  distinct "suppressed" opcode needed. HAL/S's more general PCAL/FCAL +
  XXST/XXAR/XXND argument mechanism, unlike HAL 1971's, apparently never
  ran into the "current compiler restrictions" the addendum describes,
  so the ordinary ADLP/DLPE bracket already handles this case directly.

This closes the last open item from [MSC-01847]'s review and gives a
concrete, sourced answer (rather than silence) to what would otherwise be
an unindexed, easy-to-miss gap in the predecessor-language source: all
three addendum instructions are real HAL-1971 mechanisms that HAL/S
demonstrably replaced with its own, more general instructions, not
mnemonics this project failed to find.
