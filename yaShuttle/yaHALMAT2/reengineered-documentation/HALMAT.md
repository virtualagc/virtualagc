...
- [Word Format](#word-format)
  - [Operator Word (bit 0 = 0)](#operator-word-bit-0--0)
  - [Operand Word (bit 0 = 1)](#operand-word-bit-0--1)
- [Block (Record) Structure](#block-record-structure)
- [Optimizer HALMAT](#optimizer-halmat)
- [Auxiliary HALMAT (AUXMAT)](#auxiliary-halmat-auxmat)
...
| [USA00309] | Document USA00309, "HAL/S-FC User's Manual" (2005). Primary source. §8.2 ("Runtime Characteristics") and §6.1.2–6.1.3 ("Input/Output Data Formats") reviewed in full — the authoritative source for datatype precision representation and every pairwise BIT/CHARACTER/SCALAR/INTEGER conversion. Same document as [USA003090] below (a dropped-trailing-digit citation typo predating its own extraction, left as-is rather than mass-renamed — see `STATUS.md`'s "Source material reviewed so far" table). |
| [USA003087] | Document USA003087, "HAL/S Programmer's Guide" (2005). Primary source, also covering datatype conversions (Appendix A) and I/O statement formatting (§12.2). §12.2 (WRITE), §12.3 (READ), Appendix F (standard I/O formats), and §7.5 ("Some Explicit Conversions" — VECTOR/MATRIX/INTEGER/SCALAR list-form shaping functions, `MATRIX(...)`'s general "unravel/reravel" rule) reviewed; remainder not yet. |
| [USA003088] | Document USA003088, "HAL/S Language Specification" (2005). Primary source — the formal syntax/semantics specification, more precise and detailed than [USA003087]'s Programmer's Guide framing of the same statements (e.g. §10.1.1's rule-numbered READ/READALL semantics vs. [USA003087] §12.3's prose summary of the same mechanism). §10.1.1 ("The READ and READALL Statements") and §6.5.1 ("Arithmetic Conversion Functions" — the formal grammar/semantic-rule version of [USA003087] §7.5's shaping-function material) reviewed; remainder not yet. |
| [USA003090] | Document USA003090, "HAL/S-FC User's Manual" (2005) — see [USA00309] above; Appendix C ("Execution-Time Errors," group-4 runtime error "standard fixups") reviewed in full. Also reviewed: §5.2 ("Compiler Directives" — the `DEVICE CHANNEL=n [UN]PAGED` directive and its default-channel-mode rule), §6.1.3 ("Output Data Formats"), §6.1.4 ("JCL Considerations" — PAGED/UNPAGED default LRECL), §8.2 ("Runtime Characteristics," the numbered rule list covering datatype ranges/conversions and BIT string length limits). Remainder not yet. |
...
| ["Programming in HAL/S"] | "Programming in HAL/S" (Sept. 1978), an introductory HAL/S textbook. **Not primary source** — per direct user guidance, it's a generic textbook rather than specific to this project's actual compiler (HAL/S-FC), so it's cited only to corroborate or illustrate, never as the sole basis for a behavioral claim — but noted as significant since it's typically the first HAL/S material a student encounters. Also this project's own `Source Code/Programming in HAL-S/*.hal` regression-test corpus is drawn directly from its worked examples (e.g. `029-DATATYPES.hal`/`037-ROOTS.hal`, each citing their originating page in their own header comment). §8.1 ("The WRITE Statement," pp. 8-1–8-5 — PAGED/UNPAGED CHARACTER/BIT output quoting, the standard external formats table including the `0` → `0.0` zero-value special case) and §8.3 ("The READ Statement," p. 153) reviewed; remainder not yet. |
...
Analysis citations — this section only covers how they compose.
...
["Optimizer HALMAT"](#optimizer-halmat) above and `STATUS.md`'s "Optimizer
