...
Vector add. Computes the elementwise sum of two vector operands of
matching dimension, producing a vector result.

## Usage Context

Emitted for HAL/S expressions using the `+` operator between two vector
operands. Named explicitly (alongside MADD/SADD/IADD) in the Optimizer
...
following [VASN](../class-4/VASN.md) via a `VAC`-qualified operand.
Confirmed by compiling `V3 = V1 + V2` with `HALSFC --parms="LSTALL"`; see
...
Opcode (0x482) confirmed primary-source: part of the `XMADD(1)` array
(element 1, `XVADD`) in `PASS1.PROCS/##DRIVER.xpl` — see [##DRIVER.xpl]
in `STATUS.md`. Matches [MSC-01847]'s HAL-1971 VADD opcode (0x482)
...
Behavioral description is a straightforward reading of "vector add"
