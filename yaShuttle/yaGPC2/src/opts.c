...
"                                  wall-clock anchor, and the mission time\n"
"                                  the timing unit serves, for reproducible\n"
"                                  runs.  Decomposed as GMT: the vehicle's\n"
"                                  clock is GMT and has no time zone.\n"
"                                  Default: the host's own wall clock at\n"
"                                  program start.\n"
...
"                                  switch names, so a display PASS can drive\n"
"                                  has to live on a different one.  With\n"
"                                  --gpcs, \"2:7\" ATTACHES the unit to GPC 2:\n"
"                                  two computers cannot drive one display,\n"
"                                  and one with no unit sees nothing on the\n"
"                                  display buses.  The built-in DK1 unit\n"
"                                  belongs to the first computer named\n"
...
void opts_parse(int argc, char **argv, Options *opts) {
    ...
    for (int i = start; i < argc; i++) {
        ...
        if (strcmp(tok, "-h") == 0 || strcmp(tok, "--help") == 0) {
            ...
            opts->deuBus = take_value(argc, argv, &i, tok, n);
