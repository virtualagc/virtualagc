...
- RETRACTED: "the Display Control Program came off our tape and went into
  the display unit". Unfounded -- inferred from DCPLDFL being set and from
  deumodel printing "load complete", neither of which inspects content.
  DCPLDFL is a flag BSL1 sets when its transaction finishes, real data or
  not. The user challenged it and was right.
- MEASURED: #PCVNMMU / module DEUIPLCP in PHASE03, 16393 hw, "DEU IPL
  Control Program" -- ALL ZEROS in our build (verified by reading
  PHASE03.fcm flat; imageSize 302702 == file halfwords, and #PCGMCOM in the
  same file reads 199 distinct, so the method is sound). We have no
  display-unit software and cannot build any: it is a binary COMPOOL blob
  for a different CPU.
- The 717 non-zero words that DO reach the display unit are FCW display
  content from GPCIPL's own ASSEMBLY screens (MENU12.asm, PCH10TXT), which
  emit FCWs directly -- `FCW2 ANCTL2=1`, `POS (P2,P25)`, `DC H'0' ASTERISK
  FCW`. No .dfg/.hal display format is involved in the IPL's screens, which
  is why they render with no DFG preprocessor.
- Why anything renders at all: MEDS's MDU implements the FCW protocol
  natively in JS ("The DPS display implements the original DEU FCW protocol
  generated by shuttle software" -- Don's README). No DEU/IDP software is
  needed. Per the user: the pre-2000 MCDS unit is an SP-0 DEU with 8Kx16
  RAM (hence DEU_MEMORY_WORDS 8192); the post-2000 MEDS unit is a 386DX IDP
  running DAS. Our deumodel models the DEU; MEDS models the IDP.
- DISPLAY FORMATS, surveyed: OI340600 has 133 .dfg formats and NOT ONE of
  them exists as .hal anywhere in that release; 126 have an OI301700 .hal
  counterpart (a ready-made oracle for validating DFG when it lands), 7 have
  none anywhere (CG0540, CG0543, CG1031, CG6011, XG0540, XG0543, XG1031).
  OI301700 has ZERO .dfg -- every format there is already .hal, so it is the
  release to drive if PASS displays are wanted before DFG arrives.
  13 of 25 built phases contain display-format sections, all from .hal; the
  133 .dfg formats appear in no built phase at all.
