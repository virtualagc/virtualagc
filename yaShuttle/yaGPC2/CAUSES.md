...
- **#35** *(method)* THE TRANSITION IS NOT RELIABLY REPRODUCIBLE, AND THE CAUSE IS THE KEYSTROKE GATE'S CLOCK. YAGPC_DEUKEYS '@Ns:' is measured by deu_wall_seconds() -- CLOCK_MONOTONIC, not simulated time (deumodel.c:212). The run is only PACED to real time, so whenever the host is loaded the emulator falls behind and the SIMULATED instant at which OPS,9,0,1,PRO lands drifts. A key delivered at the wrong moment produces a run that looks exactly like a transition that did nothing.
  - evidence: Identical '@410s:OPS,9,0,1,PRO' on the same tape: v91a had the machine to itself, the key landed at simulated 389.64 s, and the phase 3 and phase 8 reads followed within 1 ms. v92a, v93a and v95a/b/c all delivered their key (keys=2 every time) and NONE produced a mass-memory read after t~148 s -- five consecutive non-transitions. v95a/b/c ran three-up AND carried a byte-at-a-time fputc dump of 1 MB of main storage at t=388, which by itself pushes the emulator behind the wall clock. FIXES APPLIED: deumodel now takes the simulated clock (deumodel_set_clock, wired in run.c) and YAGPC_DEUKEYS_SIMTIME=1 gates '@Ns' on SIMULATED seconds; the delivered message now prints poll=, simt= and wall=; dump_main_storage buffers and does one fwrite. CONSEQUENCE FOR OLD RESULTS: 'keys=2' is not a validity check and neither is simulated time past 410 s -- validate by a 'mmu1: read' LATER than the key. The '~90 runs, zero 6/5/0/0' figure does not distinguish 'transitioned and failed' from 'never transitioned'.
  - tape `v36` &middot; env `DEUKEYS,YAGPC_DEUKEYS_SIMTIME,YAGPC_DUMPSTATE_AT` &middot; files `src/deumodel.c,src/deumodel.h,src/run.c,pass-run/headless-gpcmem.sh` &middot; symptom `no 6/5/0/0,no transition` &middot; tsec `389-400` &middot; run `v91a,v92a,v93a,v95a,v95b,v95c`
...
- **#29** *(method)* RETRACTED. The claim was 'phase 8's DATA transfer DOES complete', read off the MMU log's 'read N block(s) from T/F/S/B' lines. That message is emitted inside do_read at mmumodel.c:319 BEFORE queue_words_paced, and stats.blocksRead is incremented in the same loop -- it reports what the model SCHEDULED onto the bus, not what the BCE took, where it landed, or whether the overlay accepted it. Do not read a load-completion claim out of that line.
  - evidence: What survives, and it is narrower: the MMU word books balance exactly in both runs (wordsOut - wordsTaken - wordsLost = 0), so nothing was left unconsumed in the queue; and wordsLost is 720 in BOTH v91a and v92a, where v92a never reached the transition at all (last MMU activity t=136 s), so every lost word predates the transition and none belongs to the phase-8 read. That supports 'BCE18 consumed every word the MMU put on the bus' -- a statement about the bus, not about phase 8 being loaded. STILL UNTESTED in both directions: whether the words landed at the right GPC addresses, whether the load-block checksums passed, and whether the overlay completed. NOTE ledger #1's counter-claim is no better attested: its evidence field cites only '~90 runs, zero 6/5/0/0', and the specific 'FTRMGPOV reached once / FCMMGPOV entered twice' figure comes from a session plan with no run cited. The decisive test for BOTH is to watch phase 8's destination addresses and see whether tape content arrives there during 392-400.
