...
parse_pass1(), specifically its M|/E|/S|/C| line-attachment logic.
...
the smallest known real HALSFC output exercising all three annotation
line types (E|, S|, C|) and both attachment directions -- see
parse_pass1()'s own docstring for the rule being tested (an E|/C| run
preceded by a blank line attaches FORWARD to the next M| statement; an
S| run with no blank since the last M| line attaches BACKWARD to it).
T06-T08 are small synthetic cases for the remaining edge cases: an SRN
column, a "||" concatenation operator not truncating the line early,
and a non-blank unrecognized line (e.g. a page header) acting as a
boundary the same way a blank line does.
...
def main():
    ...
    check(
        "T05 multi-line leading C| comment block attaches to the following statement",
        stmts[15],
        [
            "C|        Compare vs formula for sum of squares.",
    ...
    boundary = (
        "         14 M|   WRITE(6) RMS;                                                                             |EXAMPLE_5\n"
        "^L------------------------------------------------------------------------------------------------------\n"
        "            C|  a trailing-looking comment, but really a new leading run                                  |EXAMPLE_5\n"
        "         15 M|   NEXT;                                                                                    |EXAMPLE_5\n"
    )
    b = parse(boundary)
    check("T08 unrecognized line (page break) resets attachment like a blank line", b[15],
          ["C|  a trailing-looking comment, but really a new leading run", "M|   NEXT;"])
    check("T08b ...and stmt 14 keeps no stolen trailing annotation", b[14], ["M|   WRITE(6) RMS;"])
