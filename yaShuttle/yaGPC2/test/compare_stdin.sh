#!/bin/bash
# compare_stdin.sh — like compare.sh, but feeds a given file to both
# implementations' stdin (for --interactive runs that READ from the
# terminal). Usage: test/compare_stdin.sh <label> <stdin-file> [gpc-run-args...]
set -u
...
(cd "$ROOT" && node dist/gpc.js run "$@") <"$stdin_file" >"$exp_out" 2>"$exp_err"
exp_code=$?

(cd "$ROOT" && "$YAGPC" "$@") <"$stdin_file" >"$act_out" 2>"$act_err"
act_code=$?
