...
F('Error Handling: Declaration', 'OFF ERROR statement', 'Removes a previously-installed ON ERROR modification with the identical specification.',
  'USA003087 §25.2', 'implemented',
  "Zero yaGPC2-specific code needed or exists -- confirmed a pure compile-time construct. try_on_error_dispatch() (halucp.c) never reads any 'is this handler still active' flag at all; it scans the compiler-emitted FIXV/handler slots physically present in the erroring routine's own compiled stack frame (walked via R0/SA), and HALSFC itself simply stops emitting a FIXV registration for code textually after an OFF ERROR statement -- so there is nothing live left to find there. Confirmed directly: a real compiled `ON ERROR$(4:5) GO TO CAUGHT; ...; OFF ERROR$(4:5); ...` triggers the same SQRT domain error twice, jumping to CAUGHT only the first time, falling through normally (with the default fixup value) the second -- byte-identical to yaHALMAT2's own output for the same program.",
  'tested_dedicated', "test/fixtures/offerror.hal, byte-diffed (stdout AND stderr) against yaHALMAT2 via test_scheduler.sh. See problems.md 7.18.")
...
F('Error Handling: Recovery Flow', 'Per-process error environment', "Each real-time process has its own independent set of ON ERROR overrides.",
  'USA003087 §25.1', 'implemented',
  "Confirmed correct, inherently, by construction -- not a process-agnostic global as this survey's own earlier guess suspected. try_on_error_dispatch() dispatches based purely on the CURRENTLY LIVE R0 register (the stack-area/frame pointer) at the exact moment SEND ERROR fires; R0 is part of the full register bank (R0-R7) schedule.c's sched_save_context/sched_restore_context already save and restore on every task context switch, so each TASK's own ON ERROR environment is automatically isolated with zero extra bookkeeping. Confirmed empirically now that TASK/SCHEDULE/WAIT is real (this survey's own earlier note flagged this as worth re-checking): a TASK with its own ON ERROR$(4:5) GO TO handler catches its own SQRT domain error correctly, and the PRIMAL's own later, unrelated SQRT domain error (no handler of its own) falls through to the default fixup instead of incorrectly reusing the task's now-stale handler. A real yaHALMAT2 bug found via this exact cross-check: their own interpreter's ON ERROR handler leaks across tasks (the primal's own error incorrectly re-triggers the task's GO TO) -- likely the same underlying architecture as the shared-interpreter-call-stack REENTRANT concern flagged earlier this session, now confirmed to also affect ON ERROR scoping, not just local-variable storage.",
  'tested_dedicated', "test/fixtures/errorpertask.hal, byte-diffed (stdout AND stderr) against yaGPC2's own predicted-correct output (yaHALMAT2 is NOT a valid oracle here -- its own cross-task leak was the very thing this test caught) via test_scheduler.sh. See problems.md 7.18.")
F('Error Handling: Recovery Flow', 'Dynamic scoping of error-environment modifications', 'An ON ERROR modification is in force only until the enclosing invocation returns (call-depth scoped, not lexical).',
  'USA003087 §25.1', 'implemented',
  "Confirmed correct, inherently, for the same reason as per-process isolation above: try_on_error_dispatch()'s own FIXV-slot scan walks the LIVE call-frame chain (via R0/SA and the caller's own saved R0 at SA+2) at the exact moment of the error, never a static/global table -- once a PROCEDURE/FUNCTION returns, its own frame's FIXV slots are simply no longer reachable from that walk, not because anything is explicitly 'removed.' Confirmed empirically: a FUNCTION with its own ON ERROR$(4:5) GO TO handler catches its own SQRT domain error and returns; the calling main line's own later, unrelated SQRT domain error (that FUNCTION already returned, no ON ERROR of its own at that scope) falls through correctly to the default fixup instead of incorrectly reusing the returned function's now-stale handler. Same real yaHALMAT2 bug as per-process isolation above manifests here too (their own interpreter re-triggers the already-returned function's GO TO for the main line's own later error) -- one root cause (state not unwound on return/task-switch) explaining both findings.",
  'tested_dedicated', "test/fixtures/errordynscope.hal, byte-diffed (stdout AND stderr) against yaGPC2's own predicted-correct output (yaHALMAT2 again not a valid oracle -- same underlying bug) via test_scheduler.sh. See problems.md 7.18.")
