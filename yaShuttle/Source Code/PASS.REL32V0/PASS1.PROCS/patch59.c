{
  ...
  //                            and swapped `sdr` for `swr` in p59_40.
  //            2026-05-13 RSB  Changed `swr` back to `sdr`.  It was fine as
  //                            `swr`, but it was fine as `sdr` too and I was
  //                            just confused thinking it wasn't.  Might as well
  //                            keep it as authentic as possible.  (My confusion
  //                            was in thinking that compile-time computations
  //                            that mightn't have resulted in exact integers
  //                            passed through here.  In fact, only actual
  //                            literals do, and if they're integers they'll
  //                            still be exact integers in HFP form.)

p59_0: ;
  // (59)       CALL INLINE("58", 1, 0, ADDRuFIXEDuLIMIT);       /* L   1,ADDRuLIMIT */
  address360B = (mADDRuFIXEDuLIMIT) & 0xFFFFFF;
  // Type RX, p. 7-77:		L	1,mADDRuFIXEDuLIMIT(0,0)
...
p59_4: ;
  ...
  // Type RX, p. 7-77:		L	2,mPREPuLITERALxTEMP1(0,0)
...
p59_22: ;
  ...
  // Type RX, p. 7-77:		L	1,mADDRuFIXER(0,0)
...
p59_30: ;
  ...
  // Type RX, p. 7-77:		L	1,mTABLEuADDR(0,0)
...
p59_40: ;
  ...
  sdr(2, 0); // RSB - Changed back; earlier change was okay but unnecessary.
  detailedInlineAfter();
