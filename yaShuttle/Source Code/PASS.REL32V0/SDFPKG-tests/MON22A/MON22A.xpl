...
/*                            N - SDF MODE(0-17)               */               
/*                                                             */               
/*   TEST CASE TO AUGMENT SDFPKG PAGING AREA AND RESCIND ALL   */               
/*   PAGING AREA AUGMENTS.                                     */               
/*                                                             */               
/* REVISION HISTORY :                                          */               
/* ----------------                                            */               
...
DATABUF  = ADDRESS;      /* SAVE DATABUF ADDRESS */                             
...
/* LOCATE STMT DATA CELL GIVEN SRN & INCLUDE SOUNT */                           
OUTPUT = ''; OUTPUT = '--- SDFPKG MODE 14 TEST ---';                            
NAME = '002691';                                                                
NAMEADDR = COREWORD(ADDR(NAME)) & "FFFFFF";                                     
DO I = 0 TO 5;                                                                  
   COREBYTE(ADDR(SREFNO)+I) = COREBYTE(NAMEADDR+I);                             
END;                                                                            
INCLCNT = 5;                                                                    
COREWORD(ADDR(C)) = SHL(5,24) + ADDR(SREFNO);                                   
...
END;                                                                            
 ...
 COREBYTE(DATABUF+49) = "01"; /* SET FIRST MODE FLAG */                         
/* LOCATE SYM DATA CELL AGAIN WITH FIRST MODE SET */                            
OUTPUT = ''; OUTPUT = '--- SDFPKG MODE 9  & 13 TEST - FIRST MODE ---';          
DO I = 1 TO SYMS;                                                               
   SYMBNO = I;                                                                  
   CALL MONITOR(22,9);                                                          
   NAME = STRING(SHL((SYMBNLEN-1),24) + ADDR(SYMBNAM));                         
   OUTPUT = 'SYM # '||I||' NAME = '||NAME||' BLOCK # = '||BLKNO;                
   CALL MONITOR(22,8);                                                          
   /* RETURN FIRST SYMBOL FOUND WITH THE MATCHING SYMBOL NAME */                
   CALL MONITOR(22,13);                                                         
   IF CRETURN = 0                                                               
      THEN OUTPUT = '  LOCATED '||NAME||' SYMBNO= '||SYMBNO;                    
   IF CRETURN = 20                                                              
