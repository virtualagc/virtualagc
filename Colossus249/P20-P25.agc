...
## Page 554
		...
		TC	UPFLAG		# SET R60FLAG
		ADRES	R60FLAG		# BIT 4 FLAG 5
		TC	BANKCALL
		CADR	R61CSM
		TC	DOWNFLAG	# RESET R60FLAG
		ADRES	R60FLAG		# BIT 4 FLAG 5
		EBANK=	QMIN
		CAF	EBANK5
		TS	EBANK
...
PROG22		TC	DOWNFLAG	# RESET RNDVZFLG BIT 7 FLAG 0
		...
		TC	ENDOFJOB
PROG22A		CS	OCTL7000	# SET OFFSET NO.=0
		MASK	LANDMARK
		TS	LANDMARK
		TC	INTPRET
		CLEAR
...
## Page 556
		...
		TC	+5		# PROCEED	UNPACK ABCDE
		TC	-5		# RECYCLE
		CAF	FIVE		# IMMEDIATE RETURN BLANK OUT R1,R2
		TC	BLANKET
		TC	ENDOFJOB
		CAF	FIVE
		TS	MARKINDX
		TC	INTPRET
		CALL			# UNPACK ABCDE FROM LANDMARK
			UNPACKAE
		SLOAD	DSU
			22SUBSCL
			P22MAXDE
		BPL	CALL
			DE-GR-50	# DE GREATER THAN MAX
			P22SUBRA	# SUBROUTINE A SETS LAT/LONG/ALT
		GOTO
			CALLR52
DE-GR-50	CALL			# CALL ADVANCED ORBIT ROUTINE
			ADVORB
		EXIT
		TC	2PHSCHNG
		OCT	00004
...
CALLR52		EXIT
		...
		OCT	13000
		CAF	FIVE
		TS	MARKINDX	# SET MARK INDEX=5 FOR P52
		TC	INTPRET
		CALL
			R52
DOV5N71		EXIT
...
## Page 557
		...
		TC	+5		# PROCEED	UNPACK ABCDE
		TC	-5		# RECYCLE
		CAF	FIVE		# IMMEDIATE ENTRY BLANK OUT R1,R3
		TC	BLANKET
		TC	ENDOFJOB
		TC	INTPRET
		CALL
			UNPACKAE
		CALL			# SET LAT/LANG/ALT
			P22SUBRA
PROG22C		LXC,2	SLOAD*
			MARKSTAT
			QPRET,2
...
V05N7022	VN	00570
V05N7122	VN	00571
P22MAXDE	2DEC	27 B-14

V06N89		VN	00689
OCTL77		OCT	77		# DE MASK OF ABCDE
OCTL700		OCT	700		#  C MASK OF ABCDE
OCTL7000	OCT	7000		#  B MASK OF ABCDE
		SETLOC	P20S
		BANK
		
P22SUBRA	STQ	BOFF		# SET LAT/LONG/ALT FOR KNOWN LANDMARK
			S22TOFF
			KNOWNFLG
			S22TOFF		# UNKNOWN LANDMARK, EXIT
		SLOAD	BHIZ
			22SUBSCL
			OBTAINLL	# GET LAT/LONG/ALT FROM ASTRO
		DSU	BMN
			9DWID		# 2 B-14
			S22LSITE	# GET LAT/LONG/ALT FROM RLS (LANDING SITE)
		SL1	LXC,1		# GET LAT/LONG/ALT FROM TABLES
			MPAC
		SETPD	DLOAD*
			0D
			ALTTAB,1
...
P22SUBRB	STQ	EXIT		# GET LAT/LONG/ALT FROM ASTRO
		...
		GOTO
			S22TOFF +1	# EXIT
UNPACKAE	EXIT			# UNPACK LANDMARK INTO ABCDE
		CA	LANDMARK
		MASK	OCTL77
		TS	22SUBSCL	# DE=L=LMK ID NO. N 00,01,02-26
		CA	LANDMARK
		MASK	OCTL7000
		TS	CXOFF		# B= OFFSET INDICATOR
		TC	UPFLAG		# SET KNOWNFLG
		ADRES	KNOWNFLG	# BIT 8 FLAG 6
## Page 559
		CA	LANDMARK
		MASK	BIT14		# IF BIT14 OF LANDMARK=1, A=2.  OTHERWISE A=1
		EXTEND
		BZF	+3		# A=1	LEAVE KNOWNFLAG SET FOR KNOWN LMK
		TC	DOWNFLAG	# A=2	CLEAR KNOWNFLG (BIT 8 FLAG 6) FOR
		ADRES	KNOWNFLG	#	UNKNOWN LMK.
		TC	INTPRET
		RVQ
...
#		DFATA DISPLAY.  THIS ROUTINE RETURNS TO THE ORIGINAL PROGRAM AT THE INTERRUPTED DISPLAY.
...
## Page 571
		DLOAD
			ZEROVECS
		STORE	VHFCNT		# ZEO OUT VHFCNT AND TRKMKCNT
REND6		SET
			RENDWFLG
		VLOAD	VSU
			LEMPOS
...
## Page 572
		...
		BPL	TLOAD
			REND10A
			RVARMIN
		ABS			# MIN. VARLUE WAS STORED AS NEG.
...
RENDISP4	TC	INTPRET
		GOTO
			REND3
SXTVAR		2DEC	0.04 E-6 B+16	# EXT ERROR VARIANCE = .04 (MR)SQ
...
RANGERD3	EXIT
		...
		DAD	GOTO
			...
			RANGERD2
LIGHTON		TC	TRFAILON	# TURN TRACKER LIGHT ON
		TC	INTPRET
		DLOAD
			MARKTIME
		STORE	VHFTIME
...
STORT		STCALL	P21TIME		# STORE CLOCK TIME FOR SUBR R63
		...
		EXIT
		TC	STEP2CK +4	# ELIMINATE FORCED R60 MANEUVER
AUTOCK		CAF	PRIO30
		EXTEND
		RXOR	CHAN31
		MASK	FURST3
...
STEP2CK		CS	FLAGWRD5	# IS R60FLAG SET
		MASK	BIT4
		EXTEND
		BZF	MANUEXIS	# YES, DO R60
		TC	BANKCALL
		CADR	UPACTOFF
...
S22NXTIN	CALL
		...
		LXA,1	SXA,1
			S22LOC		# SETUP ADDR. OF MARK ATA FOR GETUM SUBR.
			MARKDATA
		CALL			# COMPUTE UM
S2GETUM			GETUM
		STORE	UM
...
## Page 593
		...
		DSQ	DMP
			SCTVAR		# B+16
		SR1			# ACCOUNTS FOR 1/2 IN DEXT FORMULA
...
S2INTS1			INTSTALL
		...
		BON	CLEAR
			DMENFLG
			S22NXTIN	# RETURN ALWAYS EXCEPT OFFSET POINT MARK
			DIM0FLAG
		BOFF	SET
			ORBWFLAG
...
## Page 611
		...
		GOTO
			P23.60
P23.05		CLEAR	EXIT
			SAVECFLG	# USED TO SAVE SPACE IN P23.05
		CAF	V05N70		# REQUEST RESPONSE AND DISPLAY MEASUREMENT
		TC	BANKCALL	# IDENTIFICATION:  STAR, LMK, HOR  IDENT.
		CADR	GOFLASH
		TC	GOTOP00H	# TERMINATE
		TC	P23.15
		TC	-5		# REDISPLAY
P23.15		CA	LANDMARK	# IF C=2, LUNAFLAG=1.  IF C=1, LUNAFLAG=0
		...
		BZF	+2
		TC	+2
		CA	HORIZON		# SET LUNAFLAG FROM HORIZON OR LANDMARK
		MASK	BIT8
		EXTEND
		BZF	P23.16
		TC	INTPRET
...
P23.17		SLOAD	PUSH
		...
		CA	LANDMARK
		MASK	OCT00077
		TS	IDOFLMK		# FOR RE(DE) LUNAFLAG ALREADY SET
		CA	HORIZON
		EXTEND
		BZF	P23.12
		MASK	BIT5		# IF D=1,NORFHOR=0(NEAR).  D=2,NORFHOR=1,FAR.
		EXTEND
## Page 612
		BZF	P23.18
		TC	INTPRET
...
P23.19		EXIT
		TC	P23.30
P23.12		CA	IDOFLMK		# IF R2(DE) NONZERO, LMK IS IDENTIFIED
		EXTEND
		BZF	P23.20		# LANDMARK NOT IDENTIFIED
		TC	P23.30		# LANDMARK IS IDENTIFIED
P23.20		CAF	V6N89		# REQUEST RESPONSE AND DISPLAY LMK DATA
		TC	BANKCALL	# R1=LAT,R2=LONG/2,R3=ALT
		CADR	GOFLASH
		TC	GOTOP00H	# TERMINATE
		TC	P23.30
		TC	P23.20		# STORE NEW DATA AND REDISPLAY
P23.30		TC	INTPRET
		BON	EXIT
			SAVECFLG
			P23.85
		...
		TC	P23.55		# ENTER.  MANUAL CONTROL
P23.55		TC	INTPRET
		GOTO
			P23.56
...
## Page 614
			POINTEX
			LANDMARK	# IF LANDMARK=0 HORIZON IS DESIRED.
		BHIZ	SLOAD		# DO NOT PICK UP LMK VALUES FROM TABLE
			R23.05		# OR DISPLAY IN HORIZON CASE
			IDOFLMK
		BHIZ	CALL
			R23.0
			LUNLMKLD
		GOTO
			R23.05
R23.0		CALL			# MUST BE DONE 2ND TIME, TO ALLOW CHANGES
			LLASRDA
R23.05		BON	DLOAD
			ORBWFLAG
			R23.1
...
R23.2		SET	CALL
		...
		TC	INTPRET
		CALL
			RECT.1		# PICKUP CSM STATE VECTOR FROM PRM
		BOFF	VLOAD
			ZMEASURE	# IN SPHERE OF INFLUENCE OF PRIMARY BODY
			R23.3
			RQVV		# CALCULATED BY INTEGRATION B29
		STORE	RZC
R23.3		SLOAD	BHIZ
...
R23.6		CAF	V6N49
		TC	BANKCALL
		CADR	GOFLASHR
		TC	R23.6		# DON'T ALLOW
		TC	R23.7		# INCORPORATE DATA
		TC	P23.00		# RECYCLE FOR ANOTHER MARK
...
V50N25P		OCT	00202
SPSIX		OCT	00006
JCAXIS		2DEC	.268649805	# TRACK AXIS
		2DEC	0
		2DEC	.421695725
