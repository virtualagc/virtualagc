/*
  ...
  Mode:		07/28/04 RSB	Forked from ParseGeneral.c.
*/
  ...
  if (K.Constant)
    {
      ...
      //if (WriteOutputDebug)
      //  printf ("nnnnFields=%05o\n", i);
      if ((i & 3) == 1)
        {
	  // Come here if it's a switch instruction.
	  ...
	  OutRecord->Words[0] |= (0360 & i);
        ...
        {
	  // Come here if it's a shift instruction.
	  OutRecord->Words[0] = K.Value;
	  OutRecord->Words[0] |= (i & ~3);
        } 
      else 
        {
	  // Come here if it's not a switch or shift instruction.
