...
# MODIFIED BY BEALS TO ELIMINATE CSMMASS UPDATE LOGIC (NOW DONE IN S40.8)   # new in artemis072 ...smc
#
...
EXECPHS         CCS     TVCEXPHS        # CHECK TVCEXECUTIVE PHASE
                ...
                TC      TWIDDLE        #       BEFORE PITCHDAP, AFTER CMDSOUT		# TWIDDLE use new in artemis072 ...smc
                ADRES   EXRSTRT
...
JUMPTVC4	TC      POSTJUMP        #       IF POSITIVE OR ZERO, RESTART AT		# JUMPTVC4 new in artemis072 ...smc
                CADR    TVCINIT4        #               TVCINIT4 (ZEROS TVCPHASE, AND	# STROKE test removed (was in comanche55) ..smc
...
TVCEXADR        OCT     37772           # (UNUSED TABLE LOC, FILL WITH 60MS, T5)
        +1      GENADR  TEMPSET		# TVCEXECUTIVE RESTART POINTS (ORDERED)		# changed from comanche055 ...smc
        +2      GENADR  CORSETUP
        +3      GENADR  CORCOPY +1
        +4      GENADR  CNTRCOPY
