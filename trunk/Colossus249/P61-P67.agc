...
P63		TC	NEWMODEX
		...
		CADR	CLEANDSP		# DURING STATEMENT PASS.)
...
#	PIPTIME   (-26) CS	TIME OF PIPA UPDATE				LEFT BY READACCS
...
#	INERTIAL VELOCITY (VPRED) AND ENTRY ANGLE (GAMMABI) THAT WILL OBTAIN AT 400K FT ABOVE THE FISCHER
...
#
# FUNCTIONAL DESCRIPTION:  TIVEN TEH PRESENT POSITION, UNITR, CALCULATE A NEW UNITR THAT IS ROTATED THROUGH
#	TRANSFER ANGLE, THETA, ALONG THE TRAJECTORY.  THEN CALCULATE SIN(LAT) AND USE TO OBTAIN FISCHER RADIUS.
#	SINCE FISHCALC USED UNIT (LEFT BY ENTRY) EARTH SCALING IS ASSUMED.  (WILL IMPROVE FOR SUITABLE TENNANT)
#
# CALLING SEQUENCE:	CALL
#				FISHCALC
...
#		URONE		UR/2					LEFT BY USER(?)
#		UNI		.5 UNIT(V*R)				LEFT BY (?) / P61
#		UNITW		UNIT NORTH POLE				LEFT BY (?) LOAD
#
...
FISHCALC	PDVL	VXV		# URPR = UR CDELF + UHOR SDELF
		...
		PDVL	VXSC		# TO PDL+0, (garbled)
			URONE
			CDELF/2		# COS(THETA) / 2
		VAD	STADR
		STORE	URH		# FOR USE IN (?) FROM EMS DISPLAY
		DOT	SL1
			UNITW		# PULL UNIT VECTOR	UNIT NORTH
		STORE	ALPHAV +4	# = .5 (garbled)
DUMPFISH	GOTO
...
#		  M: (-27+NR)
#	RTERM	  E: (-29)  M: (-27)	M  TERMINAL RADIUS LENGHT		LEFT BY CALCTFF
