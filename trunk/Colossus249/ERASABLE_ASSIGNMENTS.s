...
# Filename:	ERASABLE_ASSIGNMENTS.s
...
# CALCMAN3	042D		BIT 2 FLAG 2
# (garbled)
# CMOONFLG	123D		BIT 12 FLAG 8
# CM/DSTBY	103D		BIT 2 FLAG 8
...
# FIRSTFLG	036D		BIT 7 FLAG 2	ETPIFLAG OPTNSW
...
# GRRBKFLG	065D		BIT 5 FLAG 5
# GUESSW	028D		BIT 2 FLAG 1
# GYMDIFSW	104D		BIT 1 FLAG 6
# H05GSW	102D		BIT 3 FLAG 6
...
# BIT 4 FLAG 5
R60FLAG		=	086D		# R61 MUST BE ZERO	NORMAL R61
...
# BIT 11 FLAG 9
V94FLAG		=	139D		# V94 ALLOWED DURING	V92 NOT ALLOWED
					# P23
...
# Page 69
# PAD LOAD FOR DAPS
EMDOT		EQUALS	FLAGFILL	# I(1)PL (SPS FLOW RATE, SC AT 8.3KG/CS)

# EXIT FOR V83
STATEXIT	EQUALS	FLAGFILL +2	# I(1) STQ ADDRESS FOR STATEXIT
...
INREL		EQUALS	DOTRET		# INPUT BUFFER SELECTION
...
LSTPTR		EQUALS	TEM4		# LIST POINTER FOR GRABUST
...
V1		=	ENDBUF +1	# I(2) REENTRY, P65-P65
...
DNLSTCOD	ERASE			# B(1)PRM ID CODE OF DOWNLOAD
...
# (LUNAR AND SOLAR EPHEM) STORAGE.  -- PAD LOADED --	(77D)
TIMEMO		ERASE	+76D
...
ORIGEX		EQUALS	RPQV	+6	# I(1)TMP QSAVE FOR COORD. SWITCH ROUTINE
KEPRTN		EQUALS	ORIGEX		# 	  QSAVE FOR KEPLER
RQVV		EQUALS	ORIGEX +1	#  (6)    SEC. BODY TO VEH. VECTOR (USED P23)
...
# INCORPORATION/VERB 63 COMMON STORAGE		(1D)
...
# (*** SHARING IN THIS SECTION IS TEMPORARY ONLY ****)	(8D)
...
# MORE VERB 62 NOT SHARING WITH VERB 83		(9D)
...
THETA(1)	EQUALS	-TPER	+2	# I(2)TMP SET AT END OF V62
...
# THE FOLLOWING ARE ERASABLES USED BY TEH SYSTEM TESTS.  205 USES TRANSM1.  Q'S ARE NOT USED IN 205 NORE ARE THEY
# WHILE 504 USES TRANSM1 AND ALFDK.
TRANSM1		EQUALS	2000		# (19) INITIALIZATION FOR IMU TESTS
...
# ALIGNMENT/SYSTEST/AUTO OPTICS COMMON STORAGE.	(37D)
...
PERFDLAY	EQUALS	AINLA	+138D	# R(2)..........
...
# ***** PAD LOADED TVC DAP VARIABLES ****************	(28D)
...
PACTOFF		ERASE			# B(1)PL, DSP N48 R01 = PTRIM, RD2 = YTRIM
...
OMEGAB		EQUALS	TVCRCS +6	# B(?)
OMEGAYB		EQUALS	OMEGAB +2
OMEGAZB		EQUALS	OMEGAYB +2
...
CDUXD		EQUALS	CAPSI	+1	# B(1)TMP
CDUYD		EQUALS	CDUXD	+2	# B(1)TMP
CDUZD		EQUALS	CDUYD	+2	# B(1)TMP
...
ROLLC		EQUALS	ROLLTM	+1	# 20 ROLLCOM/360 FROM ENTRY (FOR TM)
...
BOOSTEMP	EQUALS	HDOTDISP +2	# B(2)TMP

# P21 STORAGE
GENRET		EQUALS	RAD	+2	# R(1)TMP
...
YPIPBUF		EQUALS	XPIPBUF	+1	# B(1) PIPS FILED HERE EACH .3 SEC APPEAR
...
DELVLVC		=	DELVSLV
END-DELV	ERASE			# *NEXT AVAIL LOC AFTER UNSHARED ET*
...
#GAMMAL		=	GAMMABI		# 2P SHARED FOR TM.  P64
GAMMAL1		=	22D		# 2P
...
ASPUP		=	ASPS	+2	# 2P	HI-WORD ONLY APPEARING
...
ASPS		EQUALS	TTE	+2	# I(2) HI-WORD ONLY ON DNLIST FOR TEM
...
# S22.1 STORAGE.				(36D)
SVMRKDAT	EQUALS	GRP2SVQ	+1	# I(36)TMP 5 SETS OF MARK DATA +PAD OF ON

# **** CISLUNAR NAV.  ERAS.  (P20'S) ****	(57D)
...
VGDISP		=	DELVSAB		# DSP NOUN 40,42,99 FOR P30,34,35,40,41.
