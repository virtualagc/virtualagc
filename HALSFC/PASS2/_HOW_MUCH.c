/*
  File _HOW_MUCH.c generated by XCOM-I, 2024-08-09 12:39:31.
*/
...
_HOW_MUCH (int reset)
{
  ...
  // ANSBYTES=_SPACE_ROUND(_DOPE_WIDTH(DOPE)*ANS); (590)
  {
  ...
  // IF ANSBYTES <= NSTRBYTES+FREEBYTES THEN (594)
  if (1
  ...
  // CALL _TAKE_BACK(ANSBYTES-NSTRBYTES-FREEBYTES); (596)
  {
  ...
  // IF ANSBYTES > NSTRBYTES + FREEBYTES THEN (597)
  if (1
      & (xGT (getFIXED (m_HOW_MUCHxANSBYTES),
              xadd (getFIXED (m_HOW_MUCHxNSTRBYTES), getFIXED (mFREEBYTES)))))
    // IF ~ ( ( COREWORD(DOPE+20) & 0x3000000 ) ^ = 0 ) THEN (598)
    if (1
        & (xNOT (xNEQ (
      ...
      rs1:;
        ...
        // CALL COMPACTIFY; (601)
        COMPACTIFY (0);
        // NSTRBYTES=FREELIMIT-FREEPOINT-FREESTRING_TRIGGER; (602)
        {
          int32_t numberRHS
              = (int32_t)(xsubtract (xsubtract (FREELIMIT (), FREEPOINT ()),
                                     getFIXED (mFREESTRING_TRIGGER)));
        ...
        // IF ANSBYTES > NSTRBYTES+FREEBYTES THEN (603)
        if (1
            & (xGT (getFIXED (m_HOW_MUCHxANSBYTES),
                    xadd (getFIXED (m_HOW_MUCHxNSTRBYTES),
                          getFIXED (mFREEBYTES)))))
          // ANS=((NSTRBYTES+FREEBYTES)/_DOPE_WIDTH(DOPE)) -1; (604)
          {
        ...
        if (1
          ...
          // -FREESTRING_MIN))/_DOPE_WIDTH(DOPE)-1; (606)
          {
            int32_t numberRHS = (int32_t)(xsubtract (
                xdivide (
