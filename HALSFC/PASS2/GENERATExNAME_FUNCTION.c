/*
  File GENERATExNAME_FUNCTION.c generated by XCOM-I, 2024-08-08 04:32:26.
*/
...
GENERATExNAME_FUNCTION (int reset)
{
  ...
  // IF (HALMAT_OPCODE = XPMAR) THEN (2781)
  if (1 & (xEQ (COREHALFWORD (mHALMAT_OPCODE), 90)))
    // DO ; (2782)
    {
    rs1:;
      // IF (PMINDEX = NCOPY_TAG) | ( (PMINDEX = NADD_TAG) &
      // ((ARG_STACK_PTR-SAVE_ARG_STACK_PTR) ~= 2) )  THEN (2783)
      if (1
  ...
  // IF (HALMAT_OPCODE = XPMIN) THEN (2785)
  if (1 & (xEQ (COREHALFWORD (mHALMAT_OPCODE), 91)))
    // DO ; (2786)
    {
    rs2:;
      // IF (TAG = NCOPY_TAG) | ( (TAG = NADD_TAG) &  (OP ~=
      // ARG_STACK(SAVE_ARG_STACK_PTR+2)) )  THEN (2787)
      if (1
          & (xOR (
  ...
  // IF (HALMAT_OPCODE = XDSUB) & NAME_SUB THEN (2789)
  if (1
      & (xAND (xEQ (COREHALFWORD (mHALMAT_OPCODE), 25),
               BYTE0 (mGENERATExNAME_SUB))))
    // IF TAG1=6 | (TAG3 ~= 5 & TAG3 ~= 1) THEN (2790)
    if (1
  ...
  // IF (HALMAT_OPCODE = XXXAR) THEN (2792)
  if (1 & (xEQ (COREHALFWORD (mHALMAT_OPCODE), 39)))
    // DO; (2793)
    {
    rs3:;
      // IF ARG_NAME(ARG_STACK_PTR) THEN (2794)
      if (1
  ...
  // IF (HALMAT_OPCODE = XPCAL) | (HALMAT_OPCODE = XFCAL) THEN (2796)
  if (1
