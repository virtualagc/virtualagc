/*
  File _MOVE_RECS.c generated by XCOM-I, 2024-08-09 12:39:31.
*/
...
_MOVE_RECS (int reset)
{
  ...
  // IF FIRSTBLOCK ~= 0 THEN (521)
  if (1 & (xNEQ (getFIXED (mFIRSTBLOCK), 0)))
    // IF COREWORD ( FIRSTBLOCK ) = 0 THEN (522)
    if (1 & (xEQ (COREWORD (getFIXED (mFIRSTBLOCK)), 0)))
      // IF COREWORD(FIRSTBLOCK-4) >= BYTES_TO_MOVE_BY THEN (523)
      if (1
        ...
        {
        rs1:;
          // DO UNTIL CURDOPE=0; (525)
          do
            {
              // SOURCE=_DOPE_POINTER(CURDOPE); (526)
              {
                int32_t numberRHS
                    = (int32_t)(COREWORD (getFIXED (m_MOVE_RECSxCURDOPE)));
                putFIXED (m_MOVE_RECSxSOURCE, numberRHS);
              }
              // NBYTES=NBYTES+ _RECORD#BYTES(CURDOPE); (527)
              {
              ...
              // CURDOPE=_DOPE_NEXT(CURDOPE); (529)
              {
          ...
          // CALL _REDUCE_BLOCK(FIRSTBLOCK,BYTES_TO_MOVE_BY,_TRUE); (530)
          {
          ...
          // CALL _MOVE_WORDS(SOURCE,SOURCE-BYTES_TO_MOVE_BY,NBYTES); (531)
          {
  ...
  // _FREEBLOCK_SIZE(FIRSTBLOCK)); (533)
  {
    putCHARACTERp (
        m_SPMANERRxMSG,
