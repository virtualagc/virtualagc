/*
  File GENERATExGENLIBCALL.c generated by XCOM-I, 2024-08-08 04:32:26.
*/
...
GENERATExGENLIBCALL (int reset)
{
  ...
  // IF LIB_POINTER = 0 | OLD_LINKAGE THEN (6089)
  if (1 & (xOR (xEQ (COREHALFWORD (mLIB_POINTER), 0), BYTE0 (mOLD_LINKAGE))))
    // CALL SAVE_REGS(MAXARG, 3); (6090)
    {
  ...
  // ELSE (6091)
    ...
    {
    rs1:;
      // REGBITS = LIB_REGS(ABS(LIB_POINTER)); (6093)
      {
        descriptor_t *bitRHS
  ...
  // CALL EMIT_CALL(ENTER_CALL(NAME), INT); (6097)
  {
  ...
  // IF LIB_POINTER = 0 | OLD_LINKAGE THEN (6098)
  if (1 & (xOR (xEQ (COREHALFWORD (mLIB_POINTER), 0), BYTE0 (mOLD_LINKAGE))))
    // CALL CLEAR_CALL_REGS(MAXARG, INT); (6099)
    {
      ...
      for (putBIT (16, mGENERATExGENLIBCALLxI, fixedToBit (16, from78));
           bitToFixed (getBIT (16, mGENERATExGENLIBCALLxI)) <= to78; putBIT (
               ...
               fixedToBit (16, bitToFixed (getBIT (16, mGENERATExGENLIBCALLxI))
                                   + by78)))
        {
          // IF (POW_OF_2(I) & REGBITS) ~= 0 THEN (6102)
          if (1
              & (xNEQ (xAND (SHL (1, COREHALFWORD (mGENERATExGENLIBCALLxI)),
                             COREHALFWORD (mGENERATExGENLIBCALLxREGBITS)),
                       0)))
