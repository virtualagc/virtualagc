/*
  File _SQUASH_RECORDS.c generated by XCOM-I, 2024-08-08 04:31:11.
*/
...
// BLOCKLOOP: (160)
BLOCKLOOP:
  // DO WHILE CURBLOCK > 0; (161)
  while (1 & (xGT (getFIXED (m_SQUASH_RECORDSxCURBLOCK), 0)))
    {
      // BYTES_TO_MOVE_BY=BYTES_TO_MOVE_BY+_FREEBLOCK_SIZE(CURBLOCK); (162)
      {
      ...
      // DO UNTIL RECPTR<NEXTBLOCK; (172)
      do
        {
          // RECBYTES=_RECORD#BYTES(CURDOPE); (173)
          {
          ...
          // SQUASHED=SQUASHED+RECBYTES; (175)
          {
            int32_t numberRHS
          ...
          // CURDOPE=_DOPE_NEXT(CURDOPE); (177)
          {
            int32_t numberRHS = (int32_t)(COREWORD (
                xadd (getFIXED (m_SQUASH_RECORDSxCURDOPE), 16)));
            putFIXED (m_SQUASH_RECORDSxCURDOPE, numberRHS);
          }
          // IF CURDOPE <= 0 THEN (178)
          if (1 & (xLE (getFIXED (m_SQUASH_RECORDSxCURDOPE), 0)))
            // ESCAPE BLOCKLOOP; (179)
            goto es2;
          // LAST_RECPTR=RECPTR; (180)
          {
            int32_t numberRHS = (int32_t)(getFIXED (m_SQUASH_RECORDSxRECPTR));
            putFIXED (m_SQUASH_RECORDSxLAST_RECPTR, numberRHS);
          }
          // RECPTR=_DOPE_POINTER(CURDOPE); (181)
          {
            int32_t numberRHS
                = (int32_t)(COREWORD (getFIXED (m_SQUASH_RECORDSxCURDOPE)));
            putFIXED (m_SQUASH_RECORDSxRECPTR, numberRHS);
  ...
  // IF SQUASHED > 0 THEN (188)
  if (1 & (xGT (getFIXED (m_SQUASH_RECORDSxSQUASHED), 0)))
    // CALL _FREEBLOCK_CHECK; (189)
    _FREEBLOCK_CHECK (0);
  {
    reentryGuard = 0;
    return 0;
