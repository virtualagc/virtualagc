/*
  File SCANxPARM_FOUND.c generated by XCOM-I, 2024-08-08 04:31:11.
*/
...
SCANxPARM_FOUND (int reset)
{
  ...
  // DO WHILE TEMP_INDEX>0; (6787)
  while (1 & (xGT (COREHALFWORD (mTEMP_INDEX), 0)))
    {
      // DO I=1 TO NUM_OF_PARM(TEMP_INDEX); (6788)
      {
        ...
        for (putFIXED (mI, from106); getFIXED (mI) <= to106;
          ...
          {
            // IF BCD=SYT_NAME(MACRO_EXPAN_STACK(TEMP_INDEX)+I) THEN (6789)
            if (1
                & (xsEQ (
                    getCHARACTER (mBCD),
                    getCHARACTER (
              ...
              {
              rs1s1s1:;
                // PARM_EXPAN_LEVEL = PARM_EXPAN_LEVEL + 1; (6791)
                {
                  int32_t numberRHS
                ...
                // FIRST_TIME_PARM(PARM_EXPAN_LEVEL)=TRUE; (6792)
                {
                ...
                // PARM_REPLACE_PTR(PARM_EXPAN_LEVEL) = 0; (6793)
                {
                ...
                // TOP_OF_PARM_STACK-PARM_COUNT; (6794)
                {
                ...
                if (1
                  ...
                  {
                  rs1s1s1s1:;
                    // IF ~FOUND_CENT THEN (6797)
                    if (1 & (xNOT (BYTE0 (mFOUND_CENT))))
                      // DO; (6798)
                      {
                      rs1s1s1s1s1:;
                        // IF ~END_OF_MACRO THEN (6799)
                        if (1 & (xNOT (bitToFixed (SCANxEND_OF_MACRO (0)))))
                          // DO; (6800)
                          {
                          rs1s1s1s1s1s1:;
                            // IF MACRO_TEXT(MACRO_POINT-2)= 238 THEN (6801)
                            if (1
                                & (xEQ (
                                    BYTE0 (
                                        ...
                                        + 0 + 1 * (0)),
                                    238)))
                              // MACRO_POINT=MACRO_POINT-1; (6802)
                              {
                                int32_t numberRHS = (int32_t)(xsubtract (
                                    getFIXED (mMACRO_POINT), 1));
                                putFIXED (mMACRO_POINT, numberRHS);
                              }
                            // MACRO_POINT=MACRO_POINT-1; (6803)
                            {
                              int32_t numberRHS = (int32_t)(xsubtract (
                                  getFIXED (mMACRO_POINT), 1));
                              putFIXED (mMACRO_POINT, numberRHS);
                ...
                // ELSE (6804)
                  ...
                  {
                  rs1s1s1s2:;
                    // IF FIRST_TIME_PARM(PARM_EXPAN_LEVEL-1) THEN (6806)
                    if (1
                        & (bitToFixed (getBIT (
                            1, mFIRST_TIME_PARM
                                   + 1
                    ...
                    // ELSE (6810)
                      ...
                      // FIRST_TIME_PARM(PARM_EXPAN_LEVEL-1)=TRUE; (6811)
                      {
                        int32_t numberRHS = (int32_t)(1);
                        descriptor_t *bitRHS;
                        bitRHS = fixedToBit (32, (int32_t)(numberRHS));
                ...
                // P_CENT(PARM_EXPAN_LEVEL)=FOUND_CENT; (6813)
                {
      ...
      // PARM_COUNT=PARM_COUNT + NUM_OF_PARM(TEMP_INDEX); (6817)
      {
