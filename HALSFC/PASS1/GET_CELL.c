/*
  File GET_CELL.c generated by XCOM-I, 2024-08-08 04:31:11.
*/
...
GET_CELL (int reset)
{
  ...
  // IF CELL_SIZE < VMEM_PAGE_SIZE THEN (1021)
  if (1 & (xLT (getFIXED (mGET_CELLxCELL_SIZE), 3360)))
    // DO; (1022)
    {
    rs2:;
      // IF VMEM_LOOK_AHEAD THEN (1023)
      if (1 & (bitToFixed (getBIT (1, mVMEM_LOOK_AHEAD))))
        // DO; (1024)
        {
        rs2s1:;
          // PAGE = VMEM_LAST_PAGE; (1025)
          {
          ...
          // AVAIL_SIZE = VMEM_PAGE_AVAIL_SPACE(PAGE); (1026)
          {
          ...
          // IF AVAIL_SIZE >= CELL_SIZE THEN (1027)
          if (1
              & (xGE (getFIXED (mGET_CELLxAVAIL_SIZE),
                      getFIXED (mGET_CELLxCELL_SIZE))))
            // GO TO GET_SPACE; (1028)
            goto GET_SPACE;
          // ELSE (1029)
          else
            // GO TO EXTEND_VMEM; (1030)
            goto EXTEND_VMEM;
        es2s1:;
        } // End of DO block
      // DO I = 0 TO VMEM_LAST_PAGE; (1031)
      {
        ...
        for (putFIXED (mGET_CELLxI, from21); getFIXED (mGET_CELLxI) <= to21;
          ...
          {
            // PAGE = VMEM_LAST_PAGE - I; (1032)
            {
              int32_t numberRHS = (int32_t)(xsubtract (
                  COREHALFWORD (mVMEM_LAST_PAGE), getFIXED (mGET_CELLxI)));
              putFIXED (mGET_CELLxPAGE, numberRHS);
            }
            // AVAIL_SIZE = VMEM_PAGE_AVAIL_SPACE(PAGE); (1033)
            {
              descriptor_t *bitRHS = getBIT (
                  16, mVMEM_PAGE_AVAIL_SPACE + 2 * getFIXED (mGET_CELLxPAGE));
              int32_t numberRHS;
