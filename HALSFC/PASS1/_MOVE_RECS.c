/*
  File _MOVE_RECS.c generated by XCOM-I, 2024-08-08 04:31:11.
*/
...
_MOVE_RECS (int reset)
{
  ...
  // IF FIRSTBLOCK ~= 0 THEN (511)
  if (1 & (xNEQ (getFIXED (mFIRSTBLOCK), 0)))
    // IF COREWORD ( FIRSTBLOCK ) = 0 THEN (512)
    if (1 & (xEQ (COREWORD (getFIXED (mFIRSTBLOCK)), 0)))
      // IF COREWORD(FIRSTBLOCK-4) >= BYTES_TO_MOVE_BY THEN (513)
      if (1
        ...
        {
        rs1:;
          // DO UNTIL CURDOPE=0; (515)
          do
            {
              // SOURCE=_DOPE_POINTER(CURDOPE); (516)
              {
                int32_t numberRHS
                    = (int32_t)(COREWORD (getFIXED (m_MOVE_RECSxCURDOPE)));
                putFIXED (m_MOVE_RECSxSOURCE, numberRHS);
              }
              // NBYTES=NBYTES+ _RECORD#BYTES(CURDOPE); (517)
              {
              ...
              // CURDOPE=_DOPE_NEXT(CURDOPE); (519)
              {
          ...
          // CALL _REDUCE_BLOCK(FIRSTBLOCK,BYTES_TO_MOVE_BY,_TRUE); (520)
          {
          ...
          // CALL _MOVE_WORDS(SOURCE,SOURCE-BYTES_TO_MOVE_BY,NBYTES); (521)
          {
  ...
  // _FREEBLOCK_SIZE(FIRSTBLOCK)); (523)
  {
    putCHARACTERp (
        m_SPMANERRxMSG,
