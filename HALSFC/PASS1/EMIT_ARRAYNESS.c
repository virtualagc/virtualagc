/*
  File EMIT_ARRAYNESS.c generated by XCOM-I, 2024-08-08 04:31:11.
*/
...
EMIT_ARRAYNESS (int reset)
{
  ...
  // IF CURRENT_ARRAYNESS>0 THEN (7656)
  if (1 & (xGT (COREHALFWORD (mCURRENT_ARRAYNESS), 0)))
    // DO; (7657)
    {
    rs1:;
      // CALL HALMAT_POP(ACODE,CURRENT_ARRAYNESS,XCO_D,FCN_LV); (7658)
      {
      ...
      // DO I=1 TO CURRENT_ARRAYNESS; (7659)
      {
        ...
        for (putBIT (16, mEMIT_ARRAYNESSxI, fixedToBit (16, from119));
             bitToFixed (getBIT (16, mEMIT_ARRAYNESSxI)) <= to119; putBIT (
                 ...
                 fixedToBit (16, bitToFixed (getBIT (16, mEMIT_ARRAYNESSxI))
                                     + by119)))
          {
            // IF CURRENT_ARRAYNESS(I)<0 THEN (7660)
            if (1
                & (xLT (COREHALFWORD (mCURRENT_ARRAYNESS
                                      + 2 * COREHALFWORD (mEMIT_ARRAYNESSxI)),
                        0)))
              // CALL HALMAT_PIP(-CURRENT_ARRAYNESS(I),XASZ,0,0); (7661)
              {
                putBITp (
            ...
            // ELSE (7662)
              ...
              // CALL HALMAT_PIP(CURRENT_ARRAYNESS(I),XIMD,0,0); (7663)
              {
                putBITp (
                    16, mHALMAT_PIPxOPERAND,
      ...
      // CALL HALMAT_POP(XDLPE,0,XCO_N,FCN_LV); (7664)
      {
