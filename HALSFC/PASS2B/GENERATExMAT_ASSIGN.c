/*
  File GENERATExMAT_ASSIGN.c generated by XCOM-I, 2024-08-08 04:34:25.
*/
...
GENERATExMAT_ASSIGN (int reset)
{
  ...
  // IF VDLP_IN_EFFECT THEN (8954)
  if (1 & (bitToFixed (getBIT (1, mVDLP_IN_EFFECT))))
    // DO LHSPTR=2 TO NUMOP; (8955)
    {
      ...
      for (putBIT (16, mLHSPTR, fixedToBit (16, from97));
        ...
        {
          // VDLP_ACTIVE = VDLP_ACTIVE | SHR(X_BITS(LHSPTR), 1); (8956)
          {
  ...
  // IF VDLP_ACTIVE THEN (8957)
  if (1 & (bitToFixed (getBIT (1, mVDLP_ACTIVE))))
    // CALL DO_ASSIGNMENT; (8958)
  ...
  // ELSE (8959)
    ...
    rs2:;
      ...
      // IF NUMOP > 2 THEN (8962)
      if (1 & (xGT (COREHALFWORD (mNUMOP), 2)))
        // RIGHTOP = CHECK_AGGREGATE_ASSIGN(RIGHTOP); (8963)
        {
          descriptor_t *bitRHS
      ...
      // IF SYMFORM( FORM(RIGHTOP) ) THEN (8964)
      if (1
          & (bitToFixed (getBIT (
              1, mSYMFORM
                     + 1
        ...
        if (1
          ...
          {
          rs2s1:;
            // TEMPOPER = MOD_GET_OPERAND(1); (8967)
            {
              descriptor_t *bitRHS
            ...
            // IF NAME_OP_FLAG | SUBSTRUCT_FLAG THEN (8968)
            if (1
            ...
            while (
                         ...
                         xLE (COREHALFWORD (mLHSPTR), COREHALFWORD (mNUMOP)))))
              {
                ...
                // IF NAME_OP_FLAG | SUBSTRUCT_FLAG THEN (8976)
                if (1
                    & (xOR (BYTE0 (mGENERATExNAME_OP_FLAG),
                            BYTE0 (mGENERATExSUBSTRUCT_FLAG))))
                  // NEED_TEMP = TRUE; (8977)
                ...
                // IF ~VAC_FLAG THEN (8985)
                if (1 & (xNOT (BYTE0 (mGENERATExVAC_FLAG))))
                  // CALL RETURN_STACK_ENTRY( LEFTOP ); (8986)
                  {
                    putBITp (16, mGENERATExRETURN_STACK_ENTRYxP,
                             getBIT (16, mLEFTOP));
                    GENERATExRETURN_STACK_ENTRY (0);
                  }
                // LHSPTR = LHSPTR + 1; (8987)
                {
                  int32_t numberRHS
            ...
            // IF NEED_TEMP THEN (8988)
            if (1 & (bitToFixed (getBIT (1, mGENERATExMAT_ASSIGNxNEED_TEMP))))
              // RIGHTOP = VECMAT_CONVERT( RIGHTOP ); (8989)
              {
                int32_t numberRHS
      ...
      // DO LHSPTR = 2 TO NUMOP; (8991)
      {
        ...
        for (putBIT (16, mLHSPTR, fixedToBit (16, from98));
          ...
          {
            // LEFTOP = GET_OPERAND( LHSPTR ); (8992)
            {
              descriptor_t *bitRHS = (putBITp (16, mGENERATExGET_OPERANDxOP,
                                               getBIT (16, mLHSPTR)),
                                      GENERATExGET_OPERAND (0));
            ...
            // CALL ASSIGN_CLEAR(LEFTOP, 1); (8993)
            {
            ...
            // TEMPSPACE = ROW(LEFTOP) * COLUMN(LEFTOP); (8994)
            {
            ...
            // CALL VECMAT_ASSIGN(LEFTOP,RIGHTOP); (8995)
            {
      ...
      // CALL RETURN_STACK_ENTRY(RIGHTOP); (8998)
      {
