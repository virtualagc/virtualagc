...
GENERATExRETURN_STACK_ENTRYxRETURN_ENTRY (int reset)
{
  ...
  if (1
    ...
    rs1:;
      ...
      // SAVEPOINT(SAVEPTR) = NR_STACK(PTR); (2872)
      {
  ...
  // IF STACK_DUMP THEN (2875)
  if (1 & (bitToFixed (getBIT (1, mSTACK_DUMP))))
    // CALL DUMP_STACK(PTR); (2876)
    {
  ...
  // ELSE (2877)
    ...
    // IF DIAGNOSTICS THEN (2878)
    if (1 & (bitToFixed (getBIT (1, mDIAGNOSTICS))))
      // OUTPUT = 'RET STACK ' || PTR; (2879)
      {
