/*
  File _MOVE_RECS.c generated by XCOM-I, 2024-08-09 12:41:32.
*/
...
_MOVE_RECS (int reset)
{
  ...
  // IF FIRSTBLOCK ~= 0 THEN (519)
  if (1 & (xNEQ (getFIXED (mFIRSTBLOCK), 0)))
    // IF COREWORD ( FIRSTBLOCK ) = 0 THEN (520)
    if (1 & (xEQ (COREWORD (getFIXED (mFIRSTBLOCK)), 0)))
      // IF COREWORD(FIRSTBLOCK-4) >= BYTES_TO_MOVE_BY THEN (521)
      if (1
        ...
        {
        rs1:;
          // DO UNTIL CURDOPE=0; (523)
          do
            {
              // SOURCE=_DOPE_POINTER(CURDOPE); (524)
              {
                int32_t numberRHS
                    = (int32_t)(COREWORD (getFIXED (m_MOVE_RECSxCURDOPE)));
                putFIXED (m_MOVE_RECSxSOURCE, numberRHS);
              }
              // NBYTES=NBYTES+ _RECORD#BYTES(CURDOPE); (525)
              {
              ...
              // CURDOPE=_DOPE_NEXT(CURDOPE); (527)
              {
          ...
          // CALL _REDUCE_BLOCK(FIRSTBLOCK,BYTES_TO_MOVE_BY,_TRUE); (528)
          {
          ...
          // CALL _MOVE_WORDS(SOURCE,SOURCE-BYTES_TO_MOVE_BY,NBYTES); (529)
          {
  ...
  // _FREEBLOCK_SIZE(FIRSTBLOCK)); (531)
  {
    putCHARACTERp (
        m_SPMANERRxMSG,
