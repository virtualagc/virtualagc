/*
  File GENERATExARRAY_INDEX_MOD.c generated by XCOM-I, 2024-08-08 04:34:25.
*/
...
GENERATExARRAY_INDEX_MOD (int reset)
{
  ...
  // IF INDEX = 0 THEN (5634)
  if (1 & (xEQ (COREHALFWORD (mGENERATExARRAY_INDEX_MODxINDEX), 0)))
    // GO TO RETURN_OP; (5635)
    goto RETURN_OP;
  // IF SELF_ALIGNING THEN (5636)
  ...
  if (1 & (xEQ (COREHALFWORD (mGENERATExARRAY_INDEX_MODxOP), 0)))
    ...
    rs1:;
      ...
      // NEXT_USE(OP) = FORM(INDEX) = VAC; (5645)
      {
      ...
      // CALL EMITOP(LH, REG(OP), INDEX); (5655)
      {
      ...
      // CALL SET_USAGE(REG(OP), AIDX, INDEX); (5656)
      {
        putBITp (
  ...
  // ELSE (5661)
    ...
    rs2:;
      ...
      // IF REG(0) >= 0 THEN (5668)
      if (1
          & (xGE (
              COREHALFWORD (getFIXED (mIND_STACK) + 73 * (0) + 46 + 2 * (0)),
              0)))
        // CALL EMITRR(AR, REG(OP), REG(0)); (5669)
        {
      ...
      // ELSE (5670)
        ...
        // CALL EMITOP(AH, REG(OP), INDEX); (5671)
        {
      ...
      // CALL UNRECOGNIZABLE(REG(OP)); (5672)
      {
        putBITp (
