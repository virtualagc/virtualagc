...
GENERATExGEN_CLASS0xPROC_FUNC_SETUPxPARM_ARRAYNESS (int reset)
{
  ...
  // IF SYMFORM(FORM(RIGHTOP)) THEN (11452)
  if (1
      & (bitToFixed (
          getBIT (1, mSYMFORM
                         + 1
                               * COREHALFWORD (getFIXED (mIND_STACK)
                                               ...
                                               + 32 + 2 * (0))))))
    // IF TYPE(RIGHTOP) = STRUCTURE THEN (11453)
    if (1
        & (xEQ (COREHALFWORD (getFIXED (mIND_STACK)
                              + 73 * (COREHALFWORD (mRIGHTOP)) + 50 + 2 * (0)),
                BYTE0 (mSTRUCTURE))))
      // RETURN SYT_ARRAY(LOC(RIGHTOP)); (11454)
      {
        reentryGuard = 0;
        return getBIT (
    ...
    // ELSE (11455)
      ...
      // RETURN GETARRAYDIM(1, LOC2(RIGHTOP)); (11456)
      {
        reentryGuard = 0;
  ...
  // RETURN SF_RANGE(VAL(RIGHTOP)); (11457)
  {
    reentryGuard = 0;
