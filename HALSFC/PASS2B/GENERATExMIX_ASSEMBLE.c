/*
  File GENERATExMIX_ASSEMBLE.c generated by XCOM-I, 2024-08-09 12:41:32.
*/
...
GENERATExMIX_ASSEMBLE (int reset)
{
  ...
  // LITTYPE = TYPE(LEFTOP)&8 | SCALAR; (7249)
  {
  ...
  // LEFTMODE = OPMODE(TYPE(LEFTOP)); (7251)
  {
    descriptor_t *bitRHS
        = getBIT (8, mOPMODE
                         + 1
  ...
  // RIGHTMODE = OPMODE(TYPE(RIGHTOP)); (7252)
  {
    descriptor_t *bitRHS
        = getBIT (8, mOPMODE
                         + 1
  ...
  // STMT_PREC=(TYPE(LEFTOP) & 8)|(TYPE(RIGHTOP) & 8); (7254)
  {
    int32_t numberRHS = (int32_t)(xOR (
        xAND (COREHALFWORD (getFIXED (mIND_STACK)
                            + 73 * (COREHALFWORD (mLEFTOP)) + 50 + 2 * (0)),
  ...
  // IF ~VDLP_ACTIVE THEN (7255)
  if (1 & (xNOT (BYTE0 (mVDLP_ACTIVE))))
    // DO; (7256)
    {
    rs1:;
      // IF LEFTMODE ~= RIGHTMODE THEN (7257)
      if (1
        ...
        {
        rs1s1:;
          // IF LEFTMODE < RIGHTMODE THEN (7259)
          if (1
              & (xLT (COREHALFWORD (mGENERATExMIX_ASSEMBLExLEFTMODE),
                      COREHALFWORD (mGENERATExMIX_ASSEMBLExRIGHTMODE))))
            // LEFTOP = VECMAT_CONVERT(LEFTOP, 8); (7260)
            {
              int32_t numberRHS
      ...
      // IF CHECK_REMOTE(LEFTOP) | DEL(LEFTOP) > 0 THEN (7263)
      if (1
          & (xOR (bitToFixed ((putBITp (16, mGENERATExCHECK_REMOTExOP,
                  ...
                  xGT (COREHALFWORD (getFIXED (mIND_STACK)
                                     ...
                                     + 2 * (0)),
                       0))))
        // LEFTOP = VECMAT_CONVERT(LEFTOP); (7264)
        {
      ...
      // IF DATA_REMOTE & (CSECT_TYPE(LOC(LEFTOP),LEFTOP)=LOCAL#D) THEN (7265)
      if (1
          & (xAND (
              BYTE0 (mDATA_REMOTE),
              xEQ (bitToFixed ((
                       ...
                       CSECT_TYPE (0))),
                   6))))
        // LEFTOP = VECMAT_CONVERT(LEFTOP); (7266)
        {
          int32_t numberRHS = (int32_t)((
              putBITp (16, mGENERATExVECMAT_CONVERTxOP, getBIT (16, mLEFTOP)),
              GENERATExVECMAT_CONVERT (0)));
      ...
      // CALL STACK_TARGET(RIGHTOP); (7270)
      {
  ...
  // OPTYPE = TYPE(LEFTOP) | STMT_PREC; (7272)
  {
    int32_t numberRHS = (int32_t)(xOR (
