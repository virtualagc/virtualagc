/*
  File _NEEDMORE_SPACE.c generated by XCOM-I, 2024-08-08 04:32:08.
*/
...
_NEEDMORE_SPACE (int reset)
{
  ...
  // CALL _CHECK_FOR_THEFT; (621)
  _CHECK_FOR_THEFT (0);
  // IF _IS_REC_CONSTANT(DOPE) THEN (622)
  if (1
      & (xNEQ (xAND (COREWORD (xadd (getFIXED (m_NEEDMORE_SPACExDOPE), 20)),
                     50331648),
               0)))
    // CALL _SPMANERR( 'TRIED TO INCREASE CONSTANT RECORD, DOPE='||DOPE); (623)
    {
      putCHARACTERp (
          m_SPMANERRxMSG,
          xsCAT (
  ...
  // IF _DOPE_ALLOC(DOPE)=0 THEN (624)
  if (1 & (xEQ (COREWORD (xadd (getFIXED (m_NEEDMORE_SPACExDOPE), 8)), 0)))
    // CALL _ALLOCATE_SPACE(DOPE,-1 _IFA(NAME)); (625)
    {
  ...
  // ELSE (626)
    ...
    // CALL _INCREASE_RECORD(DOPE,_HOW_MUCH(DOPE,0)); (627)
    {
