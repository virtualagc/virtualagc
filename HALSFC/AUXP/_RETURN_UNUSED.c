/*
  File _RETURN_UNUSED.c generated by XCOM-I, 2024-08-08 04:32:08.
*/
...
_RETURN_UNUSED (int reset)
{
  ...
  // DIF=_DOPE_ALLOC(DOPE)-_DOPE_USED(DOPE); (301)
  {
  ...
  // ELSE (308)
    ...
    if (1
      ...
      {
      rs2:;
        // _NUM_TIMES_ZEROED(DOPE)=_NUM_TIMES_ZEROED(DOPE)+1; (311)
        {
        ...
        // IF _NUM_TIMES_ZEROED(DOPE)>_MAX_ZEROED THEN (312)
        if (1
  ...
  // IF DIF=_DOPE_ALLOC(DOPE) THEN (322)
  if (1
      & (xEQ (getFIXED (m_RETURN_UNUSEDxDIF),
              COREWORD (xadd (getFIXED (m_RETURN_UNUSEDxDOPE), 8)))))
    // CALL _RECORD_FREE(DOPE _IFA(NAME)); (323)
    {
  ...
  // ELSE (324)
    ...
    {
    rs4:;
      // TOTAL_RDESC=TOTAL_RDESC-NRECS*_DOPE_#DESCRIPTORS(DOPE); (326)
      {
        int32_t numberRHS = (int32_t)(xsubtract (
      ...
      // OLDNBYTES=_RECORD#BYTES(DOPE); (327)
      {
      ...
      // _DOPE_ALLOC(DOPE)=_DOPE_ALLOC(DOPE)-NRECS; (328)
      {
        int32_t numberRHS = (int32_t)(xsubtract (
            COREWORD (xadd (getFIXED (m_RETURN_UNUSEDxDOPE), 8)),
            getFIXED (m_RETURN_UNUSEDxNRECS)));
      ...
      // NEWNBYTES=_RECORD#BYTES(DOPE); (329)
      {
        int32_t numberRHS = (int32_t)((
            putFIXED (
                m_SPACE_ROUNDxBYTES,
      ...
      // NEWBLOCK=_DOPE_POINTER(DOPE)+OLDNBYTES-4; (333)
      {
        int32_t numberRHS = (int32_t)(xsubtract (
