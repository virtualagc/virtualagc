/*
  File _MOVE_RECS.c generated by XCOM-I, 2024-08-08 04:33:20.
*/
...
_MOVE_RECS (int reset)
{
  ...
  // IF FIRSTBLOCK ~= 0 THEN (495)
  if (1 & (xNEQ (getFIXED (mFIRSTBLOCK), 0)))
    // IF COREWORD ( FIRSTBLOCK ) = 0 THEN (496)
    if (1 & (xEQ (COREWORD (getFIXED (mFIRSTBLOCK)), 0)))
      // IF COREWORD(FIRSTBLOCK-4) >= BYTES_TO_MOVE_BY THEN (497)
      if (1
        ...
        {
        rs1:;
          // DO UNTIL CURDOPE=0; (499)
          do
            {
              // SOURCE=_DOPE_POINTER(CURDOPE); (500)
              {
                int32_t numberRHS
                    = (int32_t)(COREWORD (getFIXED (m_MOVE_RECSxCURDOPE)));
                putFIXED (m_MOVE_RECSxSOURCE, numberRHS);
              }
              // NBYTES=NBYTES+ _RECORD#BYTES(CURDOPE); (501)
              {
              ...
              // CURDOPE=_DOPE_NEXT(CURDOPE); (503)
              {
          ...
          // CALL _REDUCE_BLOCK(FIRSTBLOCK,BYTES_TO_MOVE_BY,_TRUE); (504)
          {
          ...
          // CALL _MOVE_WORDS(SOURCE,SOURCE-BYTES_TO_MOVE_BY,NBYTES); (505)
          {
  ...
  // _FREEBLOCK_SIZE(FIRSTBLOCK)); (507)
  {
    putCHARACTERp (
        m_SPMANERRxMSG,
