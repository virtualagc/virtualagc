/*
  File _RETURN_UNUSED.c generated by XCOM-I, 2024-08-09 12:40:27.
*/
...
_RETURN_UNUSED (int reset)
{
  ...
  // DIF=_DOPE_ALLOC(DOPE)-_DOPE_USED(DOPE); (286)
  {
  ...
  // ELSE (293)
    ...
    if (1
      ...
      {
      rs2:;
        // _NUM_TIMES_ZEROED(DOPE)=_NUM_TIMES_ZEROED(DOPE)+1; (296)
        {
        ...
        // IF _NUM_TIMES_ZEROED(DOPE)>_MAX_ZEROED THEN (297)
        if (1
  ...
  // IF DIF=_DOPE_ALLOC(DOPE) THEN (307)
  if (1
      & (xEQ (getFIXED (m_RETURN_UNUSEDxDIF),
              COREWORD (xadd (getFIXED (m_RETURN_UNUSEDxDOPE), 8)))))
    // CALL _RECORD_FREE(DOPE _IFA(NAME)); (308)
    {
  ...
  // ELSE (309)
    ...
    {
    rs4:;
      // TOTAL_RDESC=TOTAL_RDESC-NRECS*_DOPE_#DESCRIPTORS(DOPE); (311)
      {
        int32_t numberRHS = (int32_t)(xsubtract (
      ...
      // OLDNBYTES=_RECORD#BYTES(DOPE); (312)
      {
      ...
      // _DOPE_ALLOC(DOPE)=_DOPE_ALLOC(DOPE)-NRECS; (313)
      {
        int32_t numberRHS = (int32_t)(xsubtract (
            COREWORD (xadd (getFIXED (m_RETURN_UNUSEDxDOPE), 8)),
            getFIXED (m_RETURN_UNUSEDxNRECS)));
      ...
      // NEWNBYTES=_RECORD#BYTES(DOPE); (314)
      {
        int32_t numberRHS = (int32_t)((
            putFIXED (
                m_SPACE_ROUNDxBYTES,
      ...
      // NEWBLOCK=_DOPE_POINTER(DOPE)+OLDNBYTES-4; (318)
      {
        int32_t numberRHS = (int32_t)(xsubtract (
