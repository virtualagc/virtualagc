/*
  File _ALLOCATE_SPACE.c generated by XCOM-I, 2024-08-08 04:33:20.
*/
...
_ALLOCATE_SPACE (int reset)
{
  ...
  // IF _DOPE_WIDTH(DOPE) <= 0 THEN (587)
  if (1 & (xLE (COREHALFWORD (xadd (getFIXED (m_ALLOCATE_SPACExDOPE), 4)), 0)))
    // DO; (588)
    {
    rs1:;
      // CALL _SPMANERR('RECORD HAS WIDTH=0, DOPE='||DOPE); (589)
      {
        putCHARACTERp (
            m_SPMANERRxMSG,
  ...
  // IF _DOPE_ALLOC(DOPE) > 0 THEN (591)
  if (1 & (xGT (COREWORD (xadd (getFIXED (m_ALLOCATE_SPACExDOPE), 8)), 0)))
    // DO; (592)
    {
    rs2:;
      // CALL _SPMANERR('IN ALLOCATE_SPACE, ALREADY ALLOCATED: '|| DOPE); (593)
      {
        putCHARACTERp (
            m_SPMANERRxMSG,
            xsCAT (
