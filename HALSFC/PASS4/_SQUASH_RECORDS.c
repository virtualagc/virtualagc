/*
  File _SQUASH_RECORDS.c generated by XCOM-I, 2024-08-08 04:33:20.
*/
...
_SQUASH_RECORDS (int reset)
{
  ...
  while (1 & (xGT (getFIXED (m_SQUASH_RECORDSxCURDOPE), 0)))
    {
      ...
      if (1
        ...
        // ESCAPE; (139)
        break;
      // CURDOPE=_DOPE_NEXT(CURDOPE); (140)
      {
  ...
  // RECPTR=_DOPE_POINTER(CURDOPE); (143)
  {
    int32_t numberRHS
...
// BLOCKLOOP: (144)
BLOCKLOOP:
  // DO WHILE CURBLOCK > 0; (145)
  while (1 & (xGT (getFIXED (m_SQUASH_RECORDSxCURBLOCK), 0)))
    {
      // BYTES_TO_MOVE_BY=BYTES_TO_MOVE_BY+_FREEBLOCK_SIZE(CURBLOCK); (146)
      {
      ...
      // DO UNTIL RECPTR<NEXTBLOCK; (156)
      do
        {
          // RECBYTES=_RECORD#BYTES(CURDOPE); (157)
          {
          ...
          // SQUASHED=SQUASHED+RECBYTES; (159)
          {
            int32_t numberRHS
          ...
          // CURDOPE=_DOPE_NEXT(CURDOPE); (161)
          {
            int32_t numberRHS = (int32_t)(COREWORD (
                xadd (getFIXED (m_SQUASH_RECORDSxCURDOPE), 16)));
            putFIXED (m_SQUASH_RECORDSxCURDOPE, numberRHS);
          }
          // IF CURDOPE <= 0 THEN (162)
          if (1 & (xLE (getFIXED (m_SQUASH_RECORDSxCURDOPE), 0)))
            // ESCAPE BLOCKLOOP; (163)
            goto es2;
          // LAST_RECPTR=RECPTR; (164)
          {
  ...
  // IF SQUASHED > 0 THEN (172)
  if (1 & (xGT (getFIXED (m_SQUASH_RECORDSxSQUASHED), 0)))
    // CALL _FREEBLOCK_CHECK; (173)
    _FREEBLOCK_CHECK (0);
  {
    reentryGuard = 0;
    return 0;
