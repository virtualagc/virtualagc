/*
  File EMIT_EXTERNAL.c generated by XCOM-I, 2024-08-09 12:40:41.
*/
  ...
  rs1:
    switch (COREHALFWORD (mEXTERNALIZE))
      ...
      case 1:
        ...
        {
        rs1s1:;
          // IF TOKEN = REPLACE_TEXT THEN (6879)
          if (1 & (xEQ (getFIXED (mTOKEN), getFIXED (mREPLACE_TEXT))))
            // DO; (6880)
            {
            rs1s1s1:;
              // CALL ADD_CHAR(BYTE('"')); (6881)
              {
              ...
              // BLANK_NEEDED = TRUE; (6900)
              {
                int32_t numberRHS = (int32_t)(1);
                descriptor_t *bitRHS;
                bitRHS = fixedToBit (32, (int32_t)(numberRHS));
                putBIT (8, mEMIT_EXTERNALxBLANK_NEEDED, bitRHS);
                bitRHS->inUse = 0;
              }
            es1s1s1:;
            } // End of DO block
          // ELSE (6901)
            ...
            // IF RESERVED_WORD THEN (6902)
            if (1 & (bitToFixed (getBIT (1, mRESERVED_WORD))))
              // DO; (6903)
              {
              rs1s1s2:;
                // CHAR = STRING(VOCAB_INDEX(TOKEN)); (6904)
                {
                ...
                // IF LENGTH(CHAR) = 1 THEN (6905)
                if (1 & (xEQ (LENGTH (getCHARACTER (mEMIT_EXTERNALxCHAR)), 1)))
                  // BLANK_NEEDED = FALSE; (6906)
                  {
                ...
                // ELSE (6907)
                  ...
                  {
                  rs1s1s2s1:;
                    // IF BLANK_NEEDED THEN (6909)
                    if (1
                        & (bitToFixed (
                            getBIT (8, mEMIT_EXTERNALxBLANK_NEEDED))))
                      // CALL ADD_CHAR(BYTE(X1)); (6910)
                      {
                        putBITp (16, mEMIT_EXTERNALxADD_CHARxVAL,
                                 fixedToBit (32, (int32_t)(BYTE1 (
                                                     getCHARACTER (mX1)))));
                  ...
                  for (putBIT (16, mEMIT_EXTERNALxI, fixedToBit (16, from109));
                       ...
                       putBIT (16, mEMIT_EXTERNALxI,
                               fixedToBit (16, bitToFixed (getBIT (
                                                   ...
                                                   + by109)))
                    {
                      // CALL ADD_CHAR(BYTE(CHAR, I)); (6913)
                      {
                        putBITp (
                            16, mEMIT_EXTERNALxADD_CHARxVAL,
                            fixedToBit (
            ...
            // ELSE (6914)
              ...
              // IF TOKEN = CHARACTER_STRING THEN (6915)
              if (1 & (xEQ (getFIXED (mTOKEN), getFIXED (mCHARACTER_STRING))))
                // DO; (6916)
                {
                rs1s1s3:;
                  // CALL ADD_CHAR(BYTE(SQUOTE)); (6917)
                  {
                    putBITp (16, mEMIT_EXTERNALxADD_CHARxVAL,
                             fixedToBit (32, (int32_t)(BYTE1 (
                  ...
                  // DO WHILE (J ~= BYTE(SQUOTE)) & (I < LENGTH(BCD)); (6920)
                  while (1
                    ...
                    {
                      // IF (TRANS_OUT(J) &  255) ~= 0 THEN (6921)
                      if (1
                          & (xNEQ (
                              xAND (COREHALFWORD (
                                        mTRANS_OUT
                        ...
                        {
                        rs1s1s3s1s1:;
                          // DO K = 0 TO SHR(TRANS_OUT(J), 8) &  255; (6923)
                          {
                          ...
                          // CALL ADD_CHAR(TRANS_OUT(J) &  255); (6925)
                          {
                            putBITp (
                                16, mEMIT_EXTERNALxADD_CHARxVAL,
                                fixedToBit (
                  ...
                  // CALL ADD_CHAR(BYTE(SQUOTE)); (6931)
                  {
                    putBITp (16, mEMIT_EXTERNALxADD_CHARxVAL,
                             fixedToBit (32, (int32_t)(BYTE1 (
                                                 getCHARACTER (mSQUOTE)))));
                  ...
                  if (1
                    ...
                    {
                    rs1s1s3s2:;
                      // CALL ADD_CHAR(BYTE(SQUOTE)); (6934)
                      {
                        putBITp (
                            16, mEMIT_EXTERNALxADD_CHARxVAL,
                            fixedToBit (32, (int32_t)(BYTE1 (
              ...
              // ELSE (6937)
                ...
                {
                rs1s1s4:;
                  // IF BLANK_NEEDED THEN (6939)
                  if (1
                      & (bitToFixed (getBIT (8, mEMIT_EXTERNALxBLANK_NEEDED))))
                    // CALL ADD_CHAR(BYTE(X1)); (6940)
                    {
                      putBITp (16, mEMIT_EXTERNALxADD_CHARxVAL,
                               fixedToBit (
                                   32, (int32_t)(BYTE1 (getCHARACTER (mX1)))));
                      EMIT_EXTERNALxADD_CHAR (0);
                    }
                  // DO I = 0 TO LENGTH(BCD) - 1; (6941)
                  {
      ...
      case 2:
        ...
        rs1s2:;
          ...
          // NEWBUFF=PAD(' CLOSE ;   ',TPL_LRECL); (6947)
          {
            descriptor_t *stringRHS;
            stringRHS
          ...
          // IF TPL_FLAG=0 THEN (6952)
          if (1 & (xEQ (COREHALFWORD (mTPL_FLAG), 0)))
            // J,BYTE(VERSION,10)= 1; (6953)
            {
              int32_t numberRHS = (int32_t)(1);
              descriptor_t *bitRHS;
              bitRHS = fixedToBit (32, (int32_t)(numberRHS));
          ...
          // ELSE (6954)
            ...
            {
            rs1s2s1:;
              // DO WHILE LENGTH(OLDBUFF)>0; (6956)
              while (
                  1
                  & (xGT (LENGTH (getCHARACTER (mEMIT_EXTERNALxOLDBUFF)), 0)))
                {
                  // NEWBUFF=OLDBUFF; (6957)
                  {
                    descriptor_t *stringRHS;
              ...
              // CALL MONITOR(3, 7); (6959)
              MONITOR3 (7);
              // IF SUBSTR(NEWBUFF,0,10)~=SUBSTR(VERSION,0,10) THEN (6960)
              if (1
                  & (xsNEQ (
                ...
                rs1s2s1s2:;
                  ...
                  // IF ~SDL_OPTION THEN (6963)
                  if (1 & (xNOT (BYTE0 (mSDL_OPTION))))
                    // CALL ERROR(CLASS_XV, 1, TPL_NAME); (6964)
                    {
          ...
          // SYT_LOCK#(BLOCK_SYTREF(1))=J; (6975)
          {
          ...
          // OUTPUT(6) = VERSION || BYTE(VERSION, 10); (6976)
          {
            descriptor_t *stringRHS;
            stringRHS
      ...
      case 3:
        ...
        rs1s3:;
          ...
          // NEWBUFF=X1||VAR(MP)||NEWBUFF; (6979)
          {
            descriptor_t *stringRHS;
            stringRHS
          ...
          // NEWBUFF=PAD(NEWBUFF,TPL_LRECL); (6981)
          {
            descriptor_t *stringRHS;
            stringRHS
                = (putCHARACTERp (mPADxSTRING,
          ...
          // TPL_FLAG = FINDER(7, TPL_NAME, 1) = 0; (6984)
          {
          ...
          // IF TPL_FLAG THEN (6985)
          if (1 & (bitToFixed (getBIT (16, mTPL_FLAG))))
            // OLDBUFF=INPUT(7); (6986)
            {
              descriptor_t *stringRHS;
              stringRHS = INPUT (7);
              putCHARACTER (mEMIT_EXTERNALxOLDBUFF, stringRHS);
