/*
  File SET_BI_XREF.c generated by XCOM-I, 2024-08-09 12:40:41.
*/
...
SET_BI_XREF (int reset)
{
  ...
  // IF SUBSCRIPT_LEVEL>0 THEN (4823)
  if (1 & (xGT (COREHALFWORD (mSUBSCRIPT_LEVEL), 0)))
    // TAG=XREF_SUBSCR; (4824)
    {
  ...
  // ELSE (4825)
    ...
    // IF (SHR(ATOMS(LAST_POP#),24) = 1) & (LOC = SBIT_NDX)  THEN (4826)
    if (1
        & (xAND (
  ...
  if (1
      & (xNEQ (
          xAND (getFIXED (
                ...
                getFIXED (mXREF_MASK)),
          getFIXED (mCOMM + 4 * 3))))
    // BI_XREF#(LOC) = BI_XREF#(LOC)+1; (4831)
    {
  ...
  // BI_XREF(LOC)=ENTER_XREF(BI_XREF(LOC),TAG); (4832)
  {
    descriptor_t *bitRHS
        = (putFIXED (
               mENTER_XREFxROOT,
