/*
  File SET_OUTER_REF.c generated by XCOM-I, 2024-08-09 12:40:41.
*/
...
SET_OUTER_REF (int reset)
{
  ...
  // IF OUTER_REF_INDEX = OUTER_REF_LIM THEN (4812)
  if (1
      & (xEQ (COREHALFWORD (mOUTER_REF_INDEX), COREHALFWORD (mOUTER_REF_LIM))))
    // CALL COMPRESS_OUTER_REF; (4813)
    COMPRESS_OUTER_REF (0);
  // DO WHILE OUTER_REF_INDEX >= RECORD_TOP(OUTER_REF_TABLE); (4814)
  while (1
                     ...
                     1))))
    {
      ...
      rs1s1:;
        ...
        // OUTER_REF_TABLE ) + 8 ) THEN (4816)
        if (1
            & (xGE (COREWORD (xadd (
                        ADDR ("OUTER_REF_TABLE", 0x80000000, NULL, 0), 12)),
                    COREWORD (xadd (
                        ADDR ("OUTER_REF_TABLE", 0x80000000, NULL, 0), 8)))))
          // CALL _NEEDMORE_SPACE ( ADDR ( OUTER_REF_TABLE )  ) ; (4817)
          {
  ...
  // OUTER_REF_INDEX = OUTER_REF_INDEX + 1; (4819)
  {
  ...
  // OUTER_REF(OUTER_REF_INDEX) = LOC; (4820)
  {
  ...
  // OUTER_REF_FLAGS(OUTER_REF_INDEX) = SHR(FLAG, 13); (4821)
  {
  ...
  // OUTER_REF_MAX = MAX(OUTER_REF_MAX,OUTER_REF_INDEX); (4822)
  {
