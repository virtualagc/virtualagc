/*
  File _MOVE_RECS.c generated by XCOM-I, 2024-08-08 04:32:08.
*/
...
_MOVE_RECS (int reset)
{
  ...
  // IF FIRSTBLOCK ~= 0 THEN (510)
  if (1 & (xNEQ (getFIXED (mFIRSTBLOCK), 0)))
    // IF COREWORD ( FIRSTBLOCK ) = 0 THEN (511)
    if (1 & (xEQ (COREWORD (getFIXED (mFIRSTBLOCK)), 0)))
      // IF COREWORD(FIRSTBLOCK-4) >= BYTES_TO_MOVE_BY THEN (512)
      if (1
        ...
        {
        rs1:;
          // DO UNTIL CURDOPE=0; (514)
          do
            {
              // SOURCE=_DOPE_POINTER(CURDOPE); (515)
              {
                int32_t numberRHS
                    = (int32_t)(COREWORD (getFIXED (m_MOVE_RECSxCURDOPE)));
                putFIXED (m_MOVE_RECSxSOURCE, numberRHS);
              }
              // NBYTES=NBYTES+ _RECORD#BYTES(CURDOPE); (516)
              {
              ...
              // CURDOPE=_DOPE_NEXT(CURDOPE); (518)
              {
          ...
          // CALL _REDUCE_BLOCK(FIRSTBLOCK,BYTES_TO_MOVE_BY,_TRUE); (519)
          {
          ...
          // CALL _MOVE_WORDS(SOURCE,SOURCE-BYTES_TO_MOVE_BY,NBYTES); (520)
          {
  ...
  // _FREEBLOCK_SIZE(FIRSTBLOCK)); (522)
  {
    putCHARACTERp (
        m_SPMANERRxMSG,
