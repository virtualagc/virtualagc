/*
  File DUMP_VMEM_STATUS.c generated by XCOM-I, 2024-08-09 12:38:39.
*/
...
DUMP_VMEM_STATUS (int reset)
{
  ...
  // DO I = 0 TO VMEM_LIM_PAGES; (795)
  {
    ...
    for (putBIT (8, mDUMP_VMEM_STATUSxI, fixedToBit (8, from18));
                                    ...
                                    + by18)))
      {
        ...
        // IF (VMEM_FLAGS_STATUS(I) & RESV) ~= 0 THEN (797)
        if (1
        ...
        // IF (VMEM_FLAGS_STATUS(I) & MODF) ~= 0 THEN (799)
        if (1
            & (xNEQ (xAND (BYTE0 (mVMEM_FLAGS_STATUS
                                  + 1 * BYTE0 (mDUMP_VMEM_STATUSxI)),
                           BYTE0 (mMODF)),
                     0)))
          // FLAGS(I) = FLAGS(I) || ' MODF'; (800)
          {
            descriptor_t *stringRHS;
            stringRHS
        ...
        // IF (VMEM_FLAGS_STATUS(I) & RELS) ~= 0 THEN (801)
        if (1
            & (xNEQ (xAND (BYTE0 (mVMEM_FLAGS_STATUS
                                  + 1 * BYTE0 (mDUMP_VMEM_STATUSxI)),
                           BYTE0 (mRELS)),
                     0)))
          // FLAGS(I) = FLAGS(I) || ' RELS'; (802)
          {
            descriptor_t *stringRHS;
            stringRHS
                = xsCAT (getCHARACTER (mDUMP_VMEM_STATUSxFLAGS
  ...
  // OUTPUT = 'POINTERS IN CORE:     FLAGS:'; (805)
  {
    descriptor_t *stringRHS;
