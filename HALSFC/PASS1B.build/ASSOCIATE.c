/*
  File ASSOCIATE.c generated by XCOM-I, 2024-08-09 12:40:41.
*/
...
ASSOCIATE (int reset)
{
  ...
  // IF (SYT_FLAGS(I) & READ_ACCESS_FLAG) ~= 0 THEN (9889)
  if (1
      & (xNEQ (xAND (getFIXED (getFIXED (mSYM_TAB)
                               + 34 * (getFIXED (mASSOCIATExI)) + 8 + 4 * (0)),
                     536870912),
               0)))
    // CALL ERROR(CLASS_PS, 9, VAR(MP)); (9890)
    {
  ...
  // IF (SYT_FLAGS(I) & LOCK_FLAG) ~= 0 THEN (9891)
  if (1
      & (xNEQ (xAND (getFIXED (getFIXED (mSYM_TAB)
                               + 34 * (getFIXED (mASSOCIATExI)) + 8 + 4 * (0)),
                     1),
               0)))
    // IF ~ASSIGN_ARG_LIST THEN (9892)
    if (1 & (xNOT (BYTE0 (mASSIGN_ARG_LIST))))
      // IF UPDATE_BLOCK_LEVEL <= 0 THEN (9893)
      if (1 & (xLE (COREHALFWORD (mUPDATE_BLOCK_LEVEL), 0)))
        // CALL ERROR(CLASS_UI, 1, VAR(MP)); (9894)
        {
  ...
  // IF J>TEMPL_NAME THEN (9897)
  if (1 & (xGT (getFIXED (mASSOCIATExJ), 62)))
    // GO TO ARR_STRUC_CHECK; (9898)
      ...
      if (1 & (xGE (getFIXED (mASSOCIATExK), 3)))
        ...
        if (1 & (xLE (getFIXED (mASSOCIATExK), 4)))
          ...
          if (1
            ...
            // CALL ERROR(CLASS_SV, 2, VAR(MP)); (9908)
            {
...
ARR_STRUC_CHECK:
  ...
  // IF DELAY_CONTEXT_CHECK THEN (9914)
  if (1 & (bitToFixed (getBIT (1, mDELAY_CONTEXT_CHECK))))
    // IF SUBSCRIPT_LEVEL = 0 THEN (9915)
    if (1 & (xEQ (COREHALFWORD (mSUBSCRIPT_LEVEL), 0)))
      // DO; (9916)
      {
      rs2:;
        // CALL SAVE_ARRAYNESS; (9917)
        SAVE_ARRAYNESS (0);
        // IF (J& 518)= 514 THEN (9918)
        if (1 & (xEQ (xAND (getFIXED (mASSOCIATExJ), 518), 514)))
          // DO; (9919)
          {
          rs2s1:;
            // AS_PTR=AS_PTR-1; (9920)
            {
              int32_t numberRHS
            ...
            // EXT_P(PTR(MP))=ARRAYNESS_STACK(AS_PTR); (9921)
            {
              descriptor_t *bitRHS
            ...
            if (1 & (xGE (COREHALFWORD (mASSOCIATExTAG), 0)))
              // CALL HALMAT_FIX_POPTAG(TAG,1); (9923)
              {
            ...
            // ARRAYNESS_STACK(AS_PTR)=ARRAYNESS_STACK(AS_PTR+1)-1; (9925)
            {
  ...
  if (1 & (getFIXED (mASSOCIATExJ)))
    ...
    {
    rs3:;
      // GRAMMAR_FLAGS(K) = GRAMMAR_FLAGS(K) | LEFT_BRACKET_FLAG; (9932)
      {
        int32_t numberRHS = (int32_t)(xOR (
      ...
      // GRAMMAR_FLAGS(L) = GRAMMAR_FLAGS(L) | RIGHT_BRACKET_FLAG; (9933)
      {
        int32_t numberRHS = (int32_t)(xOR (
  ...
  // IF SHR(J,1) THEN (9934)
  if (1 & (SHR (getFIXED (mASSOCIATExJ), 1)))
    // DO; (9935)
    {
    rs4:;
      // GRAMMAR_FLAGS(K) = GRAMMAR_FLAGS(K) | LEFT_BRACE_FLAG; (9936)
      {
        int32_t numberRHS = (int32_t)(xOR (
            COREHALFWORD (mGRAMMAR_FLAGS + 2 * getFIXED (mASSOCIATExK)),
      ...
      // GRAMMAR_FLAGS(L) = GRAMMAR_FLAGS(L) | RIGHT_BRACE_FLAG; (9937)
      {
        int32_t numberRHS = (int32_t)(xOR (
            COREHALFWORD (mGRAMMAR_FLAGS + 2 * getFIXED (mASSOCIATExL)),
  ...
  // IF SHR(J,13) THEN (9939)
  if (1 & (SHR (getFIXED (mASSOCIATExJ), 13)))
    // VAL_P(PTR(MP))=J| 16; (9940)
    {
