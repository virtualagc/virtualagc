/*
  File SCANxID_LOOP.c generated by XCOM-I, 2024-08-09 12:40:41.
*/
...
SCANxID_LOOP (int reset)
{
  ...
  // IF LENGTH(BCD) < ID_LIMIT THEN (6763)
  if (1 & (xLT (LENGTH (getCHARACTER (mBCD)), 32)))
    ...
    rs1:;
      // CALL BUILD_BCD; (6765)
      SCANxBUILD_BCD (0);
      // IF OVER_PUNCH ~= 0 THEN (6766)
      if (1 & (xNEQ (BYTE0 (mOVER_PUNCH), 0)))
        // DO; (6767)
        {
        rs1s1:;
          // IF IMPLIED_TYPE > 0 THEN (6768)
          if (1 & (xGT (COREHALFWORD (mIMPLIED_TYPE), 0)))
            // CALL ERROR(CLASS_MO,3); (6769)
            {
          ...
          // ELSE (6770)
            ...
            {
            rs1s1s1:;
              // DO I = 1 TO OVER_PUNCH_SIZE; (6772)
              {
                ...
                for (putFIXED (mI, from105); getFIXED (mI) <= to105;
                  ...
                  {
                    // IF OVER_PUNCH = OVER_PUNCH_TYPE(I) THEN (6773)
                    if (1
                        & (xEQ (
                            BYTE0 (mOVER_PUNCH),
