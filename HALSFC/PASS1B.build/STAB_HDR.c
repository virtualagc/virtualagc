/*
  File STAB_HDR.c generated by XCOM-I, 2024-08-09 12:40:41.
*/
...
STAB_HDR (int reset)
{
  ...
  // IF ADDR_PRESENT THEN (7052)
  if (1 & (bitToFixed (getBIT (1, mADDR_PRESENT))))
    // CELLSIZE=CELLSIZE+16; (7053)
    {
      int32_t numberRHS
  ...
  // IF SRN_PRESENT THEN (7057)
  if (1 & (bitToFixed (getBIT (1, mSRN_PRESENT))))
    // IF SRN_COUNT(2) > 0 THEN (7058)
    if (1 & (xGT (COREHALFWORD (mSRN_COUNT + 2 * 2), 0)))
      // CELLSIZE = CELLSIZE + 17; (7059)
      {
        int32_t numberRHS
  ...
  // CELL_PTR = GET_CELL(CELLSIZE,ADDR(NODE_H),RESV+MODF); (7062)
  {
  ...
  if (1 & (bitToFixed (getBIT (1, mHMAT_OPT))))
    ...
    {
    rs1:;
      // CELL_PTR = CELL_PTR+4; (7065)
      {
        int32_t numberRHS = (int32_t)(xadd (getFIXED (mSTAB_HDRxCELL_PTR), 4));
        putFIXED (mSTAB_HDRxCELL_PTR, numberRHS);
      }
      // COREWORD(ADDR(NODE_H))=COREWORD(ADDR(NODE_H))+4; (7066)
      {
  ...
  // IF LAST_STAB_CELL_PTR ~= -1 THEN (7071)
  if (1 & (xNEQ (getFIXED (mLAST_STAB_CELL_PTR), -1)))
    // DO; (7072)
    {
    rs3:;
      // CALL LOCATE(LAST_STAB_CELL_PTR,ADDR(NODE_F),MODF); (7073)
      {
  ...
  // NODE_F(1) = LAST_STAB_CELL_PTR; (7079)
  {
...
// NODE_H(SHR(SRN_INX,1)+4) = SRN_COUNT(2); (7091)
{
...
// NODE_H(13) = NODE_H(13) |  32768; (7100)
{
...
// CELLSIZE = STAB2_STACKTOP - STAB2_MARK; (7101)
{
...
// IF CELLSIZE > 0 THEN (7102)
if (1 & (xGT (COREHALFWORD (mSTAB_HDRxCELLSIZE), 0)))
  // DO; (7103)
  {
  rs5:;
    // NODE_F(2) = GET_CELL(SHL(CELLSIZE,1)+2,ADDR(NODE_H),MODF); (7104)
    {
      int32_t numberRHS = (int32_t)((
    ...
    // NODE_H(0) = CELLSIZE; (7105)
    {
      descriptor_t *bitRHS = getBIT (16, mSTAB_HDRxCELLSIZE);
      putBIT (16, getFIXED (mSTAB_HDRxNODE_H) + 2 * (0), bitRHS);
      bitRHS->inUse = 0;
    }
    // DO I = STAB2_MARK + 1 TO STAB2_STACKTOP; (7106)
    {
...
// CELLSIZE = STAB_STACKTOP - STAB_MARK; (7111)
{
...
// IF CELLSIZE > 0 THEN (7112)
if (1 & (xGT (COREHALFWORD (mSTAB_HDRxCELLSIZE), 0)))
  // DO; (7113)
  {
  rs6:;
    // NODE_F(3) = GET_CELL(SHL(CELLSIZE,1)+2,ADDR(NODE_H),MODF); (7114)
    {
      int32_t numberRHS = (int32_t)((
          putFIXED (mGET_CELLxCELL_SIZE,
                    xadd (SHL (COREHALFWORD (mSTAB_HDRxCELLSIZE), 1), 2)),
    ...
    // NODE_H(0) = CELLSIZE; (7115)
    {
      descriptor_t *bitRHS = getBIT (16, mSTAB_HDRxCELLSIZE);
      putBIT (16, getFIXED (mSTAB_HDRxNODE_H) + 2 * (0), bitRHS);
      bitRHS->inUse = 0;
    }
    // DO I = STAB_MARK+1 TO STAB_STACKTOP; (7116)
    {
