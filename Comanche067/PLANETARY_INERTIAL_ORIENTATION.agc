...
##		2020-12-28 RSB	Implemented PCR 738.1.
...
## (<code>WEARTH</code>) vs radians/second (&omega;<sub>E</sub>).  In point of fact, Apollo 13 used values from the
## same desired epoch as Apollo 12, so the Apollo 13 code already has the corrected values, in the proper
## units &mdash; which is very fortunate, since some of the values in the Software Anomaly Report
## are illegible, as for example the value of F<sub>0</sub> (<code>FSUB0</code>), which the report quotes as
## "F<sub>0</sub> = 5.209<u>&nbsp;&nbsp;&nbsp;</u>056 RAD".  It is only because we have the Apollo 13 code that can deduce
## that the report meant "5.209<u>327</u>056".  At any rate, the block of assignments below was simply copied from Luminary 131
## (initial release of Apollo 13 LM code, CONTROLLED CONSTANTS log section).  For simplicity's sake, the constant 
## <code>BSUB0</code> was copied along with the other constants, even though it is not referenced by the Software Anomaly
## Report; its numerical value, however, remains unchanged, as has been verified by inspecting the 
## octal values generated by assembling the program.
