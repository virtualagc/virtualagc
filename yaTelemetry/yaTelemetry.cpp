...
/*
  ...
  Mods:         2009-03-09 RSB. Began.
                ...
                2022-07-17 RSB  Window made resizable to get around some
                                wxWidgets 3.2 sizing issues with the "normal"
                                ("simple") UI.  It didn't actually help, but
                                I've left it in place anyway.  I managed to
                                kludge in a fix by other means ... I hope.
                                As far as the "retro" ("main") UI is concerned,
                                I've gotten rid of the assertion error that
                                was appearing with wxWidgets 3.2 simply by
                                having --undecorated always be in effect if
                                --simple isn't selected.  I may reverse this
  ...
  (Actually, I stopped using wxGlade for any of this a long, long time ago, and
  I'm sure it would be completely unusable now.  I should go through the entire
  source tree and get rid of referenced to it.)
*/
...
SimpleFrameClass::ClearScreen (void)
{
  ...
  //TextCtrl->FitInside ();
  //InvalidateBestSize ();
  //TextCtrl->SetInitialSize(TextCtrl->GetSizeFromTextSize(TextCtrl->GetTextExtent(EmptyScreen)));
  ...
  if (firstTimeIO && Simple)
    {
      ...
      SimpleFrame->SetPosition(wxPoint(1066-size.GetWidth(), 10));
    }
  firstTimeIO = false;
...
bool yaTelemetryApp::OnInit()
{
    ...
    for (i = 1; i < argc; i++)
      {
        ...
        if (Arg.IsSameAs (wxT ("--help")))
          ...
          Help:
            ...
            printf ("                     selected.\n");
            printf ("     --undecorated   This option is now always in effect and there's no\n");
            printf ("                     way to disable it.\n");
    ...
    if (Simple)
    ...
    
      {
        ...
        MainFrame->Show();
        MainFrame->SetPosition(wxPoint(1066-MainFrame->GetSize().GetWidth(), 10));
      }
    return true;
}
