...
##		2017-06-06 RSB	Comment-text fixes (mostly whitespace) identified 
##				in transcribing Sunburst 37.  Curiously, the label
##				ENDVBFAN on p. 253 was missing, and I've added it, but
##				it turns out not to have be referenced anywhere else
##				in the code, and therefore its absence had no effect
##				on the generated octals.
##		2017-06-07 RSB	More of the same.
...
# OCTAL NUMBER.  THE VERB CODE INDICATES WHAT ACTION IS TO BE TAKEN, 
...
# CODE IN A (LOW 6 BITS FOR NOUN, NEXT 6 BITS FOR VERB).  DETAILS
# DESCRIBED ON REMARKS CARDS JUST BEFORE :NVSUB: AND :NVSUBWAIT: (SEE
...
# DESCRIBED ON REMARKS JUST BEFORE :GRABDSP:, :GRABWAIT: (SEE SYMBOL TABLE
# FOR PAGE NUMBERS).
...
## The operand in the following line reads simply "E-5 B14" in the program
## listing, but has been changed to the (numerically identical) "1 E-5 B14"
## to conformed to syntax rules of the "modern" assembler, yaYUL.  &mdash; RSB	
...
SIGNTEST	LXCH	Q		# ALLOWS +,- ONLY WHEN DSPCOUNT=R1D1,
		...
		MASK	DECBRNCH	# CONSEQUTIVE +/- CHARACTERS.
		CCS	A		# IF LOW 2 BITS OF DECBRNCH NOT = 0, SIGN
...
## The following line is visibly terminated by a period ('.') in the
## scanned program listing, but in all other currently available AGC
## program versions containing this line (namely, RETREAD, AURORA, and
## SUNBURST 37) it is terminated by a comma (',').  My belief is
## that the comma's tail has simply been cut off due to poor print
## quality, making it look like a period. &mdash; RSB
...
SHOTIME		DEC	175		# SET FOR 1.75 SEC.
		SETLOC	ENDNVBSY +1
...
TSTLTS3		CS	TSTCON3		# CALLED BY EXECUTIVE
		...
		CADR	VBTERM		# IF APPROPRIATE).
					...
					# FOR TERMINATE CONDITION, AND TO TO
