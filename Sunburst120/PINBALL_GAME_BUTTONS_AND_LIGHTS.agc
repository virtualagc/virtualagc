...
#		2016-10-08 RSB	Finished transcription.
...
# :NVSUB: AND :NVSUBWAIT: (SEE SYMBOL TABLE FOR PAGE NUMBERS).
...
#	(WITH OCTAL 01501) AND EXIT TO :ENDOFJOB:.

# CONDITIONS LEADING TO THE ABOVE ARE DESCRIBED IN DIGITAL DEVELOPMENT
# REPORT NO. 11.
...
# REQRET, CLPASS, DSPLOCK, MONSAVE, MONSAVE1, GRABLOCK, VERBREG, NOUNREG, 
# DSPLIST, +1, +2, DSPCOUNT.
...
#	GRABDSP   INTERLOCK SYSTEM FOR INTERNAL PROGRAMS THAT MUST BE USED
#	FREEDSP   IN CONJUNCTION WITH :NVSUB:.
#		    OF RELATED INTEREST   GRABWAIT
#					  GRABUSY
#					  PREGBSY

#	ENDIDLE	  ROUTINE FOR INTERNAL PROGRAMS WISHING TO TO SLEEP WHILE
#		  AWAITING OPERATORS RESPONSE.
#		    OF RELATED INTEREST   DATAWAIT
...
#	THE NOUN TABLES THEMSELVES ARE FOUND ON 7 PAGES STARTING AT :NNADTAB:


# FOR FURTHER DETAILS ABOUT OPERATION OF THE KEYBOARD AND DISPLAY SYSTEM
# PROGRAM, SEE THE MISSION PLAN AND/OR DIGITAL DEVELOPMENT REPORT NO. 11.
...
## <small>&mdash;Ron Burkey, 10/2016</small>
...
CHARIN		CAF	ONE		# BLOCK DISPLAY SYST
		...
		TC	ABRT23GO	# 23		SUBORBIT ABORT
		TC	ABRT24GO	# 24		CONORBIT ABORT
		TC	CHARALRM	# 25
		TC	CHARALRM	# 26
		TC	CHARALRM	# 27
		TC	CHARALRM	# 30
...
# IT ALSO ZEROES OUT THE :TEST ALARM: OUT BIT, WHICH TURNS OFF STBY,RESTART.
...
ERROR		XCH	21/22REG	# RESTORE ORIGINAL C(DSPLOCK). THUS ERROR
		...
		TS	DSPTAB	+11D
# Page 246
		CS	PRIO16		# RESET FAIL BITS WHICH GENERAGE PROG
...
LOWVERB		DEC	30		# LOWER VERB THAT AVOIDS NOUN TEST.
...
INTMCTBS	CA	MPAC	+2	# INTERNAL MACH CADR TO BE SPECIFIED.
		...
		AD	NET5		# MASKING MAKES VB15 LOOK LIKE VB05.
		EXTEND
...
DP3OUTSF	TC	DPOUT		# ASSUMES POINT BETWEEN BITS 7-8 OF HIGH
		TC	TPLEFTN		# PART OF SFCON. SHIFTS RESULTS LEFT 7.
		TC	SCOUTEND
...
OPTDEGIN	CCS	MPAC		# OPTICS SCALING ROUTINE
		TC	+4
		TC	+3
		CS	DSPALARM	# REJECT NEGATIVE INPUT
		TC	DSPALARM	#        DITTO
...
# ANY NVSUB CALL THAT PASSES THE DSPLOCK, OR OTHER MONITOR.
...
MONDO		CCS	MONSAVE1	# CALLED BY EXEC
		...
		MASK	MONSAVE
		TS	NVTEMP
		TC	NVSURMON	# PLACE NOUN INTO NOUNREG AND DISPLAY IT
...
CHARALRM	CAF	ENDINST		# ALARMS WHICH MUST DO ENDOFJOBS COME
		TS	ENDRET		# HERE. ALLOWS ENDRET TO BE TEMP ERASABLE
...
# VBRELDSP TURNS OFF RELEASE DISPLAY SYSTEM LIGHT(AND SERCHES LIST ONLY
...
#      IT IS RELEASED BY SPECIAL VERB = RELEASE DISPLAY, ALL TO TO VERBS,
# PROCED WITHOUT DATA, TERMINATE, INITIALIZE EXECUTIVE,
...
# Page 289
		SETLOC	NVSUBEND +2
KILMONON	CAF	BIT15		# FORCE BIT 15 OF MONSAVE1 TO 1.
...
# ENDIDLE CANNOT BE CALLED FROM ERASABLE, SINCE JOB SLEEP AND 
...
# AND JOBWAK CAN HANDLE ONLY FIXED MEMORY.
...
# Page 290
		...
		TCF	Q+2		# DATA IN. RETURN TO L+3
DATWAIT1	=	Q+1		# RETURN TO L+2
...
RECAL2		INDEX	LOCCTR
		...
		TS	LOC
		RELINT
		TC	RELDS1		# DOES NOT SEARCH LIST
		TC	ENDOFJOB
...
#
# 00000				WHOLE
# 00000				TIME SEC(XXX.XX SEC)SAME AS WHOLE (ARITH DP1)
...
#							    USE ARITHDP1
# 01101				RENDEZVOUS RADAR RANGE RATE(XXXXX.FEET/SEC
#				 			    USE ARITHDP1
...
#							       USE ARITH
# 10010				ROTATIONAL HAND CONTROL ANGLE RATES
...
LODNLV		XCH	IDAD2TEM	# PUT RETURN ADDESS INTO A, L.
		DXCH	Z
...
NNADTAB		OCT	00000		# 00 NOT IN USE
		...
		ECADR	DSPTEM1		# 26 PRIO/DELAY
		ECADR	SMODE		# 27 SELF TEST ON/OFF SWITCH
		ECADR	DSPTEM1		# 30 STAR NUMBERS
		ECADR	ALMCADR		# 31 ALMCADR, ALMCADR +1, ERCOUNT
...
# Page 296
		...
		OCT	TANG		# 41 NEW REND. RADAR ANGLES (TRUN, SHAFT)
		OCT	DSPTEM2		# 42 AOT ROTATION ANGLES
		OCT	XYMARK		# 43 AOT DETENT CODE
		OCT	FORVEL		# 44 FORWARD VELOCITY, LATERAL VELOCITY
		OCT	PCOM		# 45 ROTATIONAL HAND CONTROLLER ANGLE RATE
		OCT	00000		# 46 SPARE
		OCT	00000		# 47 SPARE
		OCT	FAILREG		# 50 FAILREG, FAILREG +1, FAILREG +2.
		OCT	00000		# 51 SPARE
		...
		OCT	02015		# 63 REND. RADAR RANGE RATE, TRUN, SHAFT
...
# Page 297		
		...
		OCT	04062		# 45 3COMP  ROT HAND CONT ANGLE RATE
		OCT	00000		# 46        SPARE
		OCT	00000		# 47        SPARE
		OCT	04000		# 50 3COMP  OCTAL ONLY
		OCT	00000		# 51        SPARE
...
SFINTAB1	OCT	00006		# WHOLE,TIME(SEC)
		OCT	03240
...
SFOUTAB		OCT	05174		# WHOLE, TIME(SEC)
		...
		OCT	35354
		OCT	03225		# RENDEZFOUS RADAR RANGE RATE
		OCT	07470
...
IDADDTAB	ECADR	RSTACK	+18D	# 01	   LANDING RADAR ALTITUDE
		...
		ECADR	RSTACK	+12D	# 04	   LANDING RADAR VELZ
		...
		ECADR	FINALALT	# 10	   INITIAL/FINAL ALTITUDE
...
CHRPRIO		=	PRIO30		# EXEC PRIORITY OF CHARIN
...
TPSL1		EXTEND			# SHIFTS MPAC, +1, +2 LEFT 1
		...
		TC	Q		# NO NET OF/UF
		TS	OVFIND		# MPAC +6 SET TO +/- 1 FOR OF/UF
		TC	Q
...
#       WHEN FINISHED , THERE MUST BE A FREEDSP , TO RELEAS THE
# SYSTEM FOR OTHER INTERNAL USERS.

#       THE CALLING SEQUENCES ARE
# L	   TC     GRABDSP
# L+1	   RETURN HERE WHEN THE SYSTEM IS ALREADY GRABBED
# L+2	   RETURN HERE MEANS YOU HAVE IT

# L        TC	  NVSUB
# L+1	   RETURN HERE IF THE OPERATOR HAS INTERVENED
...
# AFTER A TC FREEDSP, THE INTERNAL INTERLOCK IS KEPT BUSY FOR 1.75 SECS.
...
WKSEARCH	CAF	ZERO		# SEARCHES LIST. LEAVES RESULT IN A.
		...
		XCH	DSPLIST	+2
		EXTEND
		BZF	TCQ		# EMPTY, RETURN TO L+1
		INHINT			# NOT EMPTY
		TCF	Q+1		# RETURN TO L+2
...
FREDSPDO	TC	WKSEARCH	# CALLED BY EXECUTIVE
...
# Page 311
TSTCON2		OCT	40650		# DSPTAB+11D BITS 3,4,6,8,9.
