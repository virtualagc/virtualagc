...
// the correct one for all Block 2 AGC software, as far as I know
// (even the BLK2 target).
  ...
  if (!InterpretersSorted)
    {
      ...
      qsort (InterpreterOpcodes, NUM_INTERPRETERS,
      ...
      int *Warnings)
{
  ...
  if (!BlockAssigned && blk2)
    {
      // YUL target BLK2, I think.  Only the interpreter
      // codes differ from the default.
      ...
      if (NumFields >= 1)
	{
	  ...
	  if (Block1)
	      ...
	      else if (iMatch)
		{
		  ...
		  j = atoi (Fields[i]);
		  if (isdigit (Fields[i][0]) && strlen (Fields[i]) == 1
		      && j >= 0 && j <= 7)
		    {
...
//#endif
	  ...
	  && strcmp (ParseInputRecord.Operator, "CHECK="))
	{
	  ...
	  EditSymbolNew (ParseInputRecord.Label,
			 &ParseInputRecord.ProgramCounter, Type,
			 CurrentFilename, CurrentLineInFile);
	}
