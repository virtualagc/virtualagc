...
main (int argc, char *argv[])
{
  ...
  for (i = 1; i < argc; i++)
      ...
      else if (!strcmp (argv[i], "--syntax"))
	{
	  ...
	  == sscanf (argv[i], "--to-yul=%d,%[^\n]", &j, toYulOnlyLogSection))
	{
	  toYulOnly = 1;
	  toYulOnlySequenceNumber = j;
	}
      else if (!strncmp (argv[i], "--bailout=", 10))
	bailout = &argv[i][10];
  ...
  if (Fatals == 0 || Force)
	  ...
	  BankRaw++)
	{
	      ...
	      if (Value > 0)
		{
		  if (!Block1 && !blk2)
		      ...
		      if (Value < 01776)
			{
			  ...
			  Parities[Bank][Value] = CalculateParity (
			      Value + Offset);
			  Value++;
			}
		      if (Value < 01777)
	  ...
	  if (bailout != NULL)
	    {
	      Symbol_t *bailoutSymbol;
	      Address_t *address;
	      bailoutSymbol = GetSymbol(bailout);
	      if (bailoutSymbol == NULL)
		{
		  printf("Specified BAILOUT function (\"%s\") not found.\n", bailout);
		  goto Done;
		}
	      address = &bailoutSymbol->Value;
	      if (!address->Fixed || !address->Unbanked)
		{
		  printf("BAILOUT function (\"%s\") not in fixed-fixed memory.\n", bailout);
		  goto Done;
		}
	      ObjectCode[3][01776] = address->SReg;
	      Parities[3][01776] = CalculateParity(address->SReg);
	    }
	  ...
	  for (Offset = 0; Offset < 02000; Offset++)
		...
		Value = (Value & 0100000) | (Parities[Bank][Offset] << 14)
		    | ((Value & 077776) >> 1);
	      else if (Parity)
  ...
  if (RetVal)
    {
      ...
      printf ("                 YUL superbank behavior.\n");
      printf (
	  "--raytheon       Assembles Raytheon-style code.  The default is MIT.\n");
      printf (
	  "--no-checksums   Don't emit bank checksums. For use with Retread 44.\n");
      printf ("--parity         Enable parity bit calculation.\n");
      printf (
	  "--hardware       Emit binary with hardware bank order. Also implies\n"
	  "                 --parity.\n");
      printf (
      ...
      printf (
	  "--yul            Assemble as YUL rather than GAP.  Has no effect at present.\n");
      printf (
	  "--trace          Trace some of yaYUL's internal activity, for debugging.\n");
      printf (
	  "--to-yul=S,L     Processes a single input file in .agc format, outputting\n");
      printf (
	  "                 an equivalent .yul file on stdout.  S (a decimal number\n");
      printf (
	  "                 is the initial card-sequence number.  L (a string) is the\n");
      printf ("                 name of the log section to use as a P-card.\n");
      printf (
	  "--bailout=B      Allows specification of the name of the AGC bailout\n");
      printf (
	  "                 routine, usually BAILOUT or BAILOUT1, used in conjunction\n");
      printf (
	  "                 with 'fictitious' input channel 162.  Not needed unless\n");
      printf (
	  "                 channel 162 will be used for certain demo purposes.\n");
    }
