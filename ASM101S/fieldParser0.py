...
As a module, it:
    ...
    2.  Exposes the parsers to the importing program as the functions:
                ...
                `parserOperandInvocation0`    Called by `joinOperand`
                `parserOperandPrototype`      Operand of a macro prototype
                `parserOperandPrototype0`     Called by `joinOperand`
                ...
                `parserQuotedString`          A single quoted string only.
                (etc.)
...
if standAlone:
    ...
    LEAD_IN = '''
        ...
        @@whitespace :: None
    '''
    def compile(grammar, prefix):
        grammar = LEAD_IN.replace("@n@", prefix) + grammar
        name = None
        lines = grammar.split('\n')
        for line in lines:
            if "@@grammar" in line:
    ...
    ARITHMETIC_EXPRESSION = '''
        arithmeticExpression = exp+: ( term  { ( '+' | '-' ) term } ) ;
    ...
    NAME_CODE = '''
        nameCode = 
            ...
            | id+: identifier
            ;
    ''' + ARITHMETIC_EXPRESSION + VARIABLE + IDENTIFIER + CONSTANT
    ...
    NAME_MACRODEF = '''
        ...
        subName = 
            ...
            | identifier
            ;
    ''' + ARITHMETIC_EXPRESSION + VARIABLE + IDENTIFIER + CONSTANT + SEQUENCE_SYMBOL
    ...
    ''' + REGISTER + IDENTIFIER + VARIABLE + CONSTANT + ARITHMETIC_EXPRESSION
    ...
    RS_ALL = '''
        rsAll = 
            ...
            ;
    ''' + ARITHMETIC_EXPRESSION + VARIABLE + IDENTIFIER + CONSTANT + REGISTER
    
    # Operand field for an RI instruction.
    RI_ALL = '''
        riAll = register ',' immediate $ ;
    ''' + IMMEDIATE + REGISTER + IDENTIFIER + VARIABLE + CONSTANT + ARITHMETIC_EXPRESSION
    ...
    ''' + ARITHMETIC_EXPRESSION + VARIABLE + IDENTIFIER + CONSTANT + IMMEDIATE + REGISTER
    ...
    # much of the input string was parsed.  The use of `endX+:` 
    ...
    ''' + ARITHMETIC_EXPRESSION + REPLACEMENT + IDENTIFIER + VARIABLE + \
          CONSTANT + LIST + CHAR + QUOTED_STRING + SUBSTRING_EXPRESSION
    ...
    AIF_CODE = '''
        operand = '(' booleanExpression ')' sequenceSymbol ;
    ''' + BOOLEAN_EXPRESSION + SEQUENCE_SYMBOL + \
          ARITHMETIC_EXPRESSION + CHAR + VARIABLE + \
          QUOTED_STRING + SUBSTRING_EXPRESSION
    
    # Create parsers from the grammars.
    parserAifCode = compile(AIF_CODE, "aifCode")
    parserBceAll = compile(BCE_ALL, "bceAll")
    parserMscAll = compile(MSC_ALL, "mscAll")
    parserNameCode = compile(NAME_CODE, "nameCode")
    parserNameMacrodef = compile(NAME_MACRODEF, "nameMacrodef")
    parserOperationAll = compile(OPERATION_ALL, "operationAll")
    parserOperandInvocation = compile(OPERAND_INVOCATION, "operandInvocation")
    parserOperandInvocation0 = compile(OPERAND_INVOCATION_0, "operandInvocation0")
    parserOperandPrototype = compile(OPERAND_PROTOTYPE, "operandPrototype")
    parserOperandPrototype0 = compile(OPERAND_PROTOTYPE_0, "operandPrototpe0")
    parserQuotedString = compile(QUOTED_STRING, "quotedString")
    parserRiAll = compile(RI_ALL, "riAll")
    parserRrAll = compile(RR_ALL, "rrAll")
    parserRsAll = compile(RS_ALL, "rsAll")
    parserSiAll = compile(SI_ALL, "siAll")
    parserSrsAll = compile(SRS_ALL, "srsAll")

if not standAlone or not generateLocal: 
    if standAlone:
        print("Importing parsers ...")
    import parser_aifcode
    parserAifCode = parser_aifcode.aifCodeParser()
    ...
    import parser_quotedstring
    parserQuotedString = parser_quotedstring.quotedStringParser()
    import parser_riall
...
if __name__ == "__main__":
    ...
    while True:
        ...
        exercise("QUOTED_STRING",parserQuotedString)
        exercise("AIF_CODE", parserAifCode)
