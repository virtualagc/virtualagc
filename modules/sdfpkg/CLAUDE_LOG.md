...
### [2026-08-04] Target: [HANDOFF.md]
- Template dependency cycles ARE breakable, and the workaround needs no source change.  PASS1 takes TEMPLIB as both input (--pdsi=4) and output (--pdso=6), but emits NO template when the compile fails, so a failing unit cannot seed itself.  A stub carrying only the block statement can: for VM4BFSHU a two-line stub compiled clean, deposited @@VM4BFS, after which VM1BFDCY compiled (exit 0, never once attempted before) and VM4BFSHU recompiled for real over the stub (exit 0).  Cycle closed, nothing synthetic left in TEMPLIB.  See cycle-bootstrap-test.sh.
- All six OI340600 cycles are seedable the same cheap way: every seed candidate is parameterless, so each needs only a two-line stub.  Seeds are ARAGPCSW (cycle of 29), GGQCOM (7), V01TCSSC (5), VM4BFSHU (2), GV6STA (2), VS5SSTPR (2).  GV6STA looks parameterised only because its block statement is split across two cards.
- Naming stubs _stub*.hal is load-bearing: corpus-run.sh clears _*.hal at the start of a run, so a stub cannot leak into a later run.  Its .obj is NOT cleared -- delete objects/_stub*.obj by hand.
- compilePASS defaults --csects to ../mafgen, which resolved to ~/workspace/PFS/mafgen before the work directories moved and to nothing after, so the first run from ~/ForClaude compiled every HAL/S file: OI340600 attempted 1011 rather than 974, the extra 37 being the ones previously reported "Not part of PASS".  All 1011 succeeded.  Symlinked ~/ForClaude/mafgen at the masters; corpus-run.sh should also pass --csects explicitly, which was NOT done tonight because editing a running bash script can corrupt its execution and OI301700 was still executing it.
- Never-attempted is unchanged at 156, so the CSECT filter and the cycles are independent.  Accounting closes: 1011 attempted + 156 never = 1167.
- A killed run leaves orphaned HALSFC processes writing halmat.bin, litfile.bin and COMMON*.out into the work directory; a fresh run started on top of them aborts after FLO with exit 240 and "Unable to open COMMON input file".  That is almost certainly the CDR06D abort section 3 records as unreproducible, and it is NOT concurrency.  Check for strays by argv[0]: a pgrep -f on the command line matches the checking command itself and reads as a false positive.
