...
CORRECTION, TWICE OVER.  MENU12 does not need MACSMITH, and its position
symbols are not missing from the source.

MLIB80/MACROS.asm -- WHICH MENU12 DOES COPY, at its line 47 -- carries 51 PDEF
invocations of its own, P1 through P52, covering the P37 and P39 MENU12 uses:

    P1       PDEF    -475,337              C1,L1
    P2       PDEF    -456,310              C2,L2

MACSMITH holds a similar deck and is a red herring; nothing needs to copy it.
OI301700 shows the EXPANDED result because that release was pulled from
assembly listings with all macros already expanded (RSB):

    P1X      EQU   -475,-475+1025,C'@'      01-PDEF

a THREE-OPERAND EQU whose SECOND operand is the length attribute, so
L'P1X = -475+1025 = 550 and POS's L'&#-1025 recovers -475.  THE 1025 BIAS
EXISTS BECAUSE A LENGTH ATTRIBUTE CANNOT BE NEGATIVE, which is also why
defaulting an undefined symbol to 1 was wrong: it gives -1024 for every
coordinate.

THE DEFECT IS IN THE EXPANSION.  PDEF is found and loaded -- its prototype
'&N PDEF &X,&Y' appears in MENU12's listing -- but its body comes out
UNSUBSTITUTED, once per invocation:

    +   &N       EQU   1,1,C'#'
    +   &N.X     EQU   &X,&X+1025,C'@'
    +   &N.Y     EQU   &Y,&Y+1025,C'@'

where it should read 'P1X EQU -475,-475+1025,C''@'''.  Its .* macro comments
and its AIF are echoed as well, and NEITHER SHOULD EVER APPEAR IN AN EXPANSION:
macro comments are not generated and AIF is executed, not emitted.  So no
P<n>X or P<n>Y symbol is ever defined, and L' correctly reports them absent.

START THERE, not at the symbols.  The question is why PDEF's body is echoed
rather than expanded when hundreds of other macros expand correctly -- what is
different about it -- and the answer is upstream of anything to do with L',
POS, or MENU12 itself.
