...
THREE WRONG DIAGNOSES OF MENU12, AND WHAT THE EVIDENCE ACTUALLY SAYS.  Each
was stated with more confidence than it had earned; the measurements are below
so nobody repeats them.

NOT MISSING SOURCE.  MLIB80/MACROS.asm -- which MENU12 copies at its line 47 --
carries 51 PDEF invocations defining positions 1-50 and 52.  MENU12 needs
1,2,3,9,10,11,13,15,17,18,19,21,22,24,25,27,31,37,38,39,41,49.  All present.
MACSMITH is a red herring; nothing need copy it.

NOT A FAILED EXPANSION.  I read MENU12's listing as showing PDEF's body echoed
unsubstituted and called that the defect.  Wrong: those lines are the macro
DEFINITION being listed.  Assembling 'P1 PDEF -475,337' alone defines P1 and
P1X, with P1X = FFFFFE25 = -475, exactly right.

NOT THE C' AND NOT THE COPY.  A source consisting only of 'COPY MACROS'
defines 102 P-symbols, P1X again -475.  The three-operand EQU with C'@' expands
correctly.  RSB asked whether it was the C'; it is not.

AND MENU12 SHOWING ZERO P-SYMBOLS IS AN ARTEFACT: its assembly aborts, so no
symbol table is printed.  Counting symbols in a failed listing measures
nothing.

WHAT IS LEFT IS ORDERING.  MENU12's diagnostic is 'Symbol P37X not found for
L'' at PASS -1, macro-expansion time, while the symbols come from EQUs
processed when the assembly proper runs.  POS asks for the length attribute
DURING EXPANSION, before the symbol table holds it.  Everything measured is
consistent with that and nothing contradicts it.

THE DECISIVE TEST, not yet run and cheap: assemble a source that does
COPY MACROS and then one POS invocation.  If L' resolves there, the ordering
theory is wrong and something specific to MENU12 remains.  If it fails with the
symbols demonstrably defined, the theory holds and the fix is about WHEN L' is
evaluated -- deferring it to a pass that has a symbol table -- not about L'
itself, which is now implemented correctly.
