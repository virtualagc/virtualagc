...
THIS IS NOT A MACROFILES.txt DEFECT, and an earlier draft of this entry saying
so was wrong.  MACSMITH is a COPY deck, not a macro definition, and the index
classifies it correctly by commenting it out.  The gap is simpler: MENU12 does
COPY MACROS, MACROS does not copy MACSMITH, and only SSSRC/BILDNEW5.asm copies
MACSMITH at all.  So nothing ever expands those PDEF invocations for MENU12.

WHAT TO ESTABLISH NEXT is how the original build gave MENU12 those symbols --
whether a COPY is missing from the source we hold, or whether MACSMITH was
assembled into every module by the build procedure rather than by a COPY
statement.  OI301700 cannot answer it directly, its macros already being
expanded, but its MENU-family modules may show whether the symbols arrived by
COPY.  DO NOT add a COPY to MENU12 to make it assemble; that would be inventing
source.
