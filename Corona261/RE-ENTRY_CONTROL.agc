...
## 		2025-05-03 MAS  Incorporated original comments and label names
##				found in R-532 Volume I, Reentry Guidance for
##				Apollo.
...
GETVCOR		DSU	1
		DDV	BDDV		#  VCORR = VCORR(DIFF/(DIFFOLD-DIFF))
			DIFFOLD
			DIFF
			VCORR
			DIFF
...
NEGAMA		NOLOD	1		# ENTER WITH GAMMAL IN  MPAC
		...
		BDSU	BDDV
			...
			DHOOK
			VL
			LEWD/3		# OUT OF AND BACK INTO PD.
...
NEGTESTS	NOLOD	3		# IF L/D NEG, AND D-C20 POS, L/D = 0
		BPL	LODON
		DSU	BMN
		SWITCH	DMP
			LIMITL/D
...
KEP2		DAD	1		# IF Q7+KDMIN - D NEG, GO TO FINAL PHASE
		DSU	BMN
...
GLIMITER	DSU	2		# IF GMAX/2-D POS, GO TO LIMITL/D
		...
		ROUND			# L/D = (RDOT+XLIM+KGLIM1 LEQ)/(KGLIM2 D
			2HSGMXSQ
			VSQUARE
			-
...
STEER		TEST	1
		EXIT
			LATSW
			L380
STEER2		CS	ROLLC		# SCALE ROLL OUTPUT FOR 16 SPEED.
		EXTEND
		MP	NEG1/8		# ROLL = ROLLC/8  + K1ROLL
		AD	K1ROLL
...
UPTHETA		ITC	0		# FIND DESIRED SPACECRAFT ORIENTATION.
		...
		COM
		DOUBLE
		TS	THETAD +1	# COMMAND PITCH ANGLE
		
		CS	MGC
		COM
		DOUBLE			# CHANGE FORM INTERP SCALING TO CDU.
		TS	THETAD +2	# COMMAND YAW ANGLE
...
LEWD		2DEC	.2
LEWD/3		2DEC	.066666666	# LEWD/3   (LEWD = .2)
FOURTH		2DEC	.25
3ZEROS		2DEC	0
...
2HSD/LWD	2DEC	.086393304 	# 2(1.0) 28500 805/ .2 (2VS) SQ
...
KAFIX		2DEC	0.0074534161	# 6/805 (2 G-S)
...
LAD/KC1		2DEC	.375		#  LAD/KC1 = .3/.8
OKC2		2DEC	.0074051405	# 2HS LAD(32.2)25/KC2(2VS)SQ   (KC2=.7)
...
GETUNB		RTB	3		# RESET PUSH COUNTER.
		...
		COS	1		# COS (ROLLC)
		VXSC			# GET UYA COS(ROLLC).  PUSH INTO LOC 6.
			ROLLC
			16D		#       AND PUSH.
