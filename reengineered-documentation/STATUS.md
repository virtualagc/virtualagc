...
| 0x019 | DSUB | Documented | High | 93/101 | Opcode confirmed ([##DRIVER.xpl] `XDSUB`); consolidates 9 separate HAL-1971 subscript-specifier instructions (ALC/ALCE/RALC + TASB/TIDX/TTSB + AASB/AIDX/ATSB + SASB/SIDX/STSB) into one opcode — see DSUB.md's Source Analysis. All rows of the primary-source subscript-kind table (index, asterisk, array/component to-partition, at-partition, CSZ cases) now empirically confirmed; ASZ traced instead to ADLP's operand list |
