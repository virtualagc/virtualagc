...
V16N85B		VN	1685
V06N40		VN	0640
P40OK99		OCT	24020		# BITS 14,12, AND5 FOR LINUS VERB 99
V16N40		VN	1640
## Page 643
OCT27/24	OCT	27
OCT53		OCT	53
...
MDOTHI		2DEC	.289391932 B-3	# 63.8 LB/SEC, SC.AT B+3 KG/CS   (=MDOTLO)
...
P47CSM		TC	BANKCALL	#   IMU STATUS CHECK
		...
		CALRB
			MIDTOAV2
		CAE	MPAC +1		# DELTA TIME TO RPEREAD (LESS THAN 1000
...
P47BOD		CAF	PRIO15		# LOWER PRIO THAN CALCN83 (20)
		TC	PRIOCHNG	#   TO PREVENT INTERRUPTSION OF CALCN83
		TC	PHASCHNG
...
IMPLBURN	CS	BIT13		# RESET STRULLSW (COULD BE AN IMPULSIVE
		...
		CS	BIT9		# RESET IMPULSW, ENGINOFF IS NOW SET UP
		MASK	FLAGWRD2
		TS	FLAGWRD2
...
DOSPSOFF	TCR	SPSOFF		# SHUT DOWN SPS, MASS UPDATES, ETC.
...
#					       ACTIVE. ONLY V97CNTR IS THEN RIGHT
		...
		MP	MDOTHI		#      MDOT, SC<AT B+3 KG/CS
		LXCH	A
		EXTEND
		MP	1SEC		# DEC 100
...
BESTTRIM	CAF	BIT15		# CHECK FOR SWITCHOVER, SELECT BEST TRIMS
		MASK	FLAGWRD9	#      FOR NEXT IGNITION (OR REIGNITION)
		EXTEND
## Page 652
		BZF	DBARTRIM	# PRE-SWITCHOVER
		TC	Q		# POST-SWITCHOVER, SO LEAVE TRIMS AS ARE

DBARTRIM	CAE	DELPBAR		# UPDATE TRIMS WITH DELP,YBAR
		TS	PACTOFF
		CAE	DELYBAR
		TS	YACTOFF
		TC	Q
...
FAILDSP		CAF	V06N40		# SPS ENGINE-FAILED V97 FLASH
		TC	BANKCALL
		CADR	GOFLASHR	# LINUS MAKES IT A REDO,  INHINT OK
		TCF	V97T		# TERMINATE
		TCF	V97P		# PROCEED
		TCF	V97E		# ENTER
		TCF	PASTERET
...
PASTERET	CAF	P40OK99		# IMMED RETURN - SET UP V99 OR V97
		TCR	LINUS
		TCF	ENDOFJOB
...
#					       LEAVE NARROW DEADBAND FOR REIGNITE
		TC	2PHSCHNG
		OCT	00026		# A, 6.2 = PRE40.6 (-0CS), CLOKTASK (1SEC)
		OCT	05014		# C, DELTAT NEXT, TASK BELOW, IN
		DEC	-0		# -0 CS.
## Page 662
...
#          WEIGHT/G   ANTICIPATED MAG. OF VEHICLE MASS     SP B16KGM
...
## Page 668
		...
		STODL	ZSCREF		# PD POINTER AT 4 NEW SCZ AXIS SCALED AT 2
		VDEF

		STODL	YSCREF		# PD POINTER AT0 NEW SC Y AXIS SCALED AT 2
			ZSCREF
		PDDL	PDDL
			YSCREF
			XSCREF
...
# FUNCTION    (1) UPDATES TEH VELOCITY-TO-BE-GAINED VECTOR.
...
K1VAL		2DEC	884.52887 B-23	# 19885 LB-SEC, SC.AT B+23 NEWTON-SEC/E+2
K2VAL		2DEC	293.137805 B-23	#  6590 LB-SEC, SC.AT B+23 NEWTON-SEC/E+2
## Page 681
K3VAL		2DEC	11.7766668 B-9	# 26475 LBS, SC.AT B+9 NEWTONS/E+4
...
MDOTLO		2DEC	.289391932 B-3	# 63.8 LB/SEC, SC.AT B+3 KG/CS   (=MDOTHI)
...
-4ACTDEG	DEC	-169		# -4 DEG, SC.AT 65.41 ARCSEC/BIT (-169D)
+2ACTDEG	DEC	+84		# +2 DEG, SC.AT 65.41 ARCSEC/BIT (+84D)
...
RCSDAPON	CAF	0.6SECT5	# 0.6 SEC ALLOWS TVCEXEC/ROLLDAP TO DIE
  +1		TS	TIME5		# ENTRY FROM R00TOPOO
