...
## Reference:   pp. 747-780
## Assembler:   yaYUL
## Contact:     Ron Burkey <info@sandroid.org>.
## Website:     www.ibiblio.org/apollo/index.html
## Mod history: 2016-11-17 JL   Created from Luminary131 version.
##		2016-12-01 RSB	Completed transcription.
##		2016-12-02 RSB	Fixed a bunch of typos in comments, which I discovered
##				by playing around with ProoferComments.py.  By my count,
##				I fixed 31 errors in 34 pages, almost all of them actual
##				textual errors, as opposed to whitespace errors ... i.e., 
##				on average, in this file anyway, there was an average of 
...
#	   TPASS4    TARGET INTERCEPT TIME              DP     B28 CS
# OUTPUT
...
#          UNIT/R/   POSITION AT TIG FOR ASTEER          VECTOR B1
...
#           S40.8 UPDATES THE VELOCITY-TO-BE-GAINED VECTOR, VG, COMPUTES THE TIME FOR ISSUING THE ENGINE OFF COMMAND,
#      TGO, AND CALLS THE ROUTINE "FINDCDUW", WHICH GENERATES THE STEERING COMMANDS FOR THE AUTOPILOT.  FOR "ASTEER"
#      TYPE MANEUVERS, SUBROUTINE "RASTEER1" IS CALLED EACH PASS TO GENERATE A NEW VELOCITY-TO-BE-GAINED.
...
#      IF VG . DELVREF IS NEGATIVE (VG AND DELVREF OVER 90 DEGREES APART), BYPASS TGO COMPUTATIONS, BUT DO FINDCDU
# (STEERING)  SET ALARM 1407 AND RETURN TO CALLER NORMALLY.
...
#		  F  ENGINE THRUST  (USED ONLY FOR APS CALCULATIONS)  IN KG-M/CS/CS AT +7
#		  FRCS4  4 JET RCS THRUST IN KG-M/CS/CS AT +7
...
S40.13		TC	INTPRET
		SETPD	CLEAR
			00D
			IMPULSW		# ASSUME STEERING UNTIL FOUND OTHERWISE
		VLOAD	ABVAL
...
S40.131		DLOAD	DMP
		...
		DAD	DDV
			K2VAL		# KG-M/CS AT +24
			K3VAL		# KG-M/CS/CS AT +10 (SAME VALUE AS FAPS)
S40.132		SET	EXIT
			IMPULSW
S40.132*	TC	TPAGREE
		CA	MPAC
...
# CALLING SEQ     VIA FINDVAC (FROM "STEERING", EVERY "TNEWA" SECONDS)

# INPUT	AND	  SEE INITVEL FOR A COMPLETE DESCRIPTION
#   OUTPUT
#		  RINIT     - ACTIVE VEHICLE RADIUS VECTOR      METERS +29
#		  VINIT	    - ACTIVE VEHICLE VELOCITY VECTOR     M/CS + 7
#			    - BOTH RINIT AND VINIT ARE VALID AT LAST PIPTIME (FROM SERVICER)
#		  DELLT4    - DESIRED TIME OF FLIGHT FROM RINIT TO RTARG IN CS  +28
#			    - THE PREVIOUS THREE QUANTITIES ARE SET EVERY GUIDANCE CYCLE AT "ENDSTEER"
#		  RTX1	    - -2 EARTH  -10D MOON  SET IN S40.1
#		  RTX2	    -  0 EARTH    2  MOON  SET IN S40.1

#		  THE ONLY OUTPUTS FROM INITVEL THAT ARE OF INTEREST HERE ARE:

#		  MUASTEER  - MU (GRAVITATIONAL CONSTANT) AT +42 EARTH,   +36 MOON
#		  MU/A      - RATIO  OF MU TO SEMI-MAJOR AXIS AT +14 EARTH,   +10  MOON


# RASTEER1

# FUNCTION        GENERATES REQUIRED VELOCITY AND VELOCITY-TO-BE-GAINED VECTORS FOR USE DURING AIMPOINT MANEUVERS.
# ALSO ADJUSTS FOR GRAVITATIONAL EFFECTS DUE TO OBLATENESS.

# CALLING SEQ     CALLED EVERY GUIDANCE PASS (DURING AIMPOINT MANEUVERS) FROM S40.8 AS AN INTERPRETIVE SUBROUTINE.

# INPUT
#   FROM S40.1    RTX2      - -2 FOR EARTH, -10D FOR MOON
#   FROM SERVICER UNIT/R/   - POSITION VECTOR HALF-UNIT
#		  RN	    - POSITION VECTOR  METERS AT +29
#		  VN1	    - VELOCITY VECTOR M/CS AT +7 EARTH OR +5 MOON
#		  UNITGOBL  - OBLATENESS TERM OF GRAVITY VECTOR
#		  PIPTIME   - TIME OF RN AND VN1  CS  AT +28
#   FROM INITVEL  RTARG     - TARGET VECTOR  METERS  AT +29
#		  RTMAG	    - MAGNITUDE OF RTARG    AT +29
#		  MUASTEER  - MU (SEE ABOVE, S40.9)
#		  MU/A	    - RATIO  OF MU TO SEMI-MAJOR AXIS (SEE S40.9)
#		  GEOMSGN   - 1 IF TRANSFER ANGLE < 180, -1 IF > 180
#   FROM TARGETTING
#		  NORMSW    - 1 IF TRANSFER ANGLE CLOSE TO 180, 0 IF NOT
#		  TPASS4    - DESIRED INTERCEPT TIME    CS  AT +28
#   FROM OTHERS   GOBLTIME  - NOMINAL (NOT ACTUAL) IGNITION TIME  CS AT+28
## Page 772

# OUTPUT          DELVEET3  - VELOCITY-TO-BE-GAINED   M/CS   AT +7
...
RASTEER1	VLOAD	ABVAL
		...
		VSU	RTB
			RN
			NORMUNX1
		STODL	IC		# UNIT(C)  C IS CHORD VECTOR
			36D		# C(36D) = ABVAL(C)
		XAD,2	SL*
			X1
			0,2
...
## Page 773
		...
		DSU	DMP
			...
			MU/A
		BDSU
			MUASTEER	# (MU - MU(S-C)/2A)
		PDDL	DSU
			SS
			RMAG		# (S-R)
		NORM	SR1
			X1	
		DDV	DMP		# (2(S-R)/RC) (MU - MU(S-C)/2A)
			R1C
		XSU,2	SL*
			X1
			1,2
		...
		NORM	SR1
			X1		
		DDV	DMP		# (2(S-R2)/RC)  (MU - MU S / 2A)  R2=RTARG
			R1C
		XSU,2	SL*
			X1
			1,2
...
## Page 774
		...
		BOV	
			+1
		SQRT	DMP		# SQRT(C/S) SQRT((S-C)/S)=(1/S)(SQRT(C
		SR3	BDSU		#		                 (S-C)))
		SIGN	PDDL            # SGNA(ARCSIN((S-C)/S)-(1/S)SQRT(C(S-C)))
			GEOMSGN		
			2PI+3
		SR2	DSU		# (PI/2) - SGNA(ABOVE MESS)
		DMP	PDDL
			SS
			SS	
		SR3	SQRT	
		DMP			# S SQRT(S) = SQRT(S CUBED)
		PDDL	SL3	
			MUASTEER
		SQRT	BDDV		# T MINIMUM ENERGY  (TM)
		DSU	DAD	
			TPASS4	
			PIPTIME
		STODL	30D		# (TM - T2)  FOR SIGN OF B
		SIGN			
			30D		# B WITH SIGN
		STORE	30D
		BON	VLOAD
