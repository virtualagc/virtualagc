/*
 * Copyright 2004,2005,2016 Ronald S. Burkey <info@sandroid.org>
 *
 ...
 *              09/29/16 RSB    Now that there are Block 2 programs with
 *                              less than 044 banks (such as Aurora 12),
 *                              had to modify this program to gracefully
 *                              accept that condition.  Also, had to
 *                              fix the format of this file-header, which
 *                              had become messed up.
 *
...
static unsigned Rope[044][02000] =
  {
    { 0 } };
...
getByte(FILE *fin)
{
  ...
  if (endOfFile)
  ...
  else
    {
      ...
      if (b == EOF)
        {
          fprintf(stderr, "File ended prematurely, after %ld bytes.\n",
              ftell(fin));
          endOfFile = 1;
          b = 0;
        }
...
main(int argc, char *argv[])
{
  ...
  printf("(c)2004,2005,2016 Ronald S. Burkey, ver " NVER
  ", built " __DATE__ "\n");
  printf(
