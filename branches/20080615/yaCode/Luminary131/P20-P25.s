...
NOWMATX		CALL
		...
		VXV	UNIT		# COMPUTE NORMAL TO CSM ORBITAL PLANE
			VSUBC		# NSUB1=UNIT(R(CM) CROSS V(GM))
...
P20LEM1		TC	PHASCHNG
		...
		INDEX	MPAC
		TC	+1
		TC	P20LEMA		# NORMAL RETURN WTIHIN 400 N M
...
P20LEMD		CAF	1500DEC
		TC	TWIDDLE		# WAITLIST FOR 5 SECONDS
		ADRES	P20LEMD1
		TC	ENDOFJOB
...
R22LEM3		CS	FLAGWRD1	# SHOULD W BYPASS STATE VECTOR UPDATE
...
R22LEM93	EXIT			# NORMAL EXIT FROM LSR22.3
		TC	PHASCHNG	# PHASE CHANGE TO PROTECT AGAINS
...
R21END		TC	DOWNFLAG
		...
		TC	LUNSFCHK	# ARE WE ON LUNAR SURFACE
		TC	P20LEMWT	# YES -- BYPASS V50 N 72 DISPLAY
...
MAGSUB		EXTEND
		...
		AD	0
		EXTEND
		BZMF	Q+2		# ABS(A) <= CONST GO TO L+1
...
RRSONLY		TS	RDES		# SHAFT COMMANDS WARE UNRESOLVED SINCE THIS
...
TRYSWS		TC	RMODINV		# (NOT RUPT INHIBIT)
...
RRDESNB		TC	MAKECADR
		...
		TC	DOWNFLAG	# RESET FLAG TO PREVENT DODES FROM GOINING
...
RRLIMNB		INDEX	Q		# THIS ROUTINE IS IDENTICAL TO RRLIMCHK
		# Was CAF --- RSB 2004.
		CA	0		# EXCEPT THAT THE MODE 1 SHAFT LOWER
...
RADSTART	TS	Q
 +1		EXTEND
		...
		CS	L
		TS	RADTIME		# SAVE THE NEGATIVE TIME OF READ
		TC	ISWRETRN
...
C13SLOOP	NOOP			# *** NECESSARY TO PREVENT A TC TRAP ***
		...
		AD	HALF
		AD	HALF		# CORECT FOR TIME OVERFLOW
		XCH	L
...
C13STALL	CAF	PRIO36		# PRIO36 = 36000 = FCADR (garbled)
		XCH	FBANK
		TCF	C13STAL1
...
#
# ALARMS:  521 -- COUNT NOT READ RADAR DATA (RR DATA GOOD DISCRETE NOT PRESENT BEFORE AND AFTER READING THE RADAR)
...
# STATE VECTORS) CHECKING FOR THE DATA GOOD DISCRETE AND MONITORING THE ANGLE BETWEEN TEH RADAR BORESIGHT AND THE
...
# ERASABLE INITIALIZATION REQUIRED:
#
#	NCR29FLG	READRFLG		(TO 1 AND 0 BY FRESH START) (RESET NOR29FLG TO LET SERVICER RUN R29)
