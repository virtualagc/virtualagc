...
**Important note:**  This branch ("schematics") of the repository pertains to AGC/DSKY electrical schematics and electronic simulations based in KiCad EDA versions 6 and above.  For corresponding KiCad version 5 schematics and simulations, switch instead to the "schematics-KiCad-5" branch.
...
**Note:** For all available schematics as of this writing (2025-02-15), schematics were first transcribed using the KiCad EDA system version 5, but then converted into the incompatible file-format used by KiCad version 6 and above.

Once converted to CAD files, the schematics can then be used for otherwise impossible things like:
...
Electrical schematics are available the open-source [KiCad](http://kicad-pcb.org/) series of programs, version 6 and above.  Specifically, the schematic-editor program in KiCad is called "eeschema", and we'll refer interchangably to "CAD", "KiCad", or "eeschema".  KiCad is an open-source (and free of charge) system for Linux, Mac, or Windows.
...
**Note:**  This section was written when KiCad 5 was the version of choice, and settings used in KiCad 6 or above have just been inherited.  I therefore don't know if the list below remains accurate.
