...
## In the original printout, "CS" is scribbled out in the following line.
## In Luminary 210, the comment has been corrected to 
## "KG-M/CS/CS AT +10 (SAME VALUE AS FAPS)", which is consistent
## with the correction.&mdash;RSB 2017			
...
## </pre>
		SQRT	SIGN		
## (GEOMSGN = SGNA) <i>or perhaps</i> (GEOMSGN = SGN1)
			GEOMSGN		
		STORE	32D		# + OR - A
## <pre>
##                  2 (S - R)      &mu; (S - C)
...
## SGNA( ARCSIN( SQRT((S-C)/S) ) - (1/S) SQRT(C(S-C)) )
		SIGN	PDDL
			GEOMSGN		
			2PI+3
## (C/S) (PI/2 - SGNA( <i>empty, aligned with the contents of SGNA above</i> ) )
		SR2	DSU		
		DMP	PDDL
			SS
			SS
...
#
# OUTPUT:
#	1.	CDUSPCT:	DOUBLE PRECISION CDU VECTOR, ORDERED Y,Z,X.
