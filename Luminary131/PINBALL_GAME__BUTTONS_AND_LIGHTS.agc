...
# (Note that the following has been COPIED from ERASABLE_ASSIGNMENTS.agc,
...
#DSPTAB		ERASE	+11D		# 0-10, DISPLAY PANEL BUFFER 11-13, C RELAYS.
...
CHANBITS	AD	OCT21
		EXTEND
		BZF	BITSOFF2	# DON'T TOUCH CHAN7 (SUPERBANK)
...
PUTNORM		TC	SETNADD		# ECADR FROM NOUNCADR.  (garbled) NOUNADD.
		CCS	DECBRNCH
		TC	PUTDECSF	# + DEC
		TC	DCTSTCYC	# +0 OCTAL
...
SFINTABR	CADR	GOALMCYC	# ALARM AND RECYCLE IF DEC LOAD
		...
		CADR	DPINSF
		CADR	DPINSF2
		CADR	DSPALARM	# LRPOSOUT CAN'T BE LOADED.
		...
		CADR	DSPALARM	# RRANGOUT CAN'T BE LOADED.
		CADR	DSPALARM	# RRDOTOUT CAN'T BE LOADED.
ENDRUTIN	EQUALS
...
ENDSCALE	INDEX	MIXBR		# IF ROUTINE NO. IS NOT CDU DEGREES,
		...
		TC	SFMIXCAL	# BY 360 DEGREES MINUS (garbled)
MIXBACK		TC	+2
		TC	SFNORCAL
NORBACK		CS	A
		AD	BIT2
...
PASTEOPT	TS	EDOP		# RIGHT 7
		CA	EDOP		# PLACE MONITOR VERB OR (garbled) VERB INTO
...
# DSPTAB ENTRY INTO DSREL.
		...
		COUNT*	$$/PIN
DSPIN		XCH	Q		# CAN'T USE L FOR RETURN, SINCE MANYOF THE 
...
DSPOCTIN	TC	DSPIN		# SO DSPOCTWO DOESN'T USE (garbled)
		CAF	+2
		TC	BANKJUMP
...
PROCKEY		CS	VERBREG		# DON'T ALLOW PROCEED DURING LOAD.
		AD	VBSP2LD*	# DEC 22
		EXTEND
		DIM	A
		EXTEND
...
## Page 461
		...
		TC	SUPDXCHZ +1	# DTCB WITH SUPERBIT (garbled)

TESTBIT		MASK	NVTEMP		# NVTEMP CONTAINS BLANK(garbled) CODE
		CCS	A
...
RECAL1		CAF	ZERO
		...
		TC	ENDOFJOB	# PATHELOGICAL CASE EXIT
		TC	DOTERM		# - TERMINATE
