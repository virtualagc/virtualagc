...
- The source code for unHALMAT.py is at ../virtualagc/yaShuttle/"Source Code"/PASS.REL32V0/unHALMAT.py.  It can be used as a pattern for the command-line arguments and their defaults for yaHALMAT2, as well as how to find the values of literals.
- Command-line options should be provided for associating device numbers with filenames and other file characteristics for HAL/S `READ` and `WRITE` commands.  For compatibility with HALSFC, I would recommend mimicking the `--ddi` and `--ddo` options described at https://www.ibiblio.org/apollo/HAL.html#moron.
- Command-line options should be provided for associating device numbers with filenames and other file characteristics for HAL/S `FILE`.  For compatibility with HALSFC, I would recommend mimicking the `--raf` option described at https://www.ibiblio.org/apollo/HAL.html#moron.
- yaHALMAT2 shall be a command-line program (rather than having a GUI), written in C, with source code and a Makefile in the subdirectory src/. The code and Makefile should be compatible with Linux, Mac OS, and Windows.  In Linux and Mac OS, the default C compiler should be `clang`, falling back on `gcc` if absent or if selected on the `make` command line.  In Windows, the default C compiler should be Visual C from a normal developer terminal, while in an MSYS2 terminal the default should be `clang` falling back on `gcc`, but again with the ability of the `make` command line to override the compiler selection.
- A `--debugger` option would be useful, in which a user could interactively step through HALMAT instructions, view "memory", set breakpoints, and so on, with HALMAT instructions displayed similarly to unHALMAT.py, along with optional interweaved HAL/S statements and AP-101S instructions.  The `gdb` debugger should be mimicked, in some useful subset, regarding the syntax and behavior of the user commands supported in the debugging mode.
- New primary documentation:  https://www.ibiblio.org/apollo/Shuttle/Shuttle%20GPC%20Software%20Model%20AP-101S.pdf, possibly useful for relating object code to HALMAT instructions to understand how HALMAT behaves at runtime.
- yaHALMAT2 should be able to accept multiple HALMAT files (and the other associated files like literals) on the command line, integrating them into a single program. Discover how to do this, document it, and implement it in yaHALMAT2.
- Correct any documentation from preceding phases, as needed.
