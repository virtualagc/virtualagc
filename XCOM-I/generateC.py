...
translated into C entities in a straightforward way as might be naively 
...
# this precisely *once* in HAL/S-FC -- that's false, by the way --, in PASS4, 
# where the code
...
def reserveLiteral(string):
    ...
    reservedMemory["numReserved"] += 1
    reservedMemory["nextReserved"] = nextReserved
    #print("@@@ '%s' %04X" % (string, descriptor), file=sys.stderr)
    #for i in range(length):
    #    print("%0X " % memory[nextReserved + 4 + i], end="", file=sys.stderr)
...
def generateC(globalScope):
    ...
    if not sizeReducer:
        ...
        for i in range(restart, physicalTop):
            ...
            if i < physicalTop - 1:
                ...
                if 7 == i % 8:
                    j = i & 0xFFFFF8
                    print(" // %8d 0x%06X" % (j, j), file=mf)
        print("   // %8d 0x%06X" % (physicalTop - 8, physicalTop - 8), end="", file=mf)
        print("\n};", file=mf)
    else:
        ...
        print("uint8_t memory[MEMORY_SIZE];", file=mf)
        print("uint8_t memoryInitializer[NUM_INITIALIZED] = {", file=mf)
        #print("@@@ numInitialized=%06X nextReserved=%06X" % (numInitialized, reservedMemory["nextReserved"]), file=sys.stderr)
    ...
    for address in memoryMap:
        ...
        if len(record) == 1 and "" == list(record)[0]:
            print("// Note that BASED %s has no RECORD" % symbol, file=mf)
        print("basedField_t based_%s[%d] = {" % (symbol, len(record)), file=mf)
        i = 0
        recordSize = 0
        for key in record:
    ...
    print("\n// Memory map, sorted by addresses in XPL memory -------------\n",\
          file=mf)
    print("memoryMapEntry_t memoryMap[NUM_SYMBOLS] = {", file=mf)
    i = 0
    USER_MEMORY = None
    PRIVATE_MEMORY = None
    for address in memoryMap:
        ...
        print('  { %s, "%s", "%s", %d, %d, %s, %d, %d, %d, %d, %d, %d, %d }%s' % \
              (memoryMap[address]["superMangled"], symbol, datatype, 
               ...
               parentAddress, common, library, comma), file=mf)
    print("};", file=mf)
    print("\n// Memory map, sorted by symbol name -------------------------\n",\
          file=mf)
    print("// Note that the collation indicated below is that of the", file=mf)
    print("// computer running XCOM-I, and may transparently change", file=mf)
    print("// at runtime on computers with different collation.", file=mf)
    print("memoryMapEntry_t *memoryMapBySymbol[NUM_SYMBOLS] = {", file=mf)
    for entry in sorted(memoryMapIndexBySymbol):
        print("  &memoryMap[%d]," % memoryMapIndexBySymbol[entry], file=mf)
    print("};", file=mf)
    print("\n// Sorted list of all labels (mangled) -----------------------\n",\
          file=mf)
    if len(mangledLabels) == 0:
        print("char *mangledLabels;", file=mf)
    else:
        print("char *mangledLabels[NUM_MANGLED] = {", file=mf)
        line = ""
    ...
    print("\n// Initial memory regions ------------------------------------",\
          file=mf)
    print("// Note that region 6 is often displaced downward by", file=mf)
    print("// by application code prior to allocation for any BASED", file=mf)
    print("// to leave a free region 7 at the top of memory.\n", file=mf)
    print("memoryRegion_t memoryRegions[%d] = {" % len(regions), file=mf)
    for i in range(len(regions)):
        if i > 0:
            print(",", file=mf)
        print("  { %d, %d }" % tuple(regions[i]), end="", file=mf)
        print(" /* (0x%06X, 0x%06X) */" % tuple(regions[i]), end="", file=mf)
    print("\n};", file=mf)
    # Keep mf open, since we haven't generated "reserved memory" yet, and still
    ...
    print("#define XCOM_NAME RSB-XCOM-I", file=f)
    ...
    bottomReserved = reservedMemory["nextReserved"] & ~0x07
    memorySize = 0x1000000
    sizeReserved = memorySize - bottomReserved
    print(file = f) # More of configuration.h
    print("extern uint8_t memoryInitializer[NUM_INITIALIZED];", file=f)
    print("#define BOTTOM_RESERVED 0x%06X" % bottomReserved, file=f)
    print("#define SIZE_RESERVED 0x%06X" % sizeReserved, file=f)
    print("extern uint8_t reservedMemory[SIZE_RESERVED];", file=f)
    f.close() # Close configuration.h
    # More of memory.c.
    print("uint8_t reservedMemory[SIZE_RESERVED] = {", file=mf)
    for i in range(bottomReserved, memorySize):
        if 0 == i % 8:
            print("  ", end="", file=mf)
        print("0x%02X" % memory[i], end="", file=mf)
        if i < memorySize - 1:
